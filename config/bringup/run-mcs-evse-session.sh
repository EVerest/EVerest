...
# config-session-mcs-evse.yaml. No button panels: EvseManager owns the basic signalling.
#
# Prerequisites:
...
EVEREST_CONFIG=$SCRIPT_DIR/config-session-mcs-evse.yaml
...
$TMUX send -t "$PANE_AUX" "echo 'aux pane - useful here:'; echo '  ip monitor link dev cb_plc      (carrier as McsDataLink sees it)'; echo '  ip neigh show dev cb_plc        (EV liveness view)'; echo '  tail -f /tmp/everest-logs/*     (EvseManager session log)'" ENTER
