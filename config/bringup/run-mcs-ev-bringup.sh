#!/usr/bin/env bash
# MCS bring-up, EV side: starts the ChargeBridge daemon (with its status dashboard),
# an EVerest instance (ev_board_support_API + McsEvDataLink) and the two bring-up
# panels (BUEvBoardSupport, BUEvSlac) in one tmux session.
#
# Prerequisites:
...
#  - the MCS ChargeBridge (EV role) reachable - config-CB-MCS-EV.yaml discovers it via
#    mDNS (ANY_EV) by default
#
# Usage: run-mcs-ev-bringup.sh [dist-prefix]
...
#
# This instance uses the MQTT prefix "everest_mcs_ev/" so it can share the broker with
# the EVSE side (run-mcs-evse-bringup.sh) on one host. Note that with both taps on one
# host, cross-link IP traffic (ping, connector-MAC via neighbour discovery) is
...
if [ -d "$SCRIPT_DIR/../../applications/pionix_chargebridge" ]; then
    ...
    DEFAULT_CB_CONFIG=$REPO_DIR/applications/pionix_chargebridge/config/${CB_CONFIG_NAME:-config-CB-MCS-EV.yaml}
else
...
EVEREST_CONFIG=$SCRIPT_DIR/config-bringup-mcs-ev.yaml
...
SESSION=mcs-ev-bringup
# Must match settings.mqtt_everest_prefix in the EVerest config (the manager normalizes
# a trailing '/', the module CLI takes it verbatim).
EV_PREFIX=everest_mcs_ev/

if [ ! -x "$PREFIX/bin/manager" ]; then
    echo "No EVerest manager under $PREFIX - build and install first" >&2
    exit 1
fi
if [ ! -d "$MODULES_DIR/BUEvSlac" ]; then
    echo "BringUp modules not installed under $PREFIX (missing $MODULES_DIR/BUEvSlac)" >&2
    exit 1
fi
...
# Already root or already capability-armed (run-mcs-ev-bringup-netns.sh arrives here as the
# invoking user with the caps ambient, inside the namespace): no prompt at all.
TMUX="tmux -L $SESSION"
...
# that never touch the wire. Refuse to stack a second instance; checked against both this run's
# config and the stock EV config name, because the netns wrapper hands in a derived temp config
# while a pre-netns zombie still carries the original one. Old-era zombies may be root-owned, so
# this reports instead of killing.
sleep 0.5
for pattern in "$(basename "$CB_CONFIG")" "config-CB-MCS-EV\.yaml"; do
    if pgrep -af "pionix_chargebridge.*$pattern" >/dev/null 2>&1; then
        echo "ERROR: a pionix_chargebridge matching '$pattern' is already running:" >&2
        pgrep -af "pionix_chargebridge.*$pattern" >&2
...
$TMUX send -t "$PANE_BSP" "sleep 4 && $ENV $MODULES_DIR/BUEvBoardSupport/BUEvBoardSupport --module bu_bsp --mqtt_everest_prefix $EV_PREFIX" ENTER
$TMUX send -t "$PANE_SLAC" "echo 'waiting for the daemon (tap cb_plc_ev)...' && until ip link show cb_plc_ev >/dev/null 2>&1; do sleep 0.5; done && sleep 2 && $ENV $MODULES_DIR/BUEvSlac/BUEvSlac --module bu_slac --mqtt_everest_prefix $EV_PREFIX" ENTER
