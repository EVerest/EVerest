...
# McsEvDataLink - from config-CB-EVAL-MCS-EV.yaml, plus mcs_ev_control, the FTXUI panel that
# drives the simulated EV (start / wake / stop / unplug, editable charge and sleep times). The
# EVerest config has auto_exec off: nothing happens until a button is pressed.
#
...
# output), the EV control panel to the right of the manager. The panel talks to EvManager over
# external MQTT; it honours MQTT_SERVER_ADDRESS/MQTT_SERVER_PORT (set by the netns wrapper - the
# broker sits on the veth, not on localhost, inside the namespace). The same commands by hand:
...
PANE_CONTROL=$($TMUX split-window -h -l 45% -t "$PANE_MANAGER" -PF '#{pane_id}')
