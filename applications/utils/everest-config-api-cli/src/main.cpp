...
int main(int argc, char** argv) {
    ...
    topics.setup("_", "configuration", 1);
    ...
    try {
        ...
        if (command == "list_slots") {
        ...
        } else if (command == "get_active_slot") {
            handlers.get_active_slot();
            ...
            if (cmd_args.empty()) {
                std::cerr << "Usage: set_description <slot_id> <description>\n";
                return 1;
            }
            handlers.set_description(std::stoi(cmd_args[0]), cmd_args[1]);
        } else if (command == "get_configuration") {
            if (cmd_args.empty()) {
                std::cerr << "Usage: get_configuration <slot_id> [--force-read-from-db]\n";
                return 1;
            }
            bool force_read_from_db = false;
            if (!cmd_args.empty() && cmd_args[1] == "--force-read-from-db") {
                force_read_from_db = true;
            }
            handlers.get_configuration(std::stoi(cmd_args[0]), force_read_from_db);
        } else if (command == "set_config_parameters") {
            if (cmd_args.size() < 2) {
                std::cerr << "Usage: set_config_parameters <slot_id> <filename>\n";
                return 1;
            }
            handlers.set_config_parameters(std::stoi(cmd_args[0]), cmd_args[1]);
        } else if (command == "get_config_parameters") {
            if (cmd_args.size() < 2) {
                std::cerr << "Usage: get_config_parameters <slot_id> <filename> [--force-read-from-db]\n";
                return 1;
            }
            bool force_read_from_db = false;
            if (!cmd_args.empty() && cmd_args[2] == "--force-read-from-db") {
                force_read_from_db = true;
            }
            handlers.get_config_parameters(std::stoi(cmd_args[0]), cmd_args[1], force_read_from_db);
