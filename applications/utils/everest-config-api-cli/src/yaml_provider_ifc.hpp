...
class YamlProviderIfc {
public:
    virtual ~YamlProviderIfc() = default;
    ...
    // Parses a YAML which contains a list of parameters (without values) and returns a get-parameter request object
    virtual GetConfigurationParameterRequest parse_parameter_requests(int slot_id,
                                                                      const std::filesystem::path& path) = 0;
};
