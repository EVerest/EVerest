...
void print_config_parameters(const GetConfigurationParameterRequest& req, const GetConfigurationParameterResult& res) {
    ...
    for (std::size_t i = 0; i < values.size(); ++i) {
        ...
        if (entry.status == ConfigurationParameterGetResultEnum::OK && entry.parameter.has_value()) {
...
void CommandHandlers::get_configuration(int slot_id, bool force_read_from_db) {
    std::cout << "Requesting the configuration for slot " << slot_id
              << (force_read_from_db ? "; force read from DB\n" : "\n");
    auto res = m_client->get_configuration(slot_id, force_read_from_db);
    if (!res) {
...
void CommandHandlers::set_config_parameters(int slot_id, const std::string& filename) {
    ...
    auto current_cfg = m_client->get_configuration(slot_id, false);
    if (!current_cfg) {
        std::cerr << "Failed to get configuration for slot " << slot_id << "; API did not respond.\n";
        return;
    }
...
void CommandHandlers::get_config_parameters(int slot_id, const std::string& filename, bool force_read_from_db) {
    std::cout << "Requesting the configuration parameters for slot " << slot_id
              << (force_read_from_db ? "; force read from DB\n" : "\n");
    ...
    req.force_read_from_db.emplace(force_read_from_db);
