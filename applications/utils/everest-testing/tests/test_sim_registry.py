...
# DSL replay
#
# A DSL thread outlives its test unless teardown ends it: `diode_fail` ends in
# `sleep 36000`, and a live thread keeps the controller, its MQTT client and the
# EverestCore reachable, file descriptors included.
# ---------------------------------------------------------------------------


_STATE_TOPIC = "pfx/everest_api/1/ev_simulator/ev_sim_1/e2m/state"


def _replay_in_background(drv, dsl):
    worker = threading.Thread(target=drv.play_dsl, args=(dsl,), daemon=True)
    worker.start()
    time.sleep(0.2)
    return worker


def test_iec_wait_pwr_ready_holds_until_the_simulator_reports_charging(config_path):
    # EvManager's iec_wait_pwr_ready held until the EVSE drove a PWM duty
    # cycle strictly between 7 and 97 percent, the condition on which an AcIec
    # EvSimulator enters Charging. The EV side never sees PowerOn.
    fake = _FakeMqttClient()
    drv = SimRegistry(fake, "pfx/", config_path).ev_by_connector(1)

    worker = _replay_in_background(drv, "iec_wait_pwr_ready;unplug")
    fake.deliver(_STATE_TOPIC, b'"Plugged"')
    time.sleep(0.2)
    assert worker.is_alive()

    fake.deliver(_STATE_TOPIC, b'"Charging"')
    worker.join(timeout=2.0)

    assert not worker.is_alive()
    assert fake.published[-1][0].endswith("/m2e/unplug")


def test_iec_wait_pwr_ready_queries_state_once_subscribed(config_path):
    # e2m/state is not retained, and Charging may already have been published
    # by the time the step runs.
    fake = _FakeMqttClient()
    drv = SimRegistry(fake, "pfx/", config_path).ev_by_connector(1)

    _replay_in_background(drv, "iec_wait_pwr_ready")

    subscribed = fake.events.index(("subscribe", _STATE_TOPIC))
    queried = next(i for i, (kind, topic) in enumerate(fake.events)
                   if kind == "publish" and topic.endswith("/m2e/query_state"))
    assert subscribed < queried
    drv._cancel.set()


def test_iec_wait_pwr_ready_ignores_charging_from_an_earlier_session(config_path):
    fake = _FakeMqttClient()
    drv = SimRegistry(fake, "pfx/", config_path).ev_by_connector(1)
    drv.subscribe_var("state")
    fake.deliver(_STATE_TOPIC, b'"Charging"')
    fake.deliver(_STATE_TOPIC, b'"Unplugged"')

    worker = _replay_in_background(drv, "iec_wait_pwr_ready")

    assert worker.is_alive()
    drv._cancel.set()


@pytest.mark.parametrize("dsl", ["sleep 30;unplug", "iec_wait_pwr_ready;unplug",
                                 "iso_wait_for_stop 30;unplug"])
def test_cancel_ends_a_dsl_step_and_skips_the_rest(registry, mqtt_client, dsl):
    drv = registry.ev_by_connector(1)
    worker = threading.Thread(target=drv.play_dsl, args=(dsl,))
    worker.start()
    time.sleep(0.2)
    ...
    assert not worker.is_alive()
    topics = [c.args[0] for c in mqtt_client.publish.call_args_list]
    assert not any(t.endswith("/m2e/unplug") for t in topics), topics


def test_cancel_before_a_dsl_starts_runs_none_of_it(registry, mqtt_client):
    registry.cancel_dsl()

    registry.ev_by_connector(1).play_dsl("unplug")

    assert mqtt_client.publish.call_count == 0


def test_stop_ends_the_dsl_threads_the_controller_started(controller_ev_only):
    ctrl = controller_ev_only
    ctrl._registry._ready = True
    already_running = set(threading.enumerate())
    ctrl.diode_fail(connector_id=1)
    ctrl.plug_in(connector_id=1)
    started = [t for t in threading.enumerate() if t not in already_running]
    assert len(started) == 2

    ctrl.stop()

    assert not any(t.is_alive() for t in started), started


def test_stop_releases_the_mqtt_client(controller_ev_only):
    ctrl = controller_ev_only
    client = ctrl._fake_client

    ctrl.stop()

    client.disconnect.assert_called_once()
    client.loop_stop.assert_called_once()
    client._reset_sockets.assert_called_once()
    assert ctrl._mqtt_client is None
