...
class NoSimulatorError(AssertionError):
    """The controller has no simulator instance to drive for a connector.

    Derives from AssertionError so a test that asks for a car to be plugged in
    when no car can be reached reports as a failure rather than carrying on
    against a charge point nothing is plugged into.
    """


def config_has_module(config_path: Path, module_name: str) -> bool:
    """True if the resolved everest config activates `module_name`."""
    try:
        ...
        return False
    return any(
        isinstance(block, dict) and block.get("module") == module_name
        for block in (data.get("active_modules") or {}).values()
    )


class _DslCancelled(Exception):
    """The registry cancelled DSL replay; the remaining steps are skipped."""
...
class EvSimDriver:
    ...
    def payload_count(self, var: str) -> int:
        """Payloads received on `var` so far; pass as `wait_for(after=...)`."""
        info = self._subs.get(var)
        with self._lock:
            return len(info.payloads) if info else 0

    def wait_for(self, var: str, predicate: Callable[[Any], bool], timeout: float = 10.0,
                 after: int = 0) -> Any:
        """Return the first payload on `var` past the first `after` that matches."""
    ...
    def play_dsl(self, dsl: str) -> None:
        ...
        Recognised ops (others are logged and skipped):
          ...
          iec_wait_pwr_ready            -> wait for state Charging
        ...
        `SimRegistry.cancel_dsl` ends a replay: the current wait or sleep
        returns early and the remaining steps are skipped.
        """
    ...
    def _run_one(self, tok: str) -> None:
        ...
        try:
            if head == "sleep":
                ...
                # An AcIec EvSimulator enters Charging on EvManager's condition, a
                # PWM duty cycle strictly between 7 and 97 percent. The query
                # covers a Charging published before this step subscribed, and
                # `after` skips one left over from an earlier session.
                self.subscribe_var("state")
                after = self.payload_count("state")
                self.query_state()
                self.wait_for("state", lambda s: s == "Charging", timeout=30, after=after)
...
class SimRegistry:
    ...
    def _load(self, config_path: Path) -> None:
        ...
        for mid, mblock in modules.items():
            ...
            if module_name == "EvSimulator":
                drv = EvSimDriver(self._mqtt, self._prefix, mid, cancel=self._cancel)
    ...
    def cancel_dsl(self) -> None:
        """End every DSL replay running on this registry's drivers, for good."""
        self._cancel.set()
