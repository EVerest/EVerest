...
def core_config(request) -> EverestEnvironmentCoreConfiguration:
    ...
    if not everest_prefix.is_absolute():
        # Interpret relative --everest-prefix values from the pytest root path
        # (typically the tests directory), not the process cwd.
        everest_prefix = (request.config.rootpath / everest_prefix).resolve()
