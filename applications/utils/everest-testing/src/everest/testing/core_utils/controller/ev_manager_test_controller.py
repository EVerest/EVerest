...
class EvManagerTestController(TestController):
    """Drives the legacy `EvManager` car simulator over its nodered topics.

    Kept selectable alongside the EvSimulator-backed `EverestTestController`
    so the old simulator stays usable as a control arm. `fixtures.test_controller`
    picks between the two from the resolved config.
    """
