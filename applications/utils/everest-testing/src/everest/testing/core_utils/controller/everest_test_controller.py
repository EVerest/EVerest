...
# A cancelled DSL step returns within one 50 ms poll; this only bounds a hang.
_DSL_JOIN_TIMEOUT_S = 5.0
...
class EverestTestController(TestController):
    ...
    def __init__(self,
        ...
        self._dsl_threads: list[threading.Thread] = []
    ...
    def stop(self, *exc_details):
        if self._registry is not None:
            self._registry.cancel_dsl()
        for thread in self._dsl_threads:
            thread.join(timeout=_DSL_JOIN_TIMEOUT_S)
            if thread.is_alive():
                log.warning("DSL thread %s did not end within %.1fs of cancel",
                            thread.name, _DSL_JOIN_TIMEOUT_S)
        self._dsl_threads.clear()
    ...
    #
    # One timing difference from the EvManager controller these replace, and it applies to
    # every plug_in* below. EvManager became plugged in when the DSL reached
    # iec_wait_pwr_ready, i.e. after the script's leading `sleep 1`. These publish the plug
    # synchronously and then start the DSL thread, so the vehicle is plugged about a second
    # earlier relative to whatever the test did before calling. A test that sequences
    # against the plug rather than waiting for a state will see the difference.
    def plug_in(self, connector_id=1):
        ...
        self._play_dsl(
            ...
            name=f"evsim-plugin-{connector_id}")
    ...
    def plug_in_ac_iso(self, connector_id=1, payment_type=""):
        ...
        self._play_dsl(
            drv,
            "sleep 1;iso_wait_slac_matched;iso_wait_pwr_ready;iso_draw_power_regulated 16,3;sleep 60;iso_stop_charging;iso_wait_v2g_session_stopped;unplug",
            name=f"evsim-plugin-acico-{connector_id}")
    ...
    def plug_in_dc_iso(self, connector_id=1, payment_type=""):
        ...
        self._play_dsl(
            drv,
            "sleep 1;iso_wait_slac_matched;iso_wait_pwr_ready;iso_wait_for_stop 60;iso_wait_v2g_session_stopped;unplug",
            name=f"evsim-plugin-dciso-{connector_id}")

    def _play_dsl(self, drv: EvSimDriver, dsl: str, name: str) -> None:
        thread = threading.Thread(target=drv.play_dsl, args=(dsl,), daemon=True, name=name)
        self._dsl_threads.append(thread)
        thread.start()
    ...
    def diode_fail(self, connector_id=1):
        # The fault fires only once the EVSE has reached PowerOn and the car is drawing
        # current, which is the scenario the legacy DSL built and the callers still expect:
        # a diode fault on an idle CP is a different test from one under load.
        drv = self._ev_drv(connector_id)
        ...
        self._play_dsl(
            ...
            name=f"evsim-diodefail-{connector_id}")
