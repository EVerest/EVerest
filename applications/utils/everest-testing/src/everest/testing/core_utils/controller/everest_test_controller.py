...
class EverestTestController(TestController):
    ...
    def start(self, broker_connect_timeout: float = _BROKER_CONNECT_TIMEOUT_S):
        ...
        if not self._connected.wait(broker_connect_timeout):
    ...
    def _on_broker_connect(self, *_args):
        # Signature differs between paho v1 and v2 callback APIs; nothing here
        # needs the arguments. Fires on the network thread.
        self._connected.set()
