...
class StateCollector:
    ...
    def wait_for_state(self, target: str, timeout: float = 30.0) -> bool:
        ...
        miss it and spuriously time out. A high-water mark taken at call
        entry then limits the scan to states observed from there on.
        Two deliberate exceptions to that mark, both matching on the
        current state rather than a new one: already being in `target`
        returns True immediately, and the timeout path accepts
        `last_state` as well. Returns True on a match, False on timeout.
        """
        deadline = time.time() + timeout
        with self._cv:
    ...
    def wait_for_next_state(
        ...
        """Return the next `e2m/state` value published after this call.

        The high-water mark is taken before `trigger` runs, so a command
        that is answered synchronously cannot have its answer mistaken for
        pre-existing history, and the lock is not held across `trigger` so
        an MQTT publish cannot block the paho callback thread. `states` is
        append-only, which is what keeps the pre-trigger mark valid.
        Returns None if nothing is published within `timeout`.
        """
        with self._cv:
            scanned = len(self.states)
        ...
        with self._cv:
            while True:
                if scanned < len(self.states):
                    return self.states[scanned]
                remaining = deadline - time.time()
                if remaining <= 0:
                    # scanned >= len(states) is established above.
                    return None
                self._cv.wait(timeout=remaining)
...
class EvSimulatorTestController:
    ...
    def start(self, connector_id: int = 1, timeout: float = 15.0) -> None:
        """Enable the simulator (publishes `m2e/enable` for `connector_id`).

        `timeout` bounds the re-publish retry below. It is a parameter so a
        unit test can exercise the give-up path without burning the full
        default; SIL callers should leave it alone.
        """
        ...
        # never leaves Disabled). The payload is a bare JSON true.
        #
    ...
    def stop(self) -> None:
        ...
        if self._mqtt_client is not None:
            close_mqtt_client(self._mqtt_client)
    ...
    def query_state(
        self, connector_id: int = 1, timeout: float = 5.0
    ) -> Optional[str]:
        """Publish `m2e/query_state` and return the state the simulator answers.

        This drives the real command: the simulator's handle_query_state
        replies with an `e2m/state` publish, which is the topic a logger or
        a UI reads. Returning the collector cache instead would leave the
        command with no coverage at all - the cache stays warm from ordinary
        transition publishes, so a cache read passes even if query_state
        were deleted outright.

        Returns the answering state, or None if the simulator does not
        answer within `timeout`. For the passive cache read, which does not
        touch the wire, use `last_observed_state()`.
        """
        del connector_id
        return self.state_collector.wait_for_next_state(
            timeout=timeout,
            trigger=lambda: self._publish("query_state", {}),
        )

    def last_observed_state(self, connector_id: int = 1) -> Optional[str]:
        """Return the last FSM state seen on `e2m/state` without querying.

        Passive cache read, no publish. Use `query_state()` when the point
        is to exercise the pull-model command.
        """
        del connector_id
