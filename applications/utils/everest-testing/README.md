...
- **test_controller**: Fixture that references the test_controller that can be used for control events for the test cases. This includes control over simulations that trigger events like an EV plug in, EV plug out, swipe RFID and more. Which controller it hands back is decided by the config under test: a config that activates `EvManager` gets the `EvManagerTestController` that drives it over its nodered topics, and anything else gets the `EvSimulator`-backed `EverestTestController`. A controller asked to drive an EV it has no simulator instance for fails the test rather than logging and returning.
