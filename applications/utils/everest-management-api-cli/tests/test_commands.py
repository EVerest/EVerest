...
from everest_management_api_cli import cli, commands, configuration_api, lifecycle_api
...
from everest_management_api_cli.session import Session
...
@pytest.mark.parametrize("argv", [["load"], ["getp", "1", "--file"], ["setp", "1", "--file"]])
def test_missing_file_is_reported(rpc_calls, tmp_path, argv):
    with pytest.raises(CommandError, match="cannot read"):
        run(argv + [str(tmp_path / "missing.yaml")])
    assert rpc_calls == []
...
def test_broker_defaults_come_from_the_environment(monkeypatch):
    monkeypatch.setenv("MQTT_SERVER_ADDRESS", "broker.local")
    monkeypatch.setenv("MQTT_SERVER_PORT", "18830")
    parser, _ = build_parser(prog="test")
    args = parser.parse_args(["slots"])
    assert (args.host, args.port) == ("broker.local", 18830)
    session = Session()
    assert (session.host, session.port) == ("broker.local", 18830)
    assert (Session("other", 1).host, Session("other", 1).port) == ("other", 1)
...
def test_watch_prints_latest_status_and_live_notices_until_duration(capsys):
    session = FakeSession()
    session.lifecycle._on_status(None, None, status_message({"everest_running": False}))
    run(["watch", "--duration", "0.05"], session)
    assert session.on_notice is None
    assert capsys.readouterr().out == '[status] {"everest_running": false}\n'
...
@pytest.mark.parametrize("text, message", [
    ...
    ("active_modules:\n  m:\n    config_module:\n      p:\n", "must not be empty"),
    ("active_modules:\n  m:\n    config_module:\n      p: null\n", "must not be empty"),
    ("active_modules:\n  m: [a, b]\n", "module 'm' must be a mapping"),
    ("active_modules:\n  m:\n    config_module: [a, b]\n", "config_module of module 'm' must be a mapping"),
    ("active_modules:\n  m:\n    config_module: 3\n", "config_module of module 'm' must be a mapping"),
    ("active_modules:\n  m:\n    config_implementation: [main]\n",
     "config_implementation of module 'm' must be a mapping"),
    ("active_modules:\n  m:\n    config_implementation:\n      main: 40\n",
     "implementation 'main' of module 'm' must be a mapping"),
])
