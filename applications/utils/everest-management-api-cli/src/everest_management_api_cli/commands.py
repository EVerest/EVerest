...
from .lifecycle_api import LIFECYCLE_STATUS_TOPIC, lifecycle_command_topic
...
def parse_parameter_file(text: str) -> List[ParameterUpdate]:
    ...
    for module_id, module in modules.items():
        if not isinstance(module, dict):
            raise CommandError(f"module {module_id!r} must be a mapping")
        for name, value in (module.get("config_module") or {}).items():
            updates.append((module_id, name, None, _scalar_to_value(value)))
        for implementation_id, parameters in (module.get("config_implementation") or {}).items():
...
def _read_parameter_file(path_text: str) -> List[ParameterUpdate]:
    path = Path(path_text).expanduser()
    try:
        text = path.read_text()
    except OSError as exc:
        raise CommandError(f"cannot read {path}: {exc}")
    return parse_parameter_file(text)


def _parameter_ids(args, need_value: bool) -> Tuple[List[ParameterId], List[str]]:
    """Resolve the parameter selection of getp/setp: either one parameter given positionally or a
    file with many. Returns (identifiers, values); values is empty for getp."""
    if args.file is not None:
        if args.module_id is not None:
            raise CommandError("give either --file or module_id/parameter, not both")
        updates = _read_parameter_file(args.file)
        return [(m, p, i) for m, p, i, _v in updates], [v for _m, _p, _i, v in updates]
    ...
    return [(args.module_id, args.parameter, args.implementation_id)], [args.value] if need_value else []


def cmd_getp(session: Session, args) -> dict:
    parameters, _values = _parameter_ids(args, need_value=False)
...
def cmd_setp(session: Session, args) -> dict:
    parameters, values = _parameter_ids(args, need_value=True)
    updates = [(m, p, i, v) for (m, p, i), v in zip(parameters, values)]
...
def cmd_load(session: Session, args) -> dict:
    ...
    try:
        ...
        raise CommandError(f"cannot read {path}: {exc}")
    raw_yaml = prepare_yaml_for_slot(text, args.as_is)
...
def add_commands(subparsers) -> None:
    ...
    def impl_arg(parser: argparse.ArgumentParser) -> None:
    ...
    impl_arg(parser)
    parser.add_argument("-f", "--file", metavar="FILE", help="YAML file listing the parameters to query")
    db_arg(parser)
    ...
    impl_arg(parser)
    parser.add_argument("-f", "--file", metavar="FILE", help="YAML file with the parameters and values to set")
...
def build_parser(prog: Optional[str] = None,
    ...
    if include_connection_options:
        ...
        parser.add_argument("--host", default=os.environ.get("MQTT_SERVER_ADDRESS", "127.0.0.1"),
                            help="MQTT broker host (default: $MQTT_SERVER_ADDRESS or %(default)s)")
        parser.add_argument("--port", type=int, default=int(os.environ.get("MQTT_SERVER_PORT", "1883")),
