...
// when the mix moved by vcan_rate_hysteresis_pct (see vcan_rate_decrease_period).
...
// Admitting more than the bus carries loses frames at the CB, admitting less only costs
// throughput: decreases apply after vcan_rate_decrease_period (well below the ~35 ms the CB's TX
// ring holds), increases after vcan_rate_update_period. Both need a move of
// vcan_rate_hysteresis_pct, above the EWMA's flush-to-flush jitter on a bus with mixed payload
// lengths (~2 % for 0/8 byte frames); a 1 % threshold re-applied the rate on a third of the
// flushes. The info line is throttled to one per vcan_rate_log_period.
constexpr auto vcan_rate_decrease_period = std::chrono::milliseconds(10);
constexpr auto vcan_rate_log_period = std::chrono::seconds(1);
...
can_bridge::can_bridge(can_bridge_config const& config, everest::lib::io::event::event_fd& ready_notify) :
    ...
    m_can->set_rx_handler([this](auto const& data, auto&) {
        ...
        // queue.
        auto const full = can_udp_batch_max * sizeof(cb_can_message);
        if (m_can_batch.buffer.size() >= full) {
            // Pacing holds a full batch but pause_rx() did not stick (e.g. reopen in progress): a
            // 13th record would overflow the MCU's datagram buffer.
            flush_can_batch();
            if (m_can_batch.buffer.size() >= full) {
                return; // still paced: drop, as the kernel queue would have
            }
        }
        append_to_can_batch(data);
        auto const& raw = m_can->get_raw_handler();
        everest::lib::io::can::socket_can::ClientPayloadT more;
        while (m_can_batch.buffer.size() < full and raw and raw->rx(more)) {
...
void can_bridge::update_vcan_rate_limit() {
    ...
    if (wanted < installed) {
        if (now - m_vcan_rate_updated < vcan_rate_decrease_period) {
            return;
        }
        if ((installed - wanted) * 100 < installed * vcan_rate_hysteresis_pct) {
            return;
        }
    } else {
    ...
    if (manager.set_transmit_rate_limit(m_can_device, wanted, vcan_burst_bytes, vcan_limit_bytes)) {
        ...
        if (now - m_vcan_rate_logged >= vcan_rate_log_period) {
            m_vcan_rate_logged = now;
            utilities::print_info(m_identifier, "CAN/BUS")
                << "bus emulation: " << static_cast<unsigned>(m_avg_wire_bits) << " wire bits/frame -> vcan rate "
                << wanted << " bit/s (" << (wanted * 100 / m_pace_bitrate_bps) << " % of " << m_pace_bitrate_bps << ")"
                << std::endl;
        }
    }
}
