...
.. code-block:: bash
    ...
    Commands:
      ...
      get_configuration <slot_id> [--force-read-from-db]
      set_config_parameters <slot_id> <filename>
      get_config_parameters <slot_id> <filename> [--force-read-from-db]
...
.. code-block:: bash
    ...
    Changed 5 of 5 parameter(s), 4 applied.
...
.. code-block:: bash
    ...
    example|example|enum_test2 -> 1 ("WillApplyOnRestart")
...
.. code-block:: bash
    ...
    ...
    2026-05-15 14:20:07.605870 [INFO] example:Example  :: Cfg Update for 'log_interval' | old == '10', new == '3' accepted
    2026-05-15 14:20:07.716455 [INFO] example:Example  :: Cfg Update for 'enum_test' | old == 'one', new == 'two' accepted
    2026-05-15 14:20:07.772711 [INFO] example:Example  :: Cfg Update for 'current' | old == '42.000000', new == '40.000000' accepted
    2026-05-15 14:20:07.869164 [INFO] example:Example  :: Cfg Update for 'enum_test' | old == 'one', new == 'two' accepted
...
To query a specific set of configuration parameters values use `get_config_parameters` similar to
`set_config_parameters`:

.. code-block:: bash

    $ ./dist/bin/everest-config-api-cli get_config_parameters 0 cfg_update_to.yaml
    example|log_interval                               ["Integer"] : 3
    example|enum_test                                  ["String"] : two
    example|example|current                            ["Decimal"] : 40.000000
    example|example|enum_test                          ["String"] : two
    example|example|enum_test2                         ["Integer"] : 2

To query a configuration parameters value that will only become active after reboot, use the
``--force-read-from-db``:

.. code-block:: bash

    $ ./dist/bin/everest-config-api-cli get_config_parameters 0 cfg_update_to.yaml --force-read-from-db
    example|log_interval                               ["Integer"] : 3
    example|enum_test                                  ["String"] : two
    example|example|current                            ["Decimal"] : 40.000000
    example|example|enum_test                          ["String"] : two
    example|example|enum_test2                         ["Integer"] : 1

(Notice the different value for `example|example|enum_test2`)
...
When the manager shuts down it publishes this "EVerest Running" = `false` status itself, right before
...
indistinguishable to a client. As the messages on this topic are retained, they will also be
available if the client is only started now.
