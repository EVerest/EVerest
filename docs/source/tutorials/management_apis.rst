...
.. code-block:: bash
    ...
    $ ./dist/bin/everest-config-api-cli show_slot_metadata
    Slot metadata:
      Slot ID      : 0
...
.. important::
    ...
    Only the module configuration is stored and retrievable. Thus it's not possible
    to reconstruct the yaml file which may have been used to initially load this
...
.. code-block:: bash
    ...
    Available slots:
      [0] <no description>
      [1] SIL setup
    $ ./dist/bin/everest-config-api-cli show_slot_metadata 3
    Slot metadata:
      Slot ID      : 1
...
which have actual changes. E.g. the following example shows a file which is used to change
the `log_interval` parameter of the `example` module to the value `3` and the `current`
parameter of the `example` implementation to `40`.
...
Now create a duplicate from an existing slot and modify the newly created configuration.

.. code-block:: bash

    $ ./dist/bin/everest-config-api-cli duplicate_slot 1 "Created as duplicate of slot 1"
    Successfully duplicated to slot 2 with description: Created as duplicate of slot 1
    $ ./dist/bin/everest-config-api-cli set_config_parameters 2 config-update.yaml
...
change becomes effective. Also take notice, that both changes will only become active on
the next restart. This is because the slot is not active.

Requesting the same changes to the active slot gives different results:

.. code-block:: bash

    $ ./dist/bin/everest-config-api-cli set_config_parameters 0 config-update.yaml
...
.. code-block:: bash
    ...
    example|!module|log_interval -> 2 ("Applied")
...
.. code-block:: bash
    ...
    ...
    2026-05-15 11:16:29.699109 [INFO] example:Example  :: Config log actual [original value]: log_interval=10 [10]; enum_test="one" ["one"]; example|current=42.000000 [42.000000]; example|enum_test="one" ["one"]; example|enum_test2="2" ["2"]
    2026-05-15 11:16:35.309748 [INFO] example:Example  :: Cfg Update for 'log_interval' | old == '3', new == '3' accepted
    2026-05-15 11:16:35.415926 [INFO] example:Example  :: Cfg Update for 'current' | old == '42.000000', new == '40.000000' accepted
    2026-05-15 11:16:39.699526 [INFO] example:Example  :: Config log actual [original value]: log_interval=3 [10]; enum_test="one" ["one"]; example|current=42.000000 [42.000000]; example|enum_test="one" ["one"]; example|enum_test2="2" ["2"]
    ...
...
First start the lifecycle CLI client in monitoring mode:
...
.. code-block:: bash
    ...
    --------------------------------------------------------------------------------
    Timestamp                     Module Status       Configuration API                
    --------------------------------------------------------------------------------
    2026-06-01T21:31:21.536Z      "Running"           "N_A"

If you add ``--configuration-api=ro/rw`` when calling the run script the output changes accordingly.

Read-Write Usage
****************

If you stopped the monitoring CLI client, restart it as show above.
...
Note, that the client instance which issued the `stop_modules` command only reports, that the command
has been accepted and the modules are going to stop, but it will not report when this has completed.
Observe the log of the monitoring API client from above.
...
.. code-block:: bash
    ...
    $ ./dist/bin/everest-config-api-cli set_config_parameters 1 config-update.yaml
    example|log_interval                               ["Integer"] : 10 -> 3  "WillApplyOnRestart"
    example|example|current                            ["Decimal"] : 42.000000 -> 40  "WillApplyOnRestart"
    $ ./dist/bin/everest-config-api-cli mark_active_slot 1
    Successfully marked slot 1 as active.
...
Now the 17 module SIL configuration is active.
