...
class charger_information_consumer_API : public Everest::ModuleBase {
public:
    charger_information_consumer_API() = delete;
    charger_information_consumer_API(const ModuleInfo& info, Everest::MqttProvider& mqtt_provider,
                                     std::unique_ptr<generic_errorImplBase> p_main,
                                     std::unique_ptr<charger_informationIntf> r_charger_information, Conf& config) :
        ModuleBase(info),
        mqtt(mqtt_provider),
    ...
    const std::unique_ptr<charger_informationIntf> r_charger_information;
    const Conf& config;
...
private:
    ...
    void generate_api_cmd_get_charger_information();
    ...
    ev_API::CommCheckHandler<generic_errorImplBase> comm_check{
        "generic/CommunicationFault", "Bridge to implementation connection lost", this->p_main};
    size_t hb_id{0};
