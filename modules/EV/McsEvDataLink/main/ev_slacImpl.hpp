...
private:
    ...
    void run_event_loop();
    /// Run \p post on the controller with the lifecycle monitor held for the whole call.
    /// \return what \p post returned, or false (with a warning naming \p command) if the controller is unusable.
    ...
    /// Lifecycle gate plus fault state; the latter shares the monitor since raise/clear run on both thread kinds.
    struct LifecycleState : everest::lib::util::LifecycleStateT<datalink_controller> {
        bool communication_fault_raised{false};
        /// Its message; a changed reason replaces the error.
