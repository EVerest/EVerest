...
public:
    ...
    //
    // `loop` marks the block as a loop segment. It is a parameter rather than a follow-up call to
    // mark_loop because this function dispatches everything already due before it returns: a block
    // whose first step is at offset 0 would be drained, and the range marked afterwards would name
    // steps the dispatcher had already passed, so the loop would never repeat and nothing would
    // say so.
    //
    // Returns the index at which the block was placed; it stays contiguous. A pending step falling
    // inside the block's span fires after the block. If the dispatcher is idle (no prior start()),
    // this seeds the step-list.
