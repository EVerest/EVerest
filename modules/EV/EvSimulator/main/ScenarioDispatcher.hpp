...
public:
    ...
    // Splice steps into the running (or to-be-started) step-list, returning the index the
    // block was placed at; it stays contiguous, and a pending step inside its span fires
    // after it.
    //
    // Offsets are relative to THIS call, not to scenario start, and are rebased. The block
    // goes in at the position its first step is due, not at the end: the dispatcher advances
    // by index, so a block parked behind later steps would arrive as one past-due burst.
    //
