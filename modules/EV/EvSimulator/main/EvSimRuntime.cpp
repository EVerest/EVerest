...
EvSimRuntime::~EvSimRuntime() {
    // Stop taking events before anything is torn down. The eleven peer subscriptions (BSP, SLAC,
    // ISO) cannot be detached: the generated interface hands back no token, so a publication can
    // still arrive here while the queue and the eventfd below are being destroyed. This costs one
    // relaxed load on a path that is already pushing to a queue, and it is correct whatever the
    // framework does or does not guarantee about the order it shuts modules down in.
    accepting_events.store(false);
...
void EvSimRuntime::on_wake() {
    ...
    while (auto e = queue.try_pop()) {
        ...
        try {
            publish_passthrough_external(*ctx, *e, mod.config.publish_bsp_measurements);
        } catch (const std::exception& ex) {
...
void EvSimRuntime::tick_step(FsmContext& ctx, std::chrono::steady_clock::time_point now) {
    ramp_step(ctx, now);
    soc_step(ctx);
}
...
void EvSimRuntime::on_tick() {
    ...
    try {
        tick_step(*ctx, std::chrono::steady_clock::now());
    } catch (const std::exception& ex) {
...
void EvSimRuntime::apply_passthrough_vars(FsmContext& ctx, const Event& ev) {
    using K = EventKind;
    switch (kind_of(ev)) {
    case K::EvInfo:
...
void EvSimRuntime::publish_passthrough_external(FsmContext& ctx, const Event& ev, bool publish_bsp_measurements) {
    using K = EventKind;
    const auto kind = kind_of(ev);

    // The ISO lifecycle edges are surfaced here, on the way past the FSM, rather than from the
    // states: IsoV2GFinished reaches Charging in the silent-ignore arm, so a state-side publish
    // would never fire for the edge the harness waits on.
    if (const auto iso_kind = iso_session_event_kind(kind)) {
        publish_iso_session_event(ctx, *iso_kind);
    }
...
void EvSimRuntime::publish_iso_session_event(FsmContext& ctx, API_types::ev_simulator::IsoSessionEventKind kind) {
    API_types::ev_simulator::IsoSessionEvent event{kind, std::nullopt, std::nullopt};
    if (ctx.vars.dc_present_voltage_v > 0.0f) {
        event.dc_voltage_v = ctx.vars.dc_present_voltage_v;
    }
    if (ctx.vars.evse_dc_present_current_a.has_value()) {
        event.dc_current_a = ctx.vars.evse_dc_present_current_a;
    }
    ctx.publish_e2m_iso_session_event(event);
}
