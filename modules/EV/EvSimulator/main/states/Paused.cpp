...
void Paused::enter() {
    ...
    // Hold CP at C until the pause PowerDeliveryReq is on the wire, which the
    // peer reports through feed(V2gMessage) once the request has been sent.
    // ISO 15118-2 [V2G2-533] makes WeldingDetectionReq the next message after
    // PowerDeliveryRes(Stop), and [V2G2-920]/[V2G2-921]/[V2G2-922] oblige the
    // SECC to measure CP state B on receiving it and to answer FAILED if it
    // does not, so the window is one message round-trip wide.
    //
    // The IsoV2GFinished backstop is far too late on its own: it fires at the
    // transport teardown, 8.8 s after PowerDeliveryReq, by which point the EV
    // has answered four welding exchanges and a SessionStop while still
    // claiming to be in the charge loop.
    //
    // With no live session there is no PowerDeliveryReq to wait for.
