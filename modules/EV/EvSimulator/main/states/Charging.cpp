...
void Charging::enter() {
    ...
    if (ctx.vars.session && ctx.vars.session->pending_curve.has_value()) {
        ...
        // The block is spliced in at its due position, not appended, so the dispatcher decides
        // where it lands; it also marks the loop itself, before dispatching anything.
        ctx.scenario.append_steps(std::move(curve_steps), curve.loop, ctx);
...
StateBase::Result Charging::feed(EventType ev) {
    ...
    // Ignored while charging, as in every sibling state. IsoAcMaxCurrent is the one kind this
    // state acts on, so it has a body of its own below rather than sharing this group.
