...
void setup_peer_subscriptions(EvSimRuntime& rt, EvSimulator& mod) {
    ...
    if (!mod.r_ev.empty()) {
        ...
        // Per-message V2G progress from the EVCC. On the send side the peer
        // publishes this after the message has actually gone out, so a
        // PowerDeliveryReq here means that request is on the wire. Only the id
        // is forwarded, as a string: no FSM decision needs the decoded JSON or
        // the raw EXI, and both are debug-only on the wire anyway.
        iso->subscribe_v2g_messages([&rt](const ::types::iso15118::V2gMessages& m) {
            rt.enqueue(Event{V2gMessagePayload{::types::iso15118::v2g_message_id_to_string(m.id)}});
        });
    }
}
