...
void FsmContext::clear_error(const ClearErrorCmd& cmd) {
    ...
    //
    // Only a clear that names a sub-type can be pre-checked. Without one the request is the
    // wildcard that clears every sub-type of the error, and is_error_active matches the exact
    // (type, sub_type) pair, so checking it against "" would reject every error that was raised
    // with a sub-type and tell the operator, wrongly, that nothing was active.
    if (cmd.sub_type.has_value() and peer_actions.error.is_active and
        not peer_actions.error.is_active(cmd.type, *cmd.sub_type)) {
...
bool FsmContext::validate_session_config(const API_types::ev_simulator::SessionConfigParams& sp,
    ...
    if (curve.has_value()) {
        ...
        // A looping curve rewinds to its loop start and takes the next step's delay from there.
        // With one point that delay is always zero, so the dispatcher re-arms itself with no time
        // passing and spins. Two points are the fewest that can describe a period.
        if (curve->loop and curve->points.size() < 2) {
            reject_reason = "a looping curve needs at least two points";
            return false;
        }
    }
...
StateBase::Result transition_to_disabled(FsmContext& ctx) {
    ...
    // As in Unplugged::enter. A scenario left running fires into Disabled::feed, which answers
    // every event with a rejection, so the operator sees a stream of refusals from a module they
    // disabled on purpose.
    ctx.scenario.reset();
