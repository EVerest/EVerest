...
public:
    ...
    // Dropped once the destructor has run: a peer publication arriving then would push into a
    // queue that is being destroyed. See ~EvSimRuntime.
...
private:
    ...
    // Cleared first thing in the destructor. The peer subscriptions have no detach token, so this
    // is the only thing that stops a late publication pushing into a queue that is going away.
...
// The tick composition and the pre-feed var writes, as one definition each. on_tick() and on_wake()
// add only the ctx-null guard and the exception isolation around these, which the SIL smokes cover.
// The unit tests call them directly so that what they pin is the production composition; a copy in
// the test file would pass while this one was broken.
#ifdef BUILD_TESTING_MODULE_EV_SIMULATOR
public:
#endif
    static void tick_step(FsmContext& ctx, std::chrono::steady_clock::time_point now);
    static void apply_passthrough_vars(FsmContext& ctx, const Event& ev);
    static void publish_passthrough_external(FsmContext& ctx, const Event& ev, bool publish_bsp_measurements);
    static void publish_iso_session_event(FsmContext& ctx, API_types::ev_simulator::IsoSessionEventKind kind);
};
