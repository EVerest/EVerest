...
void force_internal_fault(std::unique_ptr<fsm::v2::FSM<StateBase>>& fsm, FsmContext& ctx, const std::string& message) {
    ...
    fsm.reset();
    // Built into a local and swapped in whole. The FSM constructor runs Faulted::enter(), so
    // assigning the make_unique result directly would leave `fsm` null for good if that threw,
    // and every later event would then be dropped silently behind the null check in the caller.
    auto replacement = std::make_unique<fsm::v2::FSM<StateBase>>(std::make_unique<Faulted>(ctx));
    fsm = std::move(replacement);
}
