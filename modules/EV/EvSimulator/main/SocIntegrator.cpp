...
// Milliseconds to hours, so power [W] * factor [h] is energy in [Wh].
// Must match EvManager's MS_FACTOR in car_simulation.cpp.
constexpr double MS_FACTOR = (1.0 / 60.0 / 60.0 / 1000.0);

namespace api = everest::lib::API::V1_0::types::ev_simulator;

// Mirrors the charge-mode switch in EvManager's CarSimulation::simulate_soc.
...
void soc_step(FsmContext& ctx) {
    ...
    // discharge is active, so EvManager's "skip accumulation when already over
    // capacity" short-circuit is deliberately not mirrored here: it silently
    // dropped discharge energy when the battery started above capacity. The post-integration clamp keeps
    // battery_charge_wh in [0, capacity], handling overshoot on charge and underflow on discharge symmetrically.
