...
std::size_t ScenarioDispatcher::append_steps(std::vector<ScenarioStep> steps, bool loop, FsmContext& ctx) {
    ...
    // whose first point is at offset 0 would be drained and next_idx_ past it before a caller
    // that marks afterwards could say it loops, and the loop would silently never repeat.
    if (loop) {
        mark_loop(insert_idx, insert_idx + n);
    }
