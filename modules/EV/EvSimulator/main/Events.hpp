...
// The V2G message the EVCC has just sent or received, forwarded from the ISO
// peer's v2g_messages var. Only the message id is carried: the decoded JSON and
// the raw EXI are debug-only on the wire and no FSM decision needs them. On the
// send side the peer publishes this AFTER the message has actually gone out, so
// seeing PowerDeliveryReq here means "that request is on the wire", which is
// what ISO 15118-2 [V2G2-920] ties the control-pilot B transition to.
struct V2gMessagePayload {
    std::string id;
};
...
// The externally published ISO lifecycle edge an EventKind stands for, or nullopt when it is not
// one. The harness waits on these edges, so they are published from the dispatch loop rather than
// from the states: several of them reach the state that is running in its silent-ignore arm.
std::optional<API_types::ev_simulator::IsoSessionEventKind> iso_session_event_kind(EventKind kind);
