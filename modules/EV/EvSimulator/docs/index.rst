...
.. list-table::
   ...
   * - ``set_present_values``
     - ``SetPresentValues``
     - ``SetPresentValuesParams`` (present_voltage?, present_active_power?).
       Per field: an unset field leaves that override alone rather than clearing
       it. An override that is never set reports the value the EVSE delivered.
       Cleared on entry to ``Unplugged``.
   * - ``inject_fault``
     - ``InjectFault``
...
- ``iso_session_event`` — ``IsoSessionEvent`` (kind + optional ``dc_voltage_v``
  and ``dc_current_a``) on each ISO lifecycle edge the ISO peer reports:
  ``PowerReady``, ``DcPowerOn``, ``StopFromCharger``, ``PauseFromCharger`` and
  ``V2GFinished``. Published on the way past the FSM, so an edge a state ignores
  still reaches the topic.
...
policies diverge only below 100. ``tests/core_tests/evsim_battery_full_test.py``
is the in-tree consumer that selects a non-default policy and drives it end to
end, over ``config-sil.yaml`` with ``EVSIM_BATTERY_FULL_SESSION`` applied.
...
- ``evsim_bpt_mcs_test.py`` — ``DcIsoBpt`` over ``config-sil-dc-d20.yaml`` and
  ``DcIsoMcs`` against ``config-sil-mcs.yaml``.
- ``evsim_battery_full_test.py`` — ``on_battery_full: stop_session`` at an
  80 % threshold, plus the ``m2e/query_state`` wire round trip.
- ``evsim_ac_iec_test.py`` — AC IEC 61851 basic charge plus PWM pause/resume,
  no SLAC / V2G negotiation, and a bare plug with no prior
  ``configure_session``, which is the only path that exercises the FSM
  synthesizing a session from the module's own config defaults.
...
The suites run on the mainline SIL configs under ``config/``, adjusted at test
time by ``tests/core_tests/evsim_config_adjustment.py``. There is no dedicated
EvSimulator config: five used to exist, each a copy of a mainline sibling, and
read against this module's manifest almost every line they added was the module
default written out again. What actually differs per suite is a persistent store
for the cross-boot plug state, the node-red interface a headless run does not
use, and a dozen settings named in that file.

A further 24 mainline SIL and OCPP configs under ``config/`` also instantiate
the module.
...
.. list-table::
   ...
   * - Suite
     - Config
     - Adjustment
   * - ``evsim_ac_iec_test.py``
     - ``config-sil.yaml``
     - defaults
   * - ``evsim_timing_override_test.py``
     - ``config-sil.yaml``
     - defaults
   * - ``evsim_curve_test.py``
     - ``config-sil.yaml`` / ``config-sil-dc.yaml``
     - ``EVSIM_DC_SESSION`` for the DC case
   * - ``evsim_battery_full_test.py``
     - ``config-sil.yaml``
     - ``EVSIM_BATTERY_FULL_SESSION``
   * - ``evsim_dc_iso2_test.py``
     - ``config-sil-dc.yaml``
     - ``EVSIM_DC_SESSION``, DIN 70121 off
   * - ``evsim_d20_test.py``
     - ``config-sil-dc-d20.yaml``
     - ``EVSIM_DC_SESSION``
   * - ``evsim_bpt_mcs_test.py``
     - ``config-sil-dc-d20.yaml`` / ``config-sil-mcs.yaml``
     - ``EVSIM_DC_SESSION``, ``DC_BPT,DC`` services
