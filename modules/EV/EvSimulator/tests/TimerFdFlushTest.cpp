...
//
// Flush-after-disarm. A non-blocking timer_fd retains a pending fire until read(), and
// set_timeout_ms(0) cancels future fires without consuming an already-pending one, so
// the runtime must read() after disarm or dispatch a stale handler against an unrelated
// state. disarm_and_flush classifies the post-disarm read() by errno: EAGAIN is the
// expected empty flush, any other errno is a broken fd and is logged rather than looking
// identical to it. The EAGAIN leg is pinned below; the rest is in the SIL smokes.
