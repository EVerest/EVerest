...
TEST_CASE("ScenarioDispatcher rearm precision and loop disarm", "[evsim][scenario][rearm]") {
    ...
    SECTION("Sub-second positive delay arms with millisecond precision") {
        ...
        ctx->scenario.append_steps(std::move(steps), false, *ctx);
    ...
    SECTION("Loop rewind arms strictly positive delay (does not disarm)") {
        ...
        ctx->scenario.append_steps(make_three_step_list(), false, *ctx);
...
TEST_CASE("ScenarioDispatcher forward-progress guard", "[evsim][scenario][loop-guard]") {
    ...
    SECTION("Single zero-offset step looped on itself disarms the loop") {
        auto ctx = fx.make_ctx();
        ...
        // The guard bounds the inline iterations and then disarms the loop. Arming 1 ms and
        // returning would leave the dispatcher re-arming at 1 kHz forever, which is the guard
        // failing open: the run is already wrong and the only way to stop it is to stop looping.
        CHECK(fx.timer.scenario_timer_arms.empty());
        CHECK_FALSE(ctx->scenario.active());
        // Many copies of the step were enqueued before the guard tripped, bounded by
        // steps_.size() + 128 + 1 ~= 130 here. Includes the initial on_timer_fire enqueue and
        // the inline-flushed copies.
        ...
        // And it stays stopped: a second fire has nothing left to dispatch.
        const auto enqueued_after_guard = fx.timer.enqueued_events.size();
        ctx->scenario.on_timer_fire(*ctx);
        CHECK(fx.timer.enqueued_events.size() == enqueued_after_guard);
        CHECK(fx.timer.scenario_timer_arms.empty());
    }
...
TEST_CASE("ScenarioDispatcher splices a curve at its due time", "[evsim][scenario][curve][timing]") {
    ...
    SECTION("The curve lands ahead of a later pending step") {
        ...
        fx.timer.scenario_timer_arms.clear();
        const auto placed = ctx->scenario.append_steps(make_curve(), false, *ctx);
    ...
    SECTION("Curve offsets are measured from the append, not from scenario start") {
        ...
        ctx->scenario.append_steps(std::move(curve), false, *ctx);
