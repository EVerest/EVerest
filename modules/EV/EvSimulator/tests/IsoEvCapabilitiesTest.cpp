...
//
// cp_state_changed, update_present_values and abort_charging: wiring and defaults, not
// arithmetic. The HLC stack has no board support connection of its own, so every CP state
// the FSM applies has to reach it. A present value that is never reported goes onto the
// wire as zero and reads at the SECC as a measured zero, so the property that matters is
// that an override leaves the other field alone, not merely that it sets the one asked for.
