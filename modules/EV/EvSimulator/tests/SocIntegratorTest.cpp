...
//
// Parity tests for SocIntegrator vs CarSimulation::simulate_soc, whose private peer
// dependencies keep it out of this translation unit, so the golden values are computed
// by hand from the same formula.
//
// Two deliberate divergences, both for BPT / V2X discharge: it always integrates, dropping
// EvManager's "skip accumulation when already over capacity" short-circuit that silently
// dropped discharge energy when starting above capacity, and it hard-clamps
// battery_charge_wh to [0, capacity] so underflow on discharge is handled like overshoot.
...
TEST_CASE("SocIntegrator parity with EvManager simulate_soc", "[evsim][soc]") {
    ...
    SECTION("Battery already over capacity stays clamped, no accumulation") {
        ...
        auto ctx = fx.make_ctx();
        set_mode(*ctx, api::ChargeMode::DcIso2);
        // Start above capacity, which is where EvManager short-circuits.
        ctx->vars.battery_charge_wh = ctx->vars.battery_capacity_wh + 50.0f;
...
// Observable outputs of a policy run: the two SimVars the integrator owns, plus
// which FSM events the policy fired.
...
// Runs `ticks` identical DcIso2 charge ticks from the same starting charge under
// `policy`. Everything except cfg.on_battery_full is held fixed, so any
// difference between two returned outcomes is attributable to the policy alone.
//
// 125 A live present current * 400 V present voltage = 50 kW; a 1000 ms tick
// therefore adds 50000 / 3600 = 13.8889 Wh per tick against a 60000 Wh pack.
PolicyOutcome run_dc_charge(const std::string& policy, double threshold_pct, float start_charge_wh, int ticks) {
    TestFixture fx;
    fx.cfg.dc_target_current = 125;
    fx.cfg.dc_target_voltage = 400;
    fx.cfg.tick_interval_ms = 1000;
    fx.cfg.on_battery_full = policy;
    fx.cfg.battery_full_threshold_pct = threshold_pct;
    auto ctx = fx.make_ctx();
    set_mode(*ctx, api::ChargeMode::DcIso2);
    ctx->vars.evse_dc_present_current_a = 125.0f;
    ctx->vars.battery_charge_wh = start_charge_wh;
    ctx->vars.soc_pct = start_charge_wh / ctx->vars.battery_capacity_wh * 100.0f;
    ctx->vars.was_full = false;
    ...
    return PolicyOutcome{ctx->vars.battery_charge_wh, ctx->vars.soc_pct,
                         event_kind_enqueued(fx.timer.enqueued_events, module::EventKind::StopSession),
                         event_kind_enqueued(fx.timer.enqueued_events, module::EventKind::PauseSession)};
}

} // namespace

// `clamp` and `idle_at_full` differ by exactly one statement in soc_step: the
// post-integration trim back to battery_full_threshold_pct, guarded by
// `battery_charge_wh > threshold_wh`. At the default threshold of 100 that
// guard can never hold, because the preceding hard clamp already caps
// battery_charge_wh at battery_capacity_wh and threshold_wh == capacity_wh. So
// the two policies are indistinguishable at the default and only diverge at a
// threshold strictly below 100. These cases pin the divergence at 80 % and pin
// the no-op at 100 % so a future reader cannot mistake the latter for coverage.
TEST_CASE("SocIntegrator clamp vs idle_at_full diverge below 100 % threshold", "[evsim][soc]") {
    using Catch::Matchers::WithinAbs;

    // 79.983 % of 60000 Wh: one 13.8889 Wh tick crosses the 48000 Wh mark.
    constexpr float kStart = 47990.0f;
    constexpr float kThresholdWh = 48000.0f; // 80 % of 60000

    SECTION("single crossing tick at 80 %: clamp overshoots, idle_at_full trims") {
        const auto clamp = run_dc_charge("clamp", 80.0, kStart, 1);
        const auto idle = run_dc_charge("idle_at_full", 80.0, kStart, 1);

        // The distinction itself: same scenario, different energy.
        CHECK(clamp.charge_wh > idle.charge_wh);
        CHECK(clamp.soc_pct > idle.soc_pct);

        // And the concrete values, so a regression that merely perturbs one
        // side cannot satisfy the inequality above by accident.
        CHECK_THAT(clamp.charge_wh, WithinAbs(kStart + 50000.0f / 3600.0f, 1e-2f)); // 48003.89
        CHECK_THAT(idle.charge_wh, WithinAbs(kThresholdWh, 1e-2f));                 // 48000.00
        CHECK(clamp.charge_wh > kThresholdWh);
    }

    SECTION("long run at 80 %: clamp fills the pack, idle_at_full holds the threshold") {
        // 900 ticks is past the 865 needed to carry clamp from 47990 Wh to the
        // 60000 Wh capacity, so the two policies end 20 SoC points apart.
        const auto clamp = run_dc_charge("clamp", 80.0, kStart, 900);
        const auto idle = run_dc_charge("idle_at_full", 80.0, kStart, 900);

        CHECK_THAT(clamp.soc_pct, WithinAbs(100.0f, 1e-2f));
        CHECK_THAT(idle.soc_pct, WithinAbs(80.0f, 1e-2f));
        CHECK_THAT(idle.charge_wh, WithinAbs(kThresholdWh, 1e-2f));

        // Neither policy fires an FSM event; the divergence is purely energetic.
        CHECK_FALSE(clamp.stop_session);
        CHECK_FALSE(clamp.pause_session);
        CHECK_FALSE(idle.stop_session);
        CHECK_FALSE(idle.pause_session);
    }

    SECTION("at the default 100 % threshold the two policies are indistinguishable") {
        // Documents why the sections above must use 80 %: at 100 % the trim is
        // dead code, so a suite written only at the default cannot tell clamp
        // from idle_at_full no matter how many ticks it runs.
        const auto clamp = run_dc_charge("clamp", 100.0, 59990.0f, 10);
        const auto idle = run_dc_charge("idle_at_full", 100.0, 59990.0f, 10);

        CHECK_THAT(clamp.charge_wh, WithinAbs(idle.charge_wh, 1e-3f));
        CHECK_THAT(clamp.soc_pct, WithinAbs(idle.soc_pct, 1e-3f));
        CHECK_THAT(clamp.charge_wh, WithinAbs(60000.0f, 1e-2f));
    }
}

// All four policies over one shared scenario. Each row must be distinguishable
// from the other three on at least one observable, otherwise the policy is not
// really a policy.
TEST_CASE("SocIntegrator all four on_battery_full policies are mutually distinguishable", "[evsim][soc]") {
    using Catch::Matchers::WithinAbs;
    constexpr float kStart = 47990.0f;
    constexpr float kThresholdWh = 48000.0f;

    const auto clamp = run_dc_charge("clamp", 80.0, kStart, 1);
    const auto idle = run_dc_charge("idle_at_full", 80.0, kStart, 1);
    const auto stop = run_dc_charge("stop_session", 80.0, kStart, 1);
    const auto pause = run_dc_charge("pause_if_iso", 80.0, kStart, 1);

    SECTION("clamp: no trim, no event") {
        CHECK(clamp.charge_wh > kThresholdWh);
        CHECK_FALSE(clamp.stop_session);
        CHECK_FALSE(clamp.pause_session);
    }

    SECTION("idle_at_full: trims to threshold, no event") {
        CHECK_THAT(idle.charge_wh, WithinAbs(kThresholdWh, 1e-2f));
        CHECK_FALSE(idle.stop_session);
        CHECK_FALSE(idle.pause_session);
    }

    SECTION("stop_session: trims to threshold and fires StopSession only") {
        CHECK_THAT(stop.charge_wh, WithinAbs(kThresholdWh, 1e-2f));
        CHECK(stop.stop_session);
        CHECK_FALSE(stop.pause_session);
    }

    SECTION("pause_if_iso in an ISO mode: trims to threshold and fires PauseSession only") {
        CHECK_THAT(pause.charge_wh, WithinAbs(kThresholdWh, 1e-2f));
        CHECK(pause.pause_session);
        CHECK_FALSE(pause.stop_session);
    }

    SECTION("each policy differs from every other on at least one observable") {
        // clamp is the only one that keeps energy above the threshold.
        CHECK(clamp.charge_wh > idle.charge_wh);
        CHECK(clamp.charge_wh > stop.charge_wh);
        CHECK(clamp.charge_wh > pause.charge_wh);
        // The three trimming policies are separated by their events.
        CHECK(idle.stop_session != stop.stop_session);
        CHECK(idle.pause_session != pause.pause_session);
        CHECK(stop.stop_session != pause.stop_session);
        CHECK(stop.pause_session != pause.pause_session);
    }
}
