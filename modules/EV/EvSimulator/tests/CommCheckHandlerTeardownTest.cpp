...
// internal threads from the destructor. These tests cover the heartbeat thread. The
// comm-check thread needs start(), which no test here calls, so the destructor's
// handler.join() is exercised by the SIL smokes rather than from here.
