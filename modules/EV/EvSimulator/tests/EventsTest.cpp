...
TEST_CASE("kind_of maps every variant alternative to its EventKind", "[evsim][events]") {
    ...
    CHECK(kind_of(Event{V2gMessagePayload{}}) == EventKind::V2gMessage);
...
TEST_CASE("iso_session_event_kind names exactly the ISO lifecycle edges", "[evsim][events]") {
    // These five are what the harness DSL waits on. They are published from the dispatch loop,
    // not from the states, because a state that is running may ignore the event entirely: from
    // Charging, IsoV2GFinished produces no state edge at all.
    CHECK(iso_session_event_kind(EventKind::IsoPowerReady) == api::IsoSessionEventKind::PowerReady);
    CHECK(iso_session_event_kind(EventKind::IsoDcPowerOn) == api::IsoSessionEventKind::DcPowerOn);
    CHECK(iso_session_event_kind(EventKind::IsoStopFromCharger) == api::IsoSessionEventKind::StopFromCharger);
    CHECK(iso_session_event_kind(EventKind::IsoPauseFromCharger) == api::IsoSessionEventKind::PauseFromCharger);
    CHECK(iso_session_event_kind(EventKind::IsoV2GFinished) == api::IsoSessionEventKind::V2GFinished);

    // Everything else is not a lifecycle edge, including the ISO events that only carry values.
    CHECK_FALSE(iso_session_event_kind(EventKind::IsoAcMaxCurrent).has_value());
    CHECK_FALSE(iso_session_event_kind(EventKind::IsoAcTargetPower).has_value());
    CHECK_FALSE(iso_session_event_kind(EventKind::DcEvsePresentVoltage).has_value());
    CHECK_FALSE(iso_session_event_kind(EventKind::BspEvent).has_value());
    CHECK_FALSE(iso_session_event_kind(EventKind::Plug).has_value());
    CHECK_FALSE(iso_session_event_kind(EventKind::StateDeadline).has_value());
}
