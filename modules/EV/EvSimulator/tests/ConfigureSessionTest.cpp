...
TEST_CASE("configure_session interceptor latch/reject contract", "[evsim][configure_session]") {
    ...
    // The spin above is reachable from a config, so the config is where it is refused: a looping
    // curve rewinds to its loop start and takes the next delay from there, and with one point that
    // delay is always zero. The monotonicity loop could not catch it, because it starts at i = 1
    // and never runs for a single point.
    SECTION("A looping curve with one point is rejected before it can spin") {
        auto ctx = fx.make_ctx();
        api::AcIso2SessionParams p;
        p.curve = api::ChargingCurve{{{0, 10.0f, false, std::nullopt}}, true};
        ...
        CHECK(ack.status == api::CommandAckStatus::Rejected);
        REQUIRE(ack.reason.has_value());
        CHECK(ack.reason->find("looping curve needs at least two points") != std::string::npos);
    }

    SECTION("A looping curve with two points is accepted") {
        auto ctx = fx.make_ctx();
        api::AcIso2SessionParams p;
        p.curve = api::ChargingCurve{{{0, 10.0f, false, std::nullopt}, {100, 12.0f, false, std::nullopt}}, true};

        ctx->configure_session(api::SessionConfigParams{p});

        CHECK(ctx->configured_session.has_value());
    }
