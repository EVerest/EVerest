...
// A pause with a live HLC session does not drop CP in enter(). The EV holds C
// until the pause PowerDeliveryReq is on the wire, which Paused::feed(V2gMessage)
// observes. IsoV2GFinished is only the backstop for a peer that publishes no
// per-message signal, and it is that backstop, not the primary path, that this
// case exercises.
TEST_CASE("EvSimulator Paused holds CP at C until the live V2G session finishes", "[evsim][group2]") {
    ...
    // apply_passthrough_vars clears iso_session_active before the FSM feed;
    // emulated here.
    ctx->vars.iso_session_active = false;
    fx.mocks.bsp.clear();
