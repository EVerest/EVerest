...
inline PeerActions make_peer_actions(ActionMocks& mocks) {
    ...
    actions.iso.cp_state_changed = [&mocks](::types::iso15118::CpState st) { mocks.iso.call_cp_state_changed(st); };
...
// `set_mode` starts or retargets a session of that mode, preserving any
// payment/bpt/mcs/curve already staged.
