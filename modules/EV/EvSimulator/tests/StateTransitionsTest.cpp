...
TEST_CASE("EvSimulator group2 transitions", "[evsim][group2]") {
    ...
    // PowerDeliveryReq(Stop) is on the wire, not at the transport teardown.
    // ISO 15118-2 [V2G2-533] makes WeldingDetectionReq the next message, and
    // [V2G2-920]..[V2G2-922] oblige the SECC to measure CP state B on receiving
    // it and to answer FAILED otherwise. Holding C until IsoV2GFinished (~8.9 s
    // later, after four welding exchanges) is what broke the DC resume: the
    // charger latched EVSE_Shutdown after its teardown had already cleared it,
    // and the resumed session's PowerDelivery(Start) came back
    // FAILED_PowerDeliveryNotApplied.
    SECTION("Paused: PowerDeliveryReq on the wire drops CP to B") {
        auto ctx = fx.make_ctx();
        set_mode(*ctx, api::ChargeMode::DcIso2);
        ctx->vars.iso_session_active = true;
        Paused s{*ctx};
        s.enter();
        // enter() must NOT have dropped CP yet: the pause has only been asked
        // for, nothing is on the wire.
        CHECK_FALSE(contains_substr(fx.mocks.bsp.records, "set_cp_state(cp_state=B)"));

        auto result = s.feed(Event{V2gMessagePayload{"PowerDeliveryReq"}});
        CHECK_FALSE(result.new_state);
        CHECK(contains_substr(fx.mocks.bsp.records, "set_cp_state(cp_state=B)"));
    }

    SECTION("Paused: an unrelated V2G message does not drop CP") {
        auto ctx = fx.make_ctx();
        set_mode(*ctx, api::ChargeMode::DcIso2);
        ctx->vars.iso_session_active = true;
        Paused s{*ctx};
        s.enter();
        auto result = s.feed(Event{V2gMessagePayload{"CurrentDemandReq"}});
        CHECK_FALSE(result.new_state);
        CHECK_FALSE(contains_substr(fx.mocks.bsp.records, "set_cp_state(cp_state=B)"));
    }
    ...
    SECTION("Faulted.enter with SlacTimeout makes no fault-specific BSP call but still drops power") {
        auto ctx = fx.make_ctx();
        ctx->vars.last_fault = api::FaultReport{api::FaultType::SlacTimeout, std::nullopt, std::nullopt};
        Faulted s{*ctx};
        s.enter();
        // A software fault has no BSP fault to assert, but the fault still ends the session:
        // power permission goes away whatever the type.
        CHECK_FALSE(contains_substr(fx.mocks.bsp.records, "diode_fail"));
        CHECK_FALSE(contains_substr(fx.mocks.bsp.records, "set_rcd_error"));
        CHECK_FALSE(contains_substr(fx.mocks.bsp.records, "set_cp_state"));
        CHECK(contains_substr(fx.mocks.bsp.records, "allow_power_on(value=false)"));
    ...
    SECTION("Faulted.enter with V2GTimeout makes no fault-specific BSP call but still drops power") {
        auto ctx = fx.make_ctx();
    ...
    SECTION("Faulted.enter with Internal makes no fault-specific BSP call but still drops power") {
        auto ctx = fx.make_ctx();
    ...
    // m2e/query_state is operator-facing: it backs logging and the current-state
    // display in a UI. A faulted simulator is precisely when that answer
    // matters, so Faulted must answer the query like the other ten states do
    // rather than swallowing it in the inert group.
    SECTION("Faulted: QueryState publishes Faulted, no transition") {
        auto ctx = fx.make_ctx();
        ctx->vars.last_fault = api::FaultReport{api::FaultType::DiodeFail, std::nullopt, std::nullopt};
        Faulted s{*ctx};
        auto result = s.feed(Event{EventKind::QueryState});
    ...
    SECTION("Faulted: QueryState answers again after enter already published") {
        // Clearing the sink after enter() proves the answer is a fresh publish
        // driven by the query, not the one Faulted.enter emitted.
        auto ctx = fx.make_ctx();
        ctx->vars.last_fault = api::FaultReport{api::FaultType::CpErrorE, std::nullopt, std::nullopt};
        Faulted s{*ctx};
        s.enter();
        REQUIRE(topic_recorded(fx.sink, state_topic));
        fx.sink.clear();
        REQUIRE_FALSE(topic_recorded(fx.sink, state_topic));
...
// m2e/query_state is the pull-model state feed an operator log or a UI reads.
// Its value depends on every state answering: one silent state shows a stale
// value with no error, which is how Faulted's inert arm went unnoticed for as
// long as it did. The per-state sections above cover eight of eleven states and
// are easy to forget; this case states the property once over all eleven, so
// the gap is visible as a missing SECTION rather than as absent coverage.
TEST_CASE("Every FSM state answers QueryState with its own id", "[evsim][query_state]") {
    TestFixture fx;
    const auto state_topic = fx.topics.everest_to_extern("state");

    auto answers_with_own_id = [&](auto make_state) {
        auto ctx = fx.make_ctx();
        // enter() is deliberately not run: the query must be answered by feed()
        // alone, which is what a late subscriber actually reaches. last_fault is
        // seeded only to keep the setup uniform; no QueryState arm reads it.
        ctx->vars.last_fault = api::FaultReport{api::FaultType::Internal, std::nullopt, std::nullopt};
        auto state = make_state(*ctx);
        const auto expected = state->get_id();
        fx.sink.clear();

        auto result = state->feed(Event{EventKind::QueryState});

        // No state may consume the query silently, and none may transition on it.
        CHECK(result.unhandled == false);
        CHECK(result.new_state == nullptr);
        REQUIRE(topic_recorded(fx.sink, state_topic));
        CHECK(api::deserialize<api::FsmState>(payload_for(fx.sink, state_topic)) == expected);
    };

    SECTION("Disabled") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<Disabled>(c); });
    }
    SECTION("Unplugged") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<Unplugged>(c); });
    }
    SECTION("Plugged") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<Plugged>(c); });
    }
    SECTION("SlacMatching") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<SlacMatching>(c); });
    }
    SECTION("V2GNegotiating") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<V2GNegotiating>(c); });
    }
    SECTION("BcbToggling") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<BcbToggling>(c); });
    }
    SECTION("Charging") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<Charging>(c); });
    }
    SECTION("ChargingPwmPaused") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<ChargingPwmPaused>(c); });
    }
    SECTION("Paused") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<Paused>(c); });
    }
    SECTION("Stopping") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<Stopping>(c); });
    }
    SECTION("Faulted") {
        answers_with_own_id([](FsmContext& c) { return std::make_unique<Faulted>(c); });
    }
}
