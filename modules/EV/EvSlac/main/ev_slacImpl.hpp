...
private:
    ...
    struct LifecycleState : everest::lib::util::LifecycleStateT<FSMController> {
        ...
        /// The controller a command or frame may be handed to right now: live and with the PLC
        /// I/O up. Null means drop.
        FSMController* dispatch_target() const {
            return slac_io_ready ? live_worker() : nullptr;
        }
    };
