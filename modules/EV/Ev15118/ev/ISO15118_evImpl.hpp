...
private:
    ...
    // Last control pilot state reported via cp_state_changed, as the C/D flag the stack wants.
    // Guarded by config_mutex; empty until the manager reports one. Latched because the report can
    // arrive before ready() has built the controller, and because it enables the CP-dependent
    // checks (the DC cable check waits for C/D only when the pilot is actually being reported).
    std::optional<bool> reported_cp_state;
