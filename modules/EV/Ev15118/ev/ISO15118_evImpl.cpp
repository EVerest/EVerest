...
// V2gMessageType (a libiso15118 variant) -> types::iso15118::V2gMessageId. The
// mapping is protocol data and identical for both roles, so the charger-side
// header is reused here instead of being duplicated.
#include "../../../EVSE/Evse15118D20/charger/utils.hpp"
...
iso15118::config::SSLConfig make_ssl_config(const module::Conf& config, const std::filesystem::path& etc_path) {
    ...
    // SSL_CTX_use_certificate_chain_file expects. The EV client uses a single chain (the
    // multi-chain shape of SSLConfig exists for the SECC's TLS 1.3 chain selection).
    auto& chain = ssl.chains.emplace_back();
    chain.path_certificate_chain = config.device_cert_chain_path.empty()
                                       ? (certs_path / "client/vehicle/VEHICLE_CERT_CHAIN.pem").string()
                                       : config.device_cert_chain_path;
    chain.path_certificate_key = config.device_key_path.empty()
                                     ? (certs_path / "client/vehicle/VEHICLE_LEAF.key").string()
                                     : config.device_key_path;
    ssl.path_certificate_v2g_root = config.v2g_root_cert_path.empty() ? (certs_path / "ca/v2g/V2G_ROOT_CA.pem").string()
    ...
    if (password_file.is_open()) {
        ...
        if (not password.empty()) {
            chain.private_key_password = password;
        }
    }
...
// Read a whole file into a string, or empty if it cannot be opened.
...
// Read a file and trim surrounding whitespace (for password files).
...
// Build the ISO 15118-2 Plug-and-Charge session config from the module config + committed PKI defaults.
// The OEM provisioning + contract certificate material is sourced from config-path files (like the TLS
// device cert in make_ssl_config); the trust root is the V2G root already used for TLS.
iso15118::d2::ev::PnCConfig build_pnc_config(const module::Conf& config, const std::filesystem::path& etc_path) {
    namespace crypto = iso15118::d2::crypto;
    const auto certs_path = etc_path / "certs";
    ...
    iso15118::d2::ev::PnCConfig pnc;
    pnc.prefer_contract = true;
    pnc.force_cert_install = config.pnc_force_cert_install;
    pnc.v2g_root_path = resolve(config.v2g_root_cert_path, certs_path / "ca/v2g/V2G_ROOT_CA.pem").string();

    // Contract (MO) certificate chain + key + eMAID (the pre-installed contract presented in PaymentDetails).
    const auto chain_pem =
        read_file_text(resolve(config.pnc_contract_cert_chain_path, certs_path / "client/mo/MO_CERT_CHAIN.pem"));
    const auto chain_der = crypto::pem_chain_to_der(chain_pem);
    if (not chain_der.empty()) {
        ...
        pnc.contract_key_pem =
            read_file_text(resolve(config.pnc_contract_key_path, certs_path / "client/mo/MO_LEAF.key"));
        auto contract_pw = read_file_trimmed(
            resolve(config.pnc_contract_key_password_path, certs_path / "client/mo/MO_LEAF_PASSWORD.txt"));
        if (not contract_pw.empty()) {
            pnc.contract_key_password = std::move(contract_pw);
        }
    } else {
    ...
    if (not oem_der.empty()) {
        ...
        pnc.oem_prov_key_pem =
            read_file_text(resolve(config.pnc_oem_prov_key_path, certs_path / "client/oem/OEM_LEAF.key"));
        auto oem_pw = read_file_trimmed(
            resolve(config.pnc_oem_prov_key_password_path, certs_path / "client/oem/OEM_LEAF_PASSWORD.txt"));
        if (not oem_pw.empty()) {
            pnc.oem_prov_key_password = std::move(oem_pw);
        }
    }

    // Declare the trusted V2G root in the CertificateInstallationReq ListOfRootCertificateIDs.
...
iso15118::message_2::datatypes::EnergyTransferMode to_iso2_transfer_mode(types::iso15118::EnergyTransferMode mode) {
...
std::optional<iso15118::d20::ev::DcEvBptChargeParameters> ISO15118_evImpl::build_bpt_dc_charge_parameters() const {
...
ISO15118_evImpl::select_service_for(types::iso15118::EnergyTransferMode mode) const {
    ...
    const bool has_pre20 = std::find(supported_protocols.begin(), supported_protocols.end(),
                                     iso15118::ProtocolId::ISO15118_2) != supported_protocols.end() or
                           std::find(supported_protocols.begin(), supported_protocols.end(),
                                     iso15118::ProtocolId::DIN70121) != supported_protocols.end();
    if (has_pre20) {
...
iso15118::session::ev::feedback::Callbacks ISO15118_evImpl::create_callbacks() {
    ...
    // Publish every V2G message the EV stack sends/receives, mirroring what
    // Evse15118D20 publishes on the charger side. EvManager subscribes to it
    // for its (opt-in) session log; debug information only.
    callbacks.v2g_message = [this](const iso15118::V2gMessageType& id) {
        types::iso15118::V2gMessages v2g_message;
        v2g_message.id = module::charger::convert_v2g_message_type(id);
        publish_v2g_messages(v2g_message);
    };

    callbacks.pnc_contract_installed = [this](const std::string& contract_chain_pem, const std::string& /*key_pem*/,
...
void ISO15118_evImpl::ready() {
    ...
    iso15118::session::EvSetupConfig setup_config;
    {
        ...
        // Scheduled by default: in Dynamic mode the SECC drives the EV maximum voltage as the supply
        // setpoint ceiling, which the SIL voltage-source power supply outputs verbatim (tripping the
        // over-voltage monitor); real dynamic-mode operation can be selected via d20_control_mode.
        setup_config.preferred_control_mode =
            (mod->config.d20_control_mode == "Dynamic") ? dt::ControlMode::Dynamic : dt::ControlMode::Scheduled;
        // Plug-and-Charge (Contract) is offered in addition to EIM when enabled and TLS is active (PnC
        // requires TLS [V2G2-632]); the ISO 15118-2 state machine falls back to EIM whenever the SECC
        // does not offer Contract.
        if (mod->config.enable_pnc and ev_config.use_tls) {
            setup_config.supported_auth_options = {dt::Authorization::PnC, dt::Authorization::EIM};
            setup_config.iso2_pnc = build_pnc_config(mod->config, mod->info.paths.etc);
        } else {
            if (mod->config.enable_pnc) {
                EVLOG_warning << "Ev15118: enable_pnc is set but TLS is not active; Plug-and-Charge requires "
                                 "TLS, falling back to EIM only";
            }
        ...
        // A pilot state reported before the controller existed still has to reach it.
        if (reported_cp_state.has_value()) {
            controller->set_cp_state(reported_cp_state.value());
        }
    }
...
bool ISO15118_evImpl::handle_start_charging(types::iso15118::EnergyTransferMode& EnergyTransferMode,
    ...
    // Payment option: Plug-and-Charge (Contract) is honored when the module has PnC enabled and configured
    // (the actual EIM/Contract choice is made per session by the ISO 15118-2 state machine based on what
    // the SECC offers). A Contract request without PnC configured falls back to EIM — unless the option
    // is enforced, in which case the EV selects Contract even when the SECC does not offer it and lets
    // the SECC reject the session (negative testing).
    ...
    // Applies to the session armed below; also clears a leftover enforcement from a previous session.
    controller->set_enforce_iso2_contract(enforce_contract);
...
void ISO15118_evImpl::handle_cp_state_changed(types::iso15118::CpState& cp_state) {
    ...
    std::scoped_lock lock(config_mutex);
    // Latch it: the manager may report the pilot before ready() has built the controller, and the
    // state still has to reach the first session. Reporting it at all is also what enables the
    // CP-dependent checks -- a deployment whose manager never calls this simply does not get them.
    reported_cp_state = c_or_d;
    if (controller) {
        controller->set_cp_state(c_or_d);
    }
}

void ISO15118_evImpl::handle_abort_charging() {
    EVLOG_info << "handle_abort_charging";
    std::scoped_lock lock(config_mutex);
    if (controller) {
