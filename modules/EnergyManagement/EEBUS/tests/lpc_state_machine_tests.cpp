...
TEST_F(LpcStateMachineTest, unlimited_autonomous_exits_to_limited_on_heartbeat_plus_active_limit) {
    auto h = make_handler();
    h->start();
    advance(std::chrono::seconds(121)); // Init timeout per LPC-906
    h->run_state_machine();
    ASSERT_EQ(h->get_state(), LpcUseCaseHandler::State::UnlimitedAutonomous);
...
TEST_F(LpcStateMachineTest, unlimited_autonomous_exits_to_unlimited_controlled_on_deactivated_write) {
    auto h = make_handler();
    h->start();
    advance(std::chrono::seconds(121));
    h->run_state_machine();
...
TEST_F(LpcStateMachineTest, unlimited_autonomous_does_not_re_apply_failsafe_period_limit) {
    auto h = make_handler();
    enter_failsafe(*h); // current_limit cleared on entry, state == Failsafe

    // EG sends a limit while we're still in Failsafe (no heartbeat yet).
    advance(std::chrono::seconds(5));
    send_active_limit(*h, ACTIVE_LIMIT);
    ASSERT_EQ(h->get_state(), LpcUseCaseHandler::State::Failsafe)
        << "limit alone without heartbeat must not exit Failsafe";

    // LPC-922: advance past failsafe duration minimum so we exit to UnlimitedAutonomous.
    advance(std::chrono::hours(2) + std::chrono::seconds(1));
    h->run_state_machine();
    ASSERT_EQ(h->get_state(), LpcUseCaseHandler::State::UnlimitedAutonomous);
    ASSERT_TRUE(last_limit_is_unlimited());

    // Heartbeat now arrives. With the stale limit dropped on UnlimitedAutonomous entry,
    // there is no is_active=true to drag the FSM back into Limited.
    const auto callbacks_before = limit_callback_count();
    send_heartbeat(*h);
    EXPECT_EQ(h->get_state(), LpcUseCaseHandler::State::UnlimitedAutonomous)
        << "no stale limit may drive a transition from UnlimitedAutonomous";
    EXPECT_EQ(limit_callback_count(), callbacks_before)
        << "no spurious limit callback on heartbeat in UnlimitedAutonomous";
}
...
TEST_F(LpcStateMachineTest, timed_active_limit_expiry_settles_in_unlimited_controlled) {
    auto h = make_handler();
    h->start();
    send_heartbeat(*h);

    common_types::LoadLimit timed;
    timed.set_is_active(true);
    timed.set_value(ACTIVE_LIMIT);
    timed.set_duration_nanoseconds(
        static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::seconds(30)).count()));
    timed.set_delete_duration(false);
    h->process_received_limit(timed);
    ASSERT_EQ(h->get_state(), LpcUseCaseHandler::State::Limited);

    advance(std::chrono::seconds(31));
    send_heartbeat(*h);
    ASSERT_EQ(h->get_state(), LpcUseCaseHandler::State::UnlimitedControlled);

    const auto callbacks_after_expiry = limit_callback_count();
    h->run_state_machine();
    h->run_state_machine();
    h->run_state_machine();

    EXPECT_EQ(h->get_state(), LpcUseCaseHandler::State::UnlimitedControlled)
        << "FSM must not oscillate after timed limit expiry";
    EXPECT_EQ(limit_callback_count(), callbacks_after_expiry)
        << "no additional limit callbacks expected on idle ticks after expiry";
}
...
using module::UseCaseEventReader;
