...
///
/// Module-independent (no OCPPmulti / charge_point / framework dependency) so it is unit-testable in
...
private:
    // Unguarded by design: every access goes through the owner's monitor<GridSupportState> handle (class doc).
