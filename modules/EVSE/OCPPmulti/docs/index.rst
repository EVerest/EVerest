...
the OCPP stack or the ``ocpp`` interface do. If a component/variable is defined in both storages, a warning is
logged at startup and the registration order determines the effective source; fix the component configs if this
warning appears.
