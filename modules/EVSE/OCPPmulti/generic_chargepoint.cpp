...
GenericChargePoint::on_get_15118_ev_certificate_request(std::int32_t extensions_id,
    ...
    return m_active_ptr->on_get_15118_ev_certificate_request(extensions_id, request);
}
...
void GenericChargePoint::register_variable_listener(const std::string& key, listener_t listener) {
    ...
    m_active_ptr->register_variable_listener(key, std::move(listener));
}
