...
TEST(LifecycleGate, GenericLoopHandshakeIsInherited) {
    ...
    EXPECT_EQ(state.dispatch_target(), nullptr) << "a live controller is not a target while the PLC I/O is down";
    state.slac_io_ready = true;
    EXPECT_EQ(state.dispatch_target(), &controller);
    ...
    EXPECT_EQ(state.dispatch_target(), nullptr);
    EXPECT_FALSE(state.fsm_start_allowed());
