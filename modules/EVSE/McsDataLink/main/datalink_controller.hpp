...
/// The module's runtime, wired onto one fd_event_handler.
///
/// Threading: the state machine, trackers and timers are touched only by the event loop thread
/// (slacImpl::ready()). Interface commands arrive on framework threads, are appended to \ref m_commands
/// and signalled through an event_fd. \ref run_effects is the only place the machine's effects become I/O.
...
public:
    ...
    /// Open the rtnetlink socket. On failure \ref error is set; commands still run, the link never comes up.
    bool open();
    int error() const;
    ...
    /// Framework threads. Enqueue and wake the loop.
    /// @{
...
private:
    ...
    /// One FIFO for all commands: per-command event_fds would coalesce and reorder them.
    ...
    /// TT_sync_repetition still open (V2G10-056). Passed to the machine when TT_EV_link_detect expires.
