...
- ``required_der_operating_mode`` and ``grid_connection_mode`` are dictated in the
  charge parameter discovery only. The charge loop channel for resending them
  mid-session ([V2G20-3358] to [V2G20-3361], SAE only) is not implemented, so a
  change of either takes effect at the next session rather than reaching the
  running one. It lands with the SAE ``der_relay`` above, since nothing changes
  either value mid-session until that relay exists.
