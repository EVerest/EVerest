...
///
/// Charge and discharge pass through from ac_limits; the discharge sign is applied upstream, and an absent
/// discharge limit yields zeros. Nominal and max discharge are set equal, which is defensive here: the
/// nominal-vs-maximum requirements [V2G20-3228] and [V2G20-3229] are Annex M (SAE) only, and Annex L states
/// none.
...
///
/// Nominal charge comes from the ac_limits charge maximum, and both nominal and max discharge from the
/// ac_limits discharge maximum, so the discharge pair satisfies [V2G20-3229] (magnitude compare) and the
/// charge pair [V2G20-3228] (plain <=). An absent discharge limit yields zeros. The discharge sign follows
/// ac_limits, so it is negative only under the module's negative_bidirectional_limits option, while the AMD1
/// replacement of Table 94 (binding via [V2G20-1204]) and [V2G20-1035] require discharge powers <= 0.
///
/// Reactive-power limits come from the EVSE's reported capability, as non-negative absorption and
/// non-positive injection; the grid limits from its nominal voltage and frequency. Pure and session-free.
...
/// positive nominal frequency and voltage: the voltage is the base for the PercentageV grid code curves, and
/// the frequency is the mandatory GridNominalFrequency ([V2G20-3269], Table M.41) the EV adopts in place of
/// EVSENominalFrequency ([V2G20-3233]). Either one missing yields GridParametersMissing and no SAE limits,
/// never a placeholder grid. The frequency-trip curves are absolute Hz and are not scaled by the nominal
/// frequency, so a 60 Hz site needs 60 Hz curve values supplied with the setup config. Pure and
/// session-free; the caller decides what to log and what to assign.
...
/// the service ([V2G20-3191]), so there is nothing EV-authored to map and the field is left unset.
...
///
/// ev_supported_dercontrol comes from the request's SupportedModes bitmap ([V2G20-3409]); each SAE function
/// bit maps onto the matching grid_support DirectiveType, and bits without a counterpart are dropped and
/// logged. Reserved bits outside the SAE bitmap are warned about and ignored. The four excitation fields and
/// the optional session total discharge energy available pass through one-to-one. The reactive-power limits
/// are not mapped: SAE var absorption/injection semantics differ from the IEC charge/discharge reactive
/// fields.
types::iso15118::DERChargingParameters
