...
void energyImpl::ready() {
    ...
    mod->charger->signal_charging_paused_evse_event.connect(
        [this](const types::evse_manager::ChargingPausedEVSEReasons& paused) {
            ...
            using everest::lib::util::exists;
            paused_by_user_or_error = exists(paused.reasons, PauseChargingEVSEReasonEnum::UserPause) or
                                      exists(paused.reasons, PauseChargingEVSEReasonEnum::Error);
        });
}
