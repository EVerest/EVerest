...
void EvseManager::ready() {
    ...
    if (hlc_enabled) {
        ...
        if (config.charge_mode == "AC") {
            // Recompute from the live der_available flag: a set_der_available landing before this
            // point must not be clobbered by a list built when der_available was still false.
            this->recompute_and_publish_supported_ac_energy_transfers();
        } else {
...
void EvseManager::setup_AC_mode() {
    ...
    if (hlc_enabled) {
        r_hlc[0]->call_setup(evseid, sae_mode, config.session_logging);
        // Derive the list rather than hand-building it: a locally built AC pair drops AC_BPT and
        // whichever AC DER flavor is advertised, and nothing republishes them until the next
        // capabilities update.
        const auto caps = *hw_capabilities.handle();
        this->update_supported_energy_transfers(
            get_supported_ac_energy_transfers(caps, config.supported_iso_ac_bpt, der_available.load()));
        this->publish_and_update_supported_energy_transfers();
    }
}
