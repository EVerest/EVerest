...
// to_der_ctrlr_set_variables may write on the DC path must be defined here, else the write targets an
...
// on the DC component (matching to_der_ctrlr_set_variables).
...
std::optional<std::pair<ocpp::v2::ComponentKey, std::vector<DeviceModelVariable>>> build_der_ctrlr_component_config(
    ...
    const bool ac_der_capable = supports_mode(types::iso15118::EnergyTransferMode::AC_DER) or
...
to_der_ctrlr_set_variables(const int32_t evse_id, const types::grid_support::DERCapability& capability) {
    ...
    result.push_back(
        make_set_variable_data(get_component_variable(evse_id, ocpp::v2::DERComponentVariables::Available), "true"));
    ...
    if (dc.manufacturer) {
        emit_string("InverterManufacturer", dc.manufacturer.value());
    }
    if (dc.model) {
        emit_string("InverterModel", dc.model.value());
    }
    if (dc.sw_version) {
        emit_string("InverterSwVersion", dc.sw_version.value());
    }
    if (dc.hw_version) {
        emit_string("InverterHwVersion", dc.hw_version.value());
    }
