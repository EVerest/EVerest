...
class ConfigurationApiClient:
    ...
    def subscribe_notices(self) -> None:
        ...
        self._client.subscribe([(ACTIVE_SLOT_TOPIC, 2), (CONFIG_UPDATES_TOPIC, 2)])

    def _on_active_slot(self, _client, _userdata, msg) -> None:
        notice = json.loads(msg.payload)
    ...
    def mark_active_slot_notices(self) -> int:
        return len(self.active_slot_notices)

    def mark_config_update_notices(self) -> int:
    ...
    def _wait_for(self, notices: Callable[[], List[dict]], predicate: Callable[[dict], bool],
        ...
        raise TimeoutError(
            ...
            f"received (from index {after_index}): {notices()[after_index:]}"
        )
