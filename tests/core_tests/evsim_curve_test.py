...
`test_charging_curve_taper_end_to_end` loads `config/config-sil-dc.yaml`
(the DC ISO-2 SIL config); `test_runtime_ramp_end_to_end` loads
`config/config-sil.yaml` (the AC HLC SIL config), since it drives an
...
@pytest.mark.everest_config_adaptions(EvSimulatorConfigAdjustment(overrides=EVSIM_DC_SESSION,
                                                       charger_overrides={"supported_DIN70121": False}))
