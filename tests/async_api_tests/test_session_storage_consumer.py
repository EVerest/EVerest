...
is the default page request or the full clear, get_session replies with the bare session
record or with JSON null when nothing matches, and clear_sessions replies with the
deleted count.
"""
...
async def test_clear_sessions_replies_with_the_deleted_count(
    ...
    # no payload means every record
    assert backend.requests["clear_sessions"] == {"request": {}}
    assert reply == {"cleared": 7}


@pytest.mark.asyncio
@pytest.mark.everest_core_config("probe-session-storage-consumer.yaml")
async def test_clear_sessions_forwards_the_upper_bound(
    everest_core: EverestCore,
    async_api_mqtt_handler: AsyncApiMqttHandler,
    backend: SessionStorageBackend,
    probe_module: ProbeModule,
    ...
    backend.cleared = 2
    external_request = {"up_to_session_id": SESSION["session_id"]}

    reply = await _request_until_reply(
        async_api_mqtt_handler, everest_core.mqtt_external_prefix,
        "clear_sessions", external_request)

    assert backend.requests["clear_sessions"] == {"request": external_request}
    assert reply == {"cleared": 2}
