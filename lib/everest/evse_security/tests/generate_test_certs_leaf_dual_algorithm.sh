#!/bin/bash
# Same layout as generate_test_certs_leaf_multi.sh, but the second SECC leaf is on secp521r1 so a
# prime256v1 (ISO 15118-2) and a secp521r1 (ISO 15118-20) leaf share one V2G root.
...
function create_certificate() {
  ...
  # $7: optional EC curve, defaults to $EC_CURVE
  local curve="${7:-$EC_CURVE}"
  openssl ecparam -genkey -name "$curve" | openssl ec "$SYMMETRIC_CIPHER" -passout pass:"$password" -out "${install_dir}/${name}.key"
...
# ISO 15118-20 chargepoint leaf: secp521r1 as mandated for the TLS 1.3 SECC certificate, issued
# by the same sub-CA as the prime256v1 ISO 15118-2 leaf above
create_certificate SECC_LEAF_20 "${CLIENT_CSO_PATH}" seccLeafCert_Alternate.cnf 12349 "${CA_CSMS_PATH}/CPO_SUB_CA2.pem" "${CA_CSMS_PATH}/CPO_SUB_CA2.key" secp521r1
