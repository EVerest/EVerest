...
struct TestContext : everest::slac::ev::Context {
    TestContext(ContextCallbacks& callbacks, MacAddress const& mac) :
        everest::slac::ev::Context((callbacks.now = test_clock.source(), callbacks), mac) {
    }
};
...
struct TestHarness {
    ...
    explicit TestHarness() : ctx(callbacks, Context::EV_PLC_MAC), machine(ctx) {
        ...
        callbacks.signal_state = [this](D3State value) {
            auto const state = to_string(value);
...
bool test_wait_atten_char_ind_in_wait_state_accepts_late_result() {
    ...
    test_clock.advance(std::chrono::milliseconds(defs::TT_MATCH_RESPONSE_MS + 10));
    harness.machine.update();
...
bool test_timeout_failure_reaches_failed_state() {
    ...
    // One signal per logical D-LINK transition, so entering Failed from a state already reporting
    // UNMATCHED emits nothing new; what matters is where it ends up.
    if (!assert_true(not harness.state_messages.empty() and harness.state_messages.back() == "UNMATCHED", test_name,
                     "did not report UNMATCHED after timeout failure")) {
        return false;
    }
