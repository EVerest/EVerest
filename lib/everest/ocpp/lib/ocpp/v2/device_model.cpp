...
std::optional<WebsocketConnectionOptions> DeviceModel::get_websocket_connection_options(int32_t slot) {
    const auto charge_point_id = get_value<std::string>(ControllerComponentVariables::ChargePointId);
    ...
    try {
        ...
        WebsocketConnectionOptions opts{
            ...
            get_optional_value<int>(ControllerComponentVariables::WebsocketPongTimeout).value_or(5),
