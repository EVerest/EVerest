...
public:
    ...
    get_attribute_source(const std::string& component_name, const std::optional<std::string>& component_instance,
                         const std::optional<int>& component_evse_id, const std::optional<int>& component_connector_id,
                         const std::string& variable_name, const std::optional<std::string>& variable_instance,
                         const AttributeEnum& type);
...
std::optional<std::string> InitDeviceModelDbTest::get_attribute_source(
    const std::string& component_name, const std::optional<std::string>& component_instance,
    const std::optional<int>& component_evse_id, const std::optional<int>& component_connector_id,
    const std::string& variable_name, const std::optional<std::string>& variable_instance, const AttributeEnum& type) {
    static const std::string select_statement = "SELECT va.VALUE_SOURCE "
                                                "FROM VARIABLE_ATTRIBUTE va "
                                                "WHERE va.VARIABLE_ID=("
                                                ...
                                                "AND v.INSTANCE IS @variable_instance) "
                                                "AND va.TYPE_ID=@type_id";
    std::unique_ptr<StatementInterface> s = this->database->new_statement(select_statement);
    s->bind_text("@component_name", component_name, SQLiteString::Transient);
    if (component_instance.has_value()) {
        s->bind_text("@component_instance", component_instance.value(), SQLiteString::Transient);
    } else {
        s->bind_null("@component_instance");
    }
    if (component_evse_id.has_value()) {
        s->bind_int("@evse_id", component_evse_id.value());
    } else {
        s->bind_null("@evse_id");
    }
    if (component_connector_id.has_value()) {
        s->bind_int("@connector_id", component_connector_id.value());
    } else {
        s->bind_null("@connector_id");
    }
    s->bind_text("@variable_name", variable_name, SQLiteString::Transient);
    if (variable_instance.has_value()) {
        s->bind_text("@variable_instance", variable_instance.value(), SQLiteString::Transient);
    } else {
        s->bind_null("@variable_instance");
    }
    s->bind_int("@type_id", static_cast<int>(type));
...
TEST_F(InitDeviceModelDbTest, patch_with_ocpp16_full_config_maps_values) {
    ...
    EXPECT_TRUE(attribute_has_value("SmartChargingCtrlr", std::nullopt, std::nullopt, std::nullopt, "RateUnit",
                                    std::nullopt, AttributeEnum::Actual, "A,W"));
    ...
    EXPECT_TRUE(attribute_has_value("OCPP16LegacyCtrlr", std::nullopt, std::nullopt, std::nullopt, "NumberOfConnectors",
    ...
    // SecurityCtrlr.BasicAuthPassword retains its preset value: AuthorizationKey is routed to
    // NetworkConfiguration[N].BasicAuthPassword by the special-case handler, not to SecurityCtrlr.
    ...
    EXPECT_TRUE(attribute_has_value("SecurityCtrlr", std::nullopt, std::nullopt, std::nullopt, "OrganizationName",
                                    std::nullopt, AttributeEnum::Actual, "EVerest"));
    // SecurityCtrlr.SecurityProfile is not targeted by the OCPP 1.6 migration (SecurityProfile maps to
    // NetworkConfiguration[1]), so it retains the default value from the component config
    ...
    // ============================================================================
    // OCPP 1.6 network config keys -> NetworkConfiguration[1]
    // ============================================================================
    EXPECT_TRUE(attribute_has_value("NetworkConfiguration", "1", std::nullopt, std::nullopt, "OcppCsmsUrl",
                                    std::nullopt, AttributeEnum::Actual,
                                    "127.0.0.1:8181/steve/websocket/CentralSystemService/"));
    EXPECT_TRUE(attribute_has_value("NetworkConfiguration", "1", std::nullopt, std::nullopt, "SecurityProfile",
                                    std::nullopt, AttributeEnum::Actual, "1"));
    EXPECT_TRUE(attribute_has_value("NetworkConfiguration", "1", std::nullopt, std::nullopt, "BasicAuthPassword",
                                    std::nullopt, AttributeEnum::Actual, "SECRETSECRETSECRET"));
    EXPECT_TRUE(attribute_has_value("NetworkConfiguration", "1", std::nullopt, std::nullopt, "HostName", std::nullopt,
                                    AttributeEnum::Actual, "cp002.local"));
    EXPECT_TRUE(attribute_has_value("NetworkConfiguration", "1", std::nullopt, std::nullopt, "Identity", std::nullopt,
                                    AttributeEnum::Actual, "cp002"));
    ...
    // Verify that values set by the OCPP 1.6 patcher have VALUE_SOURCE='OCPP16Config', not 'default'.
    // This protects them from being overwritten by another module's initialize_database call that uses 'default'.
    EXPECT_EQ(get_attribute_source("OCPPCommCtrlr", std::nullopt, std::nullopt, std::nullopt, "HeartbeatInterval",
                                   std::nullopt, AttributeEnum::Actual),
              "OCPP16Config");
    EXPECT_EQ(get_attribute_source("AlignedDataCtrlr", std::nullopt, std::nullopt, std::nullopt, "Interval",
                                   std::nullopt, AttributeEnum::Actual),
              "OCPP16Config");
    EXPECT_EQ(get_attribute_source("OCPPCommCtrlr", std::nullopt, std::nullopt, std::nullopt, "ResetRetries",
                                   std::nullopt, AttributeEnum::Actual),
              "OCPP16Config");
}

// Verifies that a value written with VALUE_SOURCE='OCPP16Config' is NOT overwritten when initialize_database
// is called again with component configs that carry a different default
TEST_F(InitDeviceModelDbTest, ocpp16_values_protected_from_default_overwrite) {
    const auto ocpp16_config = std::make_unique<ocpp::v16::ChargePointConfiguration>(
        ocpp::tests::make_ocpp16_test_config_full(), CONFIG_DIR_V16, USER_CONFIG_FILE_LOCATION_V16);
    ...
    // HeartbeatInterval was patched to 905 with VALUE_SOURCE='OCPP16Config'.
    ASSERT_TRUE(attribute_has_value("OCPPCommCtrlr", std::nullopt, std::nullopt, std::nullopt, "HeartbeatInterval",
                                    std::nullopt, AttributeEnum::Actual, "905"));
    ASSERT_EQ(get_attribute_source("OCPPCommCtrlr", std::nullopt, std::nullopt, std::nullopt, "HeartbeatInterval",
                                   std::nullopt, AttributeEnum::Actual),
              "OCPP16Config");

    // Simulate a second module calling initialize_database with fresh (unpatched) component configs that carry a
    // default for HeartbeatInterval. Inject a default of 42 directly into the component config before calling.
    auto fresh_configs = get_all_component_configs(STANDARD_CONFIGS_PATH);
    for (auto& [ck, variables] : fresh_configs) {
        if (ck.name != "OCPPCommCtrlr") {
            continue;
        }
        for (auto& var : variables) {
            if (var.name != "HeartbeatInterval") {
                continue;
            }
            var.default_actual_value = "42";
            for (auto& attr : var.attributes) {
                if (attr.variable_attribute.type == AttributeEnum::Actual) {
                    attr.variable_attribute.value = "42";
                }
            }
        }
    }
    db.initialize_database(fresh_configs, false);

    // The OCPP16Config-sourced value must survive — the second module's 'default' must not win.
    EXPECT_TRUE(attribute_has_value("OCPPCommCtrlr", std::nullopt, std::nullopt, std::nullopt, "HeartbeatInterval",
                                    std::nullopt, AttributeEnum::Actual, "905"));
    EXPECT_EQ(get_attribute_source("OCPPCommCtrlr", std::nullopt, std::nullopt, std::nullopt, "HeartbeatInterval",
                                   std::nullopt, AttributeEnum::Actual),
