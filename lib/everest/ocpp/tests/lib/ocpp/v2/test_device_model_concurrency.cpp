...
TEST_F(DeviceModelConcurrencyTest, concurrent_access_no_data_race) {
    ...
    // Register a listener up front so set_value takes the copy-listener-and-notify path, exercising it
    // against concurrent re-registration.
    ...
    auto worker = [&](int seed) {
        for (int i = 0; i < iterations; ++i) {
            ...
            case 4: {
                // Tolerate the throw: the Actual value may be absent depending on interleaving.
                try {
            ...
            case 7: {
                // Locked reader that walks the whole device_model_map.
                (void)dm->get_base_report_data(ReportBaseEnum::FullInventory);
                break;
            }
            case 8: {
                // Wrapper path: delegates through set_read_only_value into set_value (which locks).
                const auto& sp = ControllerComponentVariables::SecurityProfile;
                dm->set_read_only_value(sp.component, sp.variable.value(), AttributeEnum::Actual, "1", "test");
                break;
            }
            }
        }
...
/// inversion with consumer locks: a listener waits on a resource guarded by a lock whose holder concurrently
/// calls register_variable_listener. If set_value still owned the mutex while notifying, the helper below
/// could never register and the bounded wait would time out.
TEST_F(DeviceModelConcurrencyTest, listener_invoked_without_device_model_lock) {
    const Component component = cv.component;
    const Variable variable = cv.variable.value();

    // set_value only notifies for variables that have monitors, so install one first.
    SetMonitoringData monitor_request;
    monitor_request.value = 0.0;
    monitor_request.type = MonitorEnum::PeriodicClockAligned;
    monitor_request.severity = 7;
    monitor_request.component = component;
    monitor_request.variable = variable;
    const auto monitor_results = dm->set_monitors({monitor_request});
    ASSERT_EQ(monitor_results.size(), 1);
    ASSERT_EQ(monitor_results.at(0).status, SetMonitoringStatusEnum::Accepted);
    ...
    std::thread helper([&] {
        {
            ...
            if (!cond.wait_for(lk, std::chrono::seconds(30), [&] { return in_listener; })) {
                return; // listener never fired; the assertion below reports the failure
            }
        }
