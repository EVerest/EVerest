...
TEST_F(SecurityTest, v2g20_certificate_installation_enabled_requires_both_flags_on_any_version) {
    ...
    // V2G20CertificateInstallationEnabled defaults to true: enabled without touching it, on 2.0.1 as on 2.1
    this->ocpp_version = ocpp::OcppProtocolVersion::v201;
...
TEST_F(SecurityTest, sign_certificate_request_v2g20_on_ocpp201_sent_as_v2gcertificate) {
    // OCPP 2.0.1 has no V2G20Certificate: the -20 CSR is labelled V2GCertificate (and carries no 2.1-only field),
    // the CSMS tells it apart from a -2 CSR by its secp521r1 key. The awaited type stays V2G20Certificate.
    set_secc_csr_inputs();
    ocpp::GetCertificateSignRequestResult sign_request_result;
    sign_request_result.status = GetCertificateSignRequestStatus::Accepted;
    sign_request_result.csr = "csr20";
    ...
    security.sign_certificate_req(ocpp::CertificateSigningUseEnum::V2G20Certificate);
    EXPECT_EQ(security.awaited_certificate_signing_use_enum, ocpp::CertificateSigningUseEnum::V2G20Certificate);
}

TEST_F(SecurityTest, certificate_signed_v2gcertificate_while_awaiting_v2g20_installs_v2g20) {
    // A 2.0.1 CSMS echoes V2GCertificate for the -20 CSR; the chain answers the outstanding -20 request
    set_secc_csr_inputs();
    ocpp::GetCertificateSignRequestResult sign_request_result;
    sign_request_result.status = GetCertificateSignRequestStatus::Accepted;
    sign_request_result.csr = "csr20";
    ...
    security.handle_message(
        create_example_certificate_signed_request("", ocpp::v2::CertificateSigningUseEnum::V2GCertificate));
    EXPECT_FALSE(security.awaited_certificate_signing_use_enum.has_value());
}

TEST_F(SecurityTest, certificate_signed_charging_station_type_while_awaiting_v2g20_keeps_explicit_type) {
    // The SECC re-labelling only applies between the two SECC leaf types
    set_security_profile(this->device_model, 1);
    set_secc_csr_inputs();
    ocpp::GetCertificateSignRequestResult sign_request_result;
    sign_request_result.status = GetCertificateSignRequestStatus::Accepted;
...
TEST_F(SecurityTest, secc_certificate_signed_rearms_expiry_check_after_gap) {
    // Both SECC leafs are due: the -2 CSR goes first, the -20 CSR is requested by a follow-up check armed
    // V2GCertificateExpireCheckInitialDelaySeconds after the -2 round has ended. The follow-up round arms nothing.
    set_secc_csr_inputs();
    set_bool_variable(this->device_model, ControllerComponentVariables::V2GCertificateInstallationEnabled, true);
    const auto& initial_delay = ControllerComponentVariables::V2GCertificateExpireCheckInitialDelaySeconds;
    EXPECT_EQ(this->device_model->set_value(initial_delay.component, initial_delay.variable.value(),
                                            AttributeEnum::Actual, "42", "test", true),
              SetVariableStatusEnum::Accepted);
    ...
    sign_request_result.status = GetCertificateSignRequestStatus::Accepted;
    sign_request_result.csr = "csr";
    EXPECT_CALL(evse_security, get_leaf_expiry_days_count(ocpp::CertificateSigningUseEnum::V2GCertificate))
        .WillOnce(Return(0));
    EXPECT_CALL(evse_security, get_leaf_expiry_days_count(ocpp::CertificateSigningUseEnum::V2G20Certificate)).Times(0);
    EXPECT_CALL(evse_security,
    ...
    security.scheduled_check_v2g_certificate_expiration();
    ASSERT_TRUE(security.secc_follow_up.has_value());
    EXPECT_EQ(security.secc_follow_up->certificate_signing_use, ocpp::CertificateSigningUseEnum::V2G20Certificate);
    EXPECT_FALSE(security.secc_follow_up->regardless_of_expiry);
    EXPECT_EQ(timer_stub_get_timeout_called_count(), 0);

    // The -2 answer ends the round: the check is re-armed after the gap
    EXPECT_CALL(ocsp_updater, trigger_ocsp_cache_update()).Times(2);
    EXPECT_CALL(mock_dispatcher, dispatch_call_result(_)).Times(2);
    EXPECT_CALL(evse_security, update_leaf_certificate("", ocpp::CertificateSigningUseEnum::V2GCertificate))
        .WillOnce(Return(ocpp::InstallCertificateResult::Accepted));
    security.handle_message(
        create_example_certificate_signed_request("", ocpp::v2::CertificateSigningUseEnum::V2GCertificate));
    EXPECT_EQ(timer_stub_get_timeout_called_count(), 1);
    EXPECT_EQ(timer_stub_get_timeout_interval_ms(), 42000);

    // The follow-up round requests the (due) -20 leaf, as V2GCertificate on 2.0.1
    sign_request_result.csr = "csr20";
    EXPECT_CALL(evse_security, get_leaf_expiry_days_count(ocpp::CertificateSigningUseEnum::V2G20Certificate))
        .WillOnce(Return(0));
    EXPECT_CALL(evse_security,
                generate_certificate_signing_request(ocpp::CertificateSigningUseEnum::V2G20Certificate, _, _, _, _))
        .WillOnce(Return(sign_request_result));
    ...
    security.scheduled_check_v2g_certificate_expiration();
    EXPECT_FALSE(security.secc_follow_up.has_value());

    // Its answer, typed V2GCertificate, is installed as the -20 leaf; no further check is armed
    timer_stub_reset_timeout_called_count();
    EXPECT_CALL(evse_security, update_leaf_certificate("", ocpp::CertificateSigningUseEnum::V2G20Certificate))
        .WillOnce(Return(ocpp::InstallCertificateResult::Accepted));
    security.handle_message(
...
TEST_F(SecurityTest, secc_follow_up_not_scheduled_when_v2g20_disabled) {
    set_secc_csr_inputs();
    set_bool_variable(this->device_model, ControllerComponentVariables::V2GCertificateInstallationEnabled, true);
    set_bool_variable(this->device_model, ControllerComponentVariables::V2G20CertificateInstallationEnabled, false);
    timer_stub_reset_timeout_called_count();
    ...
    EXPECT_CALL(evse_security, get_leaf_expiry_days_count(ocpp::CertificateSigningUseEnum::V2GCertificate))
        .WillOnce(Return(0));
    EXPECT_CALL(evse_security, get_leaf_expiry_days_count(ocpp::CertificateSigningUseEnum::V2G20Certificate)).Times(0);
    EXPECT_CALL(evse_security, generate_certificate_signing_request(_, _, _, _, _))
        .WillOnce(Return(sign_request_result));
    EXPECT_CALL(mock_dispatcher, dispatch_call(_, _)).Times(1);
    security.scheduled_check_v2g_certificate_expiration();
    EXPECT_FALSE(security.secc_follow_up.has_value());
...
TEST_F(SecurityTest, triggered_sign_v2g_on_ocpp201_requests_v2g20_regardless_of_expiry) {
    // Before OCPP 2.1 there is no SignV2G20Certificate: a triggered SignV2GCertificate renews both SECC leafs, the
    // -20 one in the follow-up round without consulting its expiry
    set_secc_csr_inputs();
    set_bool_variable(this->device_model, ControllerComponentVariables::V2GCertificateInstallationEnabled, true);
    timer_stub_reset_timeout_called_count();
    timer_stub_reset_timeout_interval();
    ...
    sign_request_result.status = GetCertificateSignRequestStatus::Accepted;
    sign_request_result.csr = "csr";
    EXPECT_CALL(evse_security, get_leaf_expiry_days_count(_)).Times(0);
    EXPECT_CALL(evse_security,
                generate_certificate_signing_request(ocpp::CertificateSigningUseEnum::V2GCertificate, _, _, _, _))
        .WillOnce(Return(sign_request_result));
    EXPECT_CALL(mock_dispatcher, dispatch_call(_, true)).Times(1);
    security.sign_certificate_req(ocpp::CertificateSigningUseEnum::V2GCertificate, true);
    ASSERT_TRUE(security.secc_follow_up.has_value());
    EXPECT_EQ(security.secc_follow_up->certificate_signing_use, ocpp::CertificateSigningUseEnum::V2G20Certificate);
    EXPECT_TRUE(security.secc_follow_up->regardless_of_expiry);
    ...
    EXPECT_CALL(evse_security,
                generate_certificate_signing_request(ocpp::CertificateSigningUseEnum::V2G20Certificate, _, _, _, _))
        .WillOnce(Return(sign_request_result));
    EXPECT_CALL(mock_dispatcher, dispatch_call(_, false)).WillOnce(Invoke([](const json& call, bool) {
        auto request = call[ocpp::CALL_PAYLOAD].get<SignCertificateRequest>();
        EXPECT_EQ(request.certificateType.value(), ocpp::v2::CertificateSigningUseEnum::V2GCertificate);
        EXPECT_EQ(request.csr, "csr20");
    }));
...
TEST_F(SecurityTest, triggered_sign_v2g_on_ocpp21_requests_v2g_only) {
    // OCPP 2.1 has SignV2G20Certificate for the -20 leaf
    this->ocpp_version = ocpp::OcppProtocolVersion::v21;
    set_secc_csr_inputs();
    set_bool_variable(this->device_model, ControllerComponentVariables::V2GCertificateInstallationEnabled, true);
...
TEST_F(SecurityTest, sign_certificate_rejected_by_csms_ends_round_and_runs_follow_up_once) {
    // A rejected SignCertificate.req ends the round like an answer does; the follow-up round handles the other
    // leaf and, rejected as well, arms nothing further
    set_secc_csr_inputs();
    set_bool_variable(this->device_model, ControllerComponentVariables::V2GCertificateInstallationEnabled, true);
...
TEST_F(SecurityTest, sign_certificate_retries_exhausted_ends_round_and_arms_follow_up) {
    set_secc_csr_inputs();
    set_bool_variable(this->device_model, ControllerComponentVariables::V2GCertificateInstallationEnabled, true);
    ...
    // Accepted, no CertificateSigned.req: the retry timer is armed with CertSigningWaitMinimum
    ...
    timer_stub_get_callback()(); // the retry sends the second CSR

    // Retries exhausted: the round ends and the follow-up check is armed after the gap
    timer_stub_reset_timeout_called_count();
    timer_stub_reset_timeout_interval();
    security.handle_message(create_example_sign_certificate_response(GenericStatusEnum::Accepted));
    EXPECT_FALSE(security.awaited_certificate_signing_use_enum.has_value());
