...
public:
    ...
    ///
    /// \brief Remove all components from the database that match the given component keys.
    /// \details This is intended for explicitly purging components that were generated by a feature flag in a previous
    ///          run and are no longer part of the desired device model. The component must already exist in the database;
    ///          the db_id in \p components is ignored.
    /// \param components Components to remove from the database.
    ///
    /// \throws InitDeviceModelDbError When components could not be retrieved or removed from the db.
    ///
    void remove_components_from_db(const std::map<ComponentKey, std::vector<DeviceModelVariable>>& components);
