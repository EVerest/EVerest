...
constexpr double DEFAULT_LIMIT_AMPS = 48.0;
constexpr double DEFAULT_LIMIT_WATTS = 33120.0;
constexpr std::int32_t DEFAULT_AND_MAX_NUMBER_PHASES = 3;
constexpr double LOW_VOLTAGE = 230;

constexpr double NO_LIMIT_SPECIFIED = -1.0;
constexpr double NO_SETPOINT_SPECIFIED = std::numeric_limits<double>::max();
constexpr double NO_DISCHARGE_LIMIT_SPECIFIED = -std::numeric_limits<double>::max();
