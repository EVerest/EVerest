...
public:
    ...
    virtual std::optional<ocpp::v2::NetworkConnectionProfile> read_network_connection_profile(int32_t slot) = 0;

    /// \brief Persist a NetworkConnectionProfile for \p slot.
    /// \return true on success.
    virtual bool write_network_connection_profile(int32_t slot, const ocpp::v2::NetworkConnectionProfile& profile,
                                                  const std::string& source) = 0;

    /// \brief Erase all persisted data for \p slot.
    virtual void clear_network_connection_profile(int32_t slot) = 0;
    ...
    virtual int32_t get_security_profile() = 0;
    ...
    virtual std::optional<int32_t> get_network_config_timeout() = 0;
    ...
    ///         URL is malformed.
    virtual std::optional<WebsocketConnectionOptions> get_websocket_connection_options(int32_t slot) = 0;

    /// \brief Persist the security profile of the currently active connection.
    virtual void set_active_security_profile(int32_t security_profile, const std::string& source) = 0;

    /// \brief Persist the currently active network-profile slot.
    virtual void set_active_network_profile_slot(int32_t slot, const std::string& source) = 0;

    /// \brief Persist the negotiated OCPP protocol version for a specific slot.
    virtual void set_per_slot_ocpp_version(int32_t slot, const std::string& version,
                                           const std::string& source) = 0;

    /// \brief Persist the security profile on the SecurityCtrlr component after a successful connect.
    virtual void set_security_ctrl_security_profile(int32_t security_profile, const std::string& source) = 0;
