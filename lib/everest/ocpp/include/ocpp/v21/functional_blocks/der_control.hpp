...
/// \brief Lifecycle state reported to the provider when a DER control is applied to / removed from
/// the hardware. libocpp tracks the controls; the provider acts on these transitions.
enum class DERControlAppliedState {
    Set,                ///< A control became active immediately on an accepted SetDERControl (a future
                        ///< startTime instead fires ScheduledActivated later).
    ScheduledActivated, ///< A scheduled control's startTime arrived and it became active.
    Cleared,            ///< A control was removed by ClearDERControl.
    Superseded,         ///< A control was displaced by a control with a strictly lower priority value.
    Expired,            ///< A scheduled control's duration elapsed and it was removed.
};

/// \brief Callback invoked when a DER control transitions to one of \ref DERControlAppliedState.
/// The request is the control that transitioned, reconstructed from the persisted control.
using DERControlAppliedCallback = std::function<void(const SetDERControlRequest&, DERControlAppliedState)>;
