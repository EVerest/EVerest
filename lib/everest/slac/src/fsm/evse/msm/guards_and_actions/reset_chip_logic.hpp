...
struct reset_unsupported {
    template <class Fsm, class Evt, class SrcT, class TarT> bool operator()(Evt const&, Fsm& fsm, SrcT&, TarT&) {
        const auto vendor = fsm.ctx->modem_vendor;
        return vendor != defs::ModemVendor::Qualcomm and vendor != defs::ModemVendor::Lumissil;
    }
};
...
struct send_message {
    template <class Fsm, class Evt, class SrcT, class TarT> void operator()(Evt const&, Fsm& fsm, SrcT&, TarT&) {
        ...
        if (ctx.modem_vendor == defs::ModemVendor::Qualcomm) {
            ...
            ctx.log_info("Resetting HW chip using CM_RESET_DEVICE.REQ");
            ...
            ctx.log_warn("Chip reset is not supported on this modem, skipping it");
        }
    }
};

struct log_reset_timeout {
    template <class Fsm, class Evt, class SrcT, class TarT> void operator()(Evt const&, Fsm& fsm, SrcT&, TarT&) {
        fsm.ctx->log_error("Reset timeout, no CM_RESET_DEVICE.CNF received - failed to confirm the chip reset");
    }
};
