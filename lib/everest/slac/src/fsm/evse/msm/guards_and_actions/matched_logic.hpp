...
// The update tick of a vendor state serves two timers, the link-status poll and the CM_AMP_MAP
// retransmission, in one internal transition so that both run when both are due in the same tick.
struct matched_tick_due {
    template <class Fsm, class Evt, class SrcT, class TarT> bool operator()(Evt const&, Fsm& fsm, SrcT& src, TarT&) {
        auto const now = fsm.ctx->current_time;
        return src.state_timeout(now) or fsm.amp_map.retransmit_due(now);
    }
};
struct service_matched_tick {
    template <class Fsm, class Evt, class SrcT, class TarT> void operator()(Evt const&, Fsm& fsm, SrcT& src, TarT&) {
        auto const now = fsm.ctx->current_time;
        if (src.state_timeout(now)) {
            src.to.reset(now);
            src.link_status_req(fsm);
        }
        if (fsm.amp_map.retransmit_due(now)) {
            fsm.amp_map.retransmit(*fsm.ctx);
        }
    }
};
