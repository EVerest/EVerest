...
struct Session_def : public state_machine_def<Session_def> {
    ...
    // Rows sharing source and event have mutually exclusive guards; never rely on table order.
    using retry_due = And_<timeout, Not_<retry_limit>>;
    using give_up = And_<timeout, retry_limit>;
    using match_timeout = And_<timeout, Not_<validation_window_expired>>;
    ...
    struct transition_table : boost::mpl::vector<
        ...
        Row   < WaitAttenRsp     , update  , WaitAttenRsp     , retry_snd             , retry_due                 >,
        Row   < WaitAttenRsp     , update  , Failed           , log_no_atten_rsp      , give_up                   >,
        Row   < WaitAttenRsp     , message , WaitSlacMatch    , on_atten_char_rsp     , is_atten_char_rsp         >,
        Row   < WaitSlacMatch    , update  , Failed           , log_no_slac_match     , match_timeout             >,
