...
struct SlacFSM_def : state_machine_def<SlacFSM_def> {
    ...
    struct transition_table : boost::mpl::vector<
        ...
        //  +-------------------+-----------+-------------+-----------------+-------------------+
        Row < ResetChip         , reset     , Reset       , none            , none              >,
    ...
    template <class FSM, class Event> void no_transition(Event const& e, FSM&, int state) {
        if constexpr (std::is_same_v<Event, message>) {
            ...
            ss << "Ignoring SLAC message 0x" << std::hex << e.payload.get_mmtype() << " in state " << std::dec << state;
            ctx->log_debug(ss.str());
        } else if constexpr (not std::is_same_v<Event, update>) {
            std::string const name = std::is_same_v<Event, reset>       ? "reset"
                                     : std::is_same_v<Event, enter_bcd> ? "enter_bcd"
                                     : std::is_same_v<Event, leave_bcd> ? "leave_bcd"
                                                                        : "event";
            ctx->log_debug("Ignoring " + name + " in state " + std::to_string(state));
        }
    }
