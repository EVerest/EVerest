...
bool is_cm_amp_map_req(messages::HomeplugMessage const& msg) {
    return msg.get_mmtype() == (defs::MMTYPE_CM_AMP_MAP | defs::MMTYPE_MODE_REQ);
}
...
bool test_short_link_status_cnf_does_not_leave_matched_or_failed() {
    ...
    auto changed_state = wait_for(std::chrono::milliseconds(120), machine,
                                  [&ctx]() { return ctx.status.match_state != SlacState::Matched; });
    if (!assert_true(not changed_state, test_name, "short link-status CNF changed matched state")) {
        return false;
    }

    // A full-length negative link-status CNF is a connection loss: the SECC must leave the AVLN, i.e.
    // go to Reset (re-key with a fresh NMK), not linger in Matched (ISO 15118-5 PLCLinkStatus_005).
    machine.message(create_lumissil_link_status_cnf(modem_source, 0x00));
    auto left_matched = wait_for(std::chrono::milliseconds(200), machine, [&ctx]() {
        return ctx.status.match_state == SlacState::Reset || ctx.status.match_state == SlacState::Idle;
    });
    if (!assert_true(left_matched, test_name, "did not transition to Reset on negative full-length link-status CNF")) {
        return false;
    }
...
bool test_cm_validate_bcb_toggle_detection() {
    ...
    ctx.bc_transition_count.store(7);
    ...
    ctx.bc_transition_count.store(7 + 3);
...
bool test_matched_link_status_rejects_only_negative_cnf() {
    ...
    // Connection loss while matched: leave the AVLN via Reset (fresh NMK re-key), not Failed
    // (ISO 15118-5 PLCLinkStatus_005; commit "leave AVLN on matched-state connection loss").
    machine.message(create_lumissil_link_status_cnf(modem_source, 0x00));
    auto left_matched = wait_for(std::chrono::milliseconds(200), machine, [&ctx]() {
        return ctx.status.match_state == SlacState::Reset || ctx.status.match_state == SlacState::Idle;
    });
    if (!assert_true(left_matched, test_name, "did not transition to Reset on negative link-status CNF")) {
        return false;
    }

    return assert_true(not ctx.match_confirm_cache.valid, test_name, "Reset retained cached CM_SLAC_MATCH.CNF");
}
...
bool test_matched_link_status_neg_debounce_tolerates_transient_flaps() {
    const char* test_name = "test_matched_link_status_neg_debounce_tolerates_transient_flaps";
    ContextCallbacks callbacks{};
    std::vector<SentMessage> sent_messages;
    callbacks.send_raw_slac = [&sent_messages](messages::HomeplugMessage& hp_message) {
        sent_messages.push_back({sent_messages.size(), hp_message});
    ...
    auto const stays_matched = [&ctx, &machine]() {
        return not wait_for(std::chrono::milliseconds(60), machine,
                            [&ctx]() { return ctx.status.match_state != SlacState::Matched; });
    };

    machine.message(create_qualcomm_link_status_cnf(modem_source, 0x00));
    if (!assert_true(stays_matched(), test_name, "1st negative link-status CNF left Matched despite debounce")) {
        return false;
    }
    machine.message(create_qualcomm_link_status_cnf(modem_source, 0x00));
    if (!assert_true(stays_matched(), test_name, "2nd negative link-status CNF left Matched despite debounce")) {
        return false;
    }

    machine.message(create_qualcomm_link_status_cnf(modem_source, defs::D_LINK_STATUS_LINKED));
    if (!assert_true(stays_matched(), test_name, "positive link-status CNF changed matched state")) {
        return false;
    }
    machine.message(create_qualcomm_link_status_cnf(modem_source, 0x00));
    if (!assert_true(stays_matched(), test_name,
                     "1st negative after recovery left Matched (positive CNF did not reset the count)")) {
        return false;
    }
    machine.message(create_qualcomm_link_status_cnf(modem_source, 0x00));
    if (!assert_true(stays_matched(), test_name,
                     "2nd negative after recovery left Matched (positive CNF did not reset the count)")) {
        return false;
    }
...
// Guard pairs on one source/event must be exclusive; these two pin the behaviour the pairs encode.

bool test_atten_char_rsp_retransmitted_twice_then_session_fails() {
    const char* test_name = "test_atten_char_rsp_retransmitted_twice_then_session_fails";
    ContextCallbacks callbacks{};
    std::vector<SentMessage> sent_messages;
    callbacks.send_raw_slac = [&sent_messages](messages::HomeplugMessage& hp_message) {
        sent_messages.push_back({sent_messages.size(), hp_message});
    ...
    EvMac ev_mac = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0x21};
    auto run_id = fill_run_id(0x21);
    machine.message(create_cm_slac_parm_req(ev_mac, run_id));
    if (!wait_for_parm_cnf_count(sent_messages, 1, machine, 100)) {
        return assert_true(false, test_name, "did not emit CM_SLAC_PARM.CNF");
    }
    ...
    if (!wait_for_atten_char_ind_count(sent_messages, 1, machine, 300)) {
        return assert_true(false, test_name, "did not emit the first CM_ATTEN_CHAR.IND");
    }

    // No CM_ATTEN_CHAR.RSP ever arrives: C_EV_match_retry (2) retransmissions, TT_match_response apart.
    if (!wait_for_atten_char_ind_count(sent_messages, 2, machine, defs::TT_MATCH_RESPONSE_MS + 50)) {
        return assert_true(false, test_name, "first CM_ATTEN_CHAR.IND retransmission missing");
    }
    if (!wait_for_atten_char_ind_count(sent_messages, 3, machine, defs::TT_MATCH_RESPONSE_MS + 50)) {
        return assert_true(false, test_name, "second CM_ATTEN_CHAR.IND retransmission missing");
    }
    // Then the session gives up and, being the only one, takes Matching to Failed.
    if (!wait_for_match_state(ctx, SlacState::Failed, machine, defs::TT_MATCH_RESPONSE_MS + 100)) {
        return assert_true(false, test_name, "did not fail after the retransmissions were exhausted");
    }
    return assert_true(count_cm_atten_char_ind(sent_messages) == 3, test_name,
                       "CM_ATTEN_CHAR.IND count is not initial + C_EV_match_retry");
}

bool test_matched_poll_and_amp_map_retransmit_run_in_the_same_tick() {
    const char* test_name = "test_matched_poll_and_amp_map_retransmit_run_in_the_same_tick";
    ContextCallbacks callbacks{};
    std::vector<SentMessage> sent_messages;
    callbacks.send_raw_slac = [&sent_messages](messages::HomeplugMessage& hp_message) {
        sent_messages.push_back({sent_messages.size(), hp_message});
    ...
    ctx.slac_config.link_status.do_detect = true;
    ctx.slac_config.link_status.retry = 20ms;
    // Same cadence as the CM_AMP_MAP retransmission (TT_match_response), so both fall due together.
    ctx.slac_config.link_status.poll_in_matched_state = std::chrono::milliseconds(defs::TT_MATCH_RESPONSE_MS);
    ctx.slac_config.link_status.timeout = 300ms;
    ...
    EvMac ev_mac = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0x22};
    auto run_id = fill_run_id(0x22);
    if (!perform_full_match_sequence(ctx, sent_messages, machine, ev_mac, run_id, SlacState::WaitForLink, 700)) {
        return false;
    }
    EvMac modem_source = {0x00, 0xB0, 0x52, 0x00, 0x00, 0x02};
    ...
    if (!assert_true(count_cm_amp_map_req(sent_messages) == 1, test_name,
                     "entering Matched did not send the first CM_AMP_MAP.REQ")) {
        return false;
    }

    // Tick until the poll fires. The amp-map retransmission is due on the same tick and must go out in
    // that very update; one tick earlier or later means the two rows competed instead of being served together.
    const auto polls_before = count_qualcomm_link_status_req(sent_messages);
    const auto amp_before = count_cm_amp_map_req(sent_messages);
    auto amp_before_this_tick = amp_before;
    bool poll_fired = false;
    const auto deadline = test_clock.now() + std::chrono::milliseconds(defs::TT_MATCH_RESPONSE_MS + 50);
    while (test_clock.now() < deadline) {
        amp_before_this_tick = count_cm_amp_map_req(sent_messages);
        machine.update();
        if (count_qualcomm_link_status_req(sent_messages) > polls_before) {
            poll_fired = true;
            break;
        }
        test_clock.advance_ms(1);
    }
    if (!assert_true(poll_fired, test_name, "matched-state LINK_STATUS.REQ poll did not fire")) {
        return false;
    }
    if (!assert_true(amp_before_this_tick == amp_before, test_name,
                     "CM_AMP_MAP.REQ retransmission went out on an earlier tick than the poll")) {
        return false;
    }
    return assert_true(count_cm_amp_map_req(sent_messages) == amp_before_this_tick + 1, test_name,
...
int main() {
    const auto tests = std::array<std::pair<const char*, bool (*)()>, 39>{
        ...
        std::make_pair("test_atten_char_rsp_retransmitted_twice_then_session_fails",
