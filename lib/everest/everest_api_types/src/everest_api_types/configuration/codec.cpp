...
std::string serialize(ConfigurationParameterGetResultEnum val) noexcept {
    return utilities::dump_json(val);
}
...
std::ostream& operator<<(std::ostream& os, ConfigurationParameterGetResultEnum const& val) {
    os << serialize(val);
    return os;
}
...
template <> ConfigurationParameterGetResultEnum deserialize(std::string_view val) {
    return utilities::parse_json<ConfigurationParameterGetResultEnum>(val);
}
