...
void to_json(json& j, ConfigurationParameterGetResultEnum const& k) noexcept {
    switch (k) {
    case ConfigurationParameterGetResultEnum::OK:
        j = "OK";
        return;
    case ConfigurationParameterGetResultEnum::DoesNotExist:
        j = "DoesNotExist";
        return;
    }

    j = "INVALID_VALUE__everest::lib::API::V1_0::types::configuration::ConfigurationParameterGetResultEnum";
}

void from_json(const json& j, ConfigurationParameterGetResultEnum& k) {
    std::string s = j;
    if (s == "OK") {
        k = ConfigurationParameterGetResultEnum::OK;
        return;
    }
    if (s == "DoesNotExist") {
        k = ConfigurationParameterGetResultEnum::DoesNotExist;
        return;
    }

    throw std::out_of_range("Provided string " + s +
                            " could not be converted to enum of type "
                            "everest::lib::API::V1_0::types::configuration::ConfigurationParameterGetResultEnum");
}
...
void to_json(json& j, GetConfigurationParameterRequest const& k) noexcept {
    ...
    if (k.force_read_from_db) {
        j["force_read_from_db"] = *k.force_read_from_db;
    }
}
...
void from_json(const json& j, GetConfigurationParameterRequest& k) {
    ...
    if (j.contains("force_read_from_db")) {
        k.force_read_from_db = j.at("force_read_from_db");
    }
}
...
void from_json(const json& j, ModuleConfiguration& k) {
    ...
    k.config_access = j.at("config_access");
}
...
void from_json(const json& j, GetConfigurationResult& k) {
    ...
    if (j.contains("module_configurations")) {
        k.module_configurations = j.at("module_configurations");
    }
}
