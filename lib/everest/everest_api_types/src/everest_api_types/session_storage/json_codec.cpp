...
void to_json(json& j, ClearSessionsRequest const& k) noexcept {
    j = json({});
    if (k.up_to_session_id) {
        j["up_to_session_id"] = k.up_to_session_id.value();
    }
}

void from_json(const json& j, ClearSessionsRequest& k) {
    if (j.contains("up_to_session_id")) {
        k.up_to_session_id.emplace(j.at("up_to_session_id"));
    }
}
