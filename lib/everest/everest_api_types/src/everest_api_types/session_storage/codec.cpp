...
std::string serialize(ClearSessionsRequest const& val) noexcept {
    return utilities::dump_json(val);
}
...
std::ostream& operator<<(std::ostream& os, ClearSessionsRequest const& val) {
    os << serialize(val);
    return os;
}
...
template <> ClearSessionsRequest deserialize(std::string_view val) {
    return utilities::parse_json<ClearSessionsRequest>(val);
}
