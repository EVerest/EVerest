...
struct MatchingSession {
    MatchingSession(const uint8_t* ev_mac, const uint8_t* run_id, const uint8_t* evse_mac);
    MatchingSessionData session_data;
    ...
    messages::cm_atten_char_ind calculate_avg() const;
};
...
struct MatchingState : public FSMSimpleState {
    ...
    void handle_slac_message(messages::HomeplugMessage&);

    void handle_cm_slac_parm_req(const messages::cm_slac_parm_req&);
    void handle_cm_start_atten_char_ind(const messages::cm_start_atten_char_ind&);
    void handle_cm_mnbc_sound_ind(const messages::cm_mnbc_sound_ind&);
    void handle_cm_atten_profile_ind(const messages::cm_atten_profile_ind&);
    void handle_cm_atten_char_rsp(const messages::cm_atten_char_rsp&);
    void handle_cm_validate_req(const messages::cm_validate_req&);
    void handle_cm_slac_match_req(const messages::cm_slac_match_req&);
    ...
    std::unique_ptr<messages::cm_slac_match_cnf> match_cnf_message;
    int failed_count{0};
};
