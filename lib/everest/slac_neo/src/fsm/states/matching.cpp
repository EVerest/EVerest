...
MatchingSession::MatchingSession(const uint8_t* ev_mac, const uint8_t* run_id, const uint8_t* evse_mac) :
    session_data(ev_mac, run_id, evse_mac) {
}
...
bool MatchingSession::is_identified_by(const uint8_t* ev_mac, const uint8_t* run_id) const {
    if (0 != memcmp(run_id, this->session_data.run_id, sizeof(this->session_data.run_id))) {
        return false;
    }

    if (0 != memcmp(ev_mac, this->session_data.ev_mac, sizeof(this->session_data.ev_mac))) {
        return false;
    }
    ...
    for (auto& session : sessions) {
        ...
        while (session.timeout_active) {
            ...
            if (session.state == MatchingSubState::WAIT_FOR_START_ATTEN_CHAR) {
                ...
                session.session_data.num_retries = 0;
            } else if (session.state == MatchingSubState::WAIT_FOR_ATTEN_CHAR_RSP) {
                session.session_data.num_retries++;
                if (session.session_data.num_retries <= slac::defs::C_EV_MATCH_RETRY) {
                    session_log(ctx, session, LogLevel::WARN,
