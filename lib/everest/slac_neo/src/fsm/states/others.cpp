...
FSMSimpleState::CallbackReturnType ResetState::callback() {
    ...
    if (setup_has_been_send == false) {
        auto set_key_req = MatchingSessionData::create_cm_set_key_req(cfg.session_nmk);
...
bool ResetState::handle_slac_message(messages::HomeplugMessage& message) {
    const auto mmtype = message.get_mmtype();
    if (mmtype != (defs::MMTYPE_CM_SET_KEY | defs::MMTYPE_MODE_CNF)) {
        // unexpected message
...
bool ResetChipState::handle_slac_message(messages::HomeplugMessage& message) {
    const auto mmtype = message.get_mmtype();
    if (mmtype != (defs::qualcomm::MMTYPE_CM_RESET_DEVICE | defs::MMTYPE_MODE_CNF)) {
        // unexpected message
        ctx.log_warn("Received non-expected SLAC message of type " + format_mmtype(mmtype));
        return false;
    } else {
...
static std::optional<bool> check_link_status_cnf(const defs::ModemVendor modem_vendor,
    ...
    if (modem_vendor == defs::ModemVendor::Qualcomm &&
        mmtype == (defs::qualcomm::MMTYPE_LINK_STATUS | defs::MMTYPE_MODE_CNF)) {
        const auto success = message.get_payload<messages::qualcomm::link_status_cnf>().link_status == 0x01;
        return {success};

    } else if (modem_vendor == defs::ModemVendor::Lumissil &&
               mmtype == (defs::lumissil::MMTYPE_NSCM_GET_D_LINK_STATUS | defs::MMTYPE_MODE_CNF)) {
        const auto success = message.get_payload<messages::lumissil::nscm_get_d_link_status_cnf>().link_status == 0x01;
        return {success};
    }
    return {};
}

static bool send_link_status_req(fsm::evse::Context& ctx) {
    if (ctx.modem_vendor == defs::ModemVendor::Qualcomm) {
...
WaitForLinkState::WaitForLinkState(Context& ctx, std::unique_ptr<messages::cm_slac_match_cnf> sent_match_cnf_message) :
    FSMSimpleState(ctx), match_cnf_message(std::move(sent_match_cnf_message)) {
}

bool WaitForLinkState::handle_slac_message(messages::HomeplugMessage& message) {
    const auto mmtype = message.get_mmtype();
    ...
    if (mmtype == (defs::MMTYPE_CM_SLAC_MATCH | defs::MMTYPE_MODE_REQ)) {
...
FSMSimpleState::CallbackReturnType InitState::callback() {
    ...
    if (sub_state == SubState::QUALCOMM_OP_ATTR) {
        ...
        messages::qualcomm::op_attr_req op_attr_req;
        ...
        sub_state = SubState::DONE;
        messages::lumissil::nscm_get_version_req version_req;
...
static std::string get_qualcomm_device_info(messages::qualcomm::op_attr_cnf const& msg) {
...
static std::string get_lumissil_device_info(messages::lumissil::nscm_get_version_cnf const& msg) {
...
void InitState::handle_slac_message(messages::HomeplugMessage& message) {
    const auto mmtype = message.get_mmtype();
    if (mmtype == (defs::qualcomm::MMTYPE_OP_ATTR | defs::MMTYPE_MODE_CNF)) {
        const auto msg = message.get_payload<messages::qualcomm::op_attr_cnf>();
    ...
    } else if (mmtype == (defs::lumissil::MMTYPE_NSCM_GET_VERSION | defs::MMTYPE_MODE_CNF)) {
        const auto msg = message.get_payload<messages::lumissil::nscm_get_version_cnf>();
