...
TEST_F(TlsTest, TCKeysCert) {
    ...
    auto client_handler_fn = [this, &subject, &negotiated_version](tls::Client::ConnectionPtr& connection) {
        if (connection) {
            if (connection->connect() == result_t::success) {
                this->set(ClientTest::flags_t::connected);
                subject = openssl::certificate_subject(connection->peer_certificate());
...
#pragma GCC diagnostic pop
    ...
    // trusted_ca_keys is the TLS 1.2 branch of the certificate callback's version dispatch.
