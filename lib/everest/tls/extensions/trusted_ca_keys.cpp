...
bool ServerTrustedCaKeys::init_ssl(SslContext* ctx) {
    bool bRes{true};
    // Legacy trusted_ca_keys is TLS 1.2 and below; the TLS 1.3 equivalent is the peer CA
    // list read in handle_certificate_cb().
...
int ServerTrustedCaKeys::apply_selection_locked(SSL* ssl, const chain_t* selected, const char* context_label) {
    ...
    if (fallback == nullptr) {
        // use_certificate_and_key() already called SSL_certs_clear(): the connection holds
        // no certificate and no default chain remains, so the handshake cannot continue.
        log_warning(std::string("terminating TLS handshake: ") + context_label +
...
int ServerTrustedCaKeys::handle_certificate_cb(SSL* ssl, void* arg) {
    ...
    result = tck_p->apply_selection_locked(ssl, selected, context_label);
    return result;
}
