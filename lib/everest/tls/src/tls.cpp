...
bool configure_ssl_ctx(bool is_server, SSL_CTX*& ctx, const char* ciphersuites, const char* cipher_list,
    ...
    if (ctx == nullptr) {
        ...
        // Default: pin signature_algorithms and ECDHE groups to match the
        // leaf cert's EC curve. No-op for non-EC keys. Hard failure on
        // SSL_CTX_set1_sigalgs_list or SSL_CTX_set1_groups_list error.
        if (result && cert_config.certificate_chain_file != nullptr) {
            if (!openssl::pin_sigalgs_to_cert_curve(ctx)) {
                result = false;
            }
            if (!openssl::pin_groups_to_cert_curve(ctx)) {
                result = false;
            }
        }
    }
