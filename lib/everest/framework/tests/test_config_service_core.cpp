  ...
  dummy_module:
    ...
    config_implementation:
      main:
        ...
        auto it_dup = std::find_if(slots.begin(), slots.end(), [new_slot_id](const auto& s) { return s.id == new_slot_id; });
        ...
        slots = config_service.list_all_slots();
        auto it_dup2 = std::find_if(slots.begin(), slots.end(), [new_slot_id2](const auto& s) { return s.id == new_slot_id2; });
...
active_modules:
  dummy_module:
    ...
    SECTION("State Tracking: remaining state change events") {
        ...
        config_service.register_active_slot_update_handler([&recorded_statuses](const ActiveSlotUpdate& update) {
            recorded_statuses.push_back(update.status);
        });
    ...
    SECTION("Handling WriteOnly parameters and SetCallFailed") {
        ...
        auto result = config_service.set_config_parameters(ConfigServiceInterface::ACTIVE_SLOT, {wo_update, rw_update}, origin);
        REQUIRE(result.parameter_results.has_value());
    ...
    SECTION("Edge Cases: set_config_parameters with non-existent parameter") {
        ...
        auto active_result = config_service.set_config_parameters(ConfigServiceInterface::ACTIVE_SLOT, {update}, origin);
        ...
        CHECK(active_result.parameter_results->front().status_info == "Unknown parameter: ghost_param in module: dummy_module");
        ...
        CHECK(inactive_result.parameter_results->front().status_info == "Unknown parameter: ghost_param in module: dummy_module");
    }
}
