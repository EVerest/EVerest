  ...
  dummy_module:
    ...
    config_implementation:
      main:
        ...
        SECTION("modules Stopped") {
        }
        SECTION("modules FailedToStart") {
            config_service.notice_cfg_validation_failed();
        }
        ...
        // With the modules down, the in-memory configuration follows the persisted value right away.
        auto in_memory = config_service.get_config_parameters(0, {param_id});
        auto from_db = config_service.get_config_parameters(0, {param_id}, true);
        REQUIRE(in_memory.status == GetConfigurationStatus::Success);
        REQUIRE(from_db.status == GetConfigurationStatus::Success);
        REQUIRE(in_memory.parameters.size() == 1);
        REQUIRE(from_db.parameters.size() == 1);
        REQUIRE(in_memory.parameters[0].has_value());
        REQUIRE(from_db.parameters[0].has_value());
        CHECK(std::get<std::string>(in_memory.parameters[0]->value) == "to_be_applied_later");
        CHECK(in_memory.parameters[0]->value == from_db.parameters[0]->value);

        // The lock-free snapshot the manager starts the modules from is updated as well.
        const auto snapshot = config_service.get_active_module_configurations();
        const auto& module_params = snapshot->at("dummy_module").configuration_parameters.at("!module");
        const auto it = std::find_if(module_params.begin(), module_params.end(),
                                     [](const auto& p) { return p.name == "valid_config_entry"; });
        REQUIRE(it != module_params.end());
        CHECK(std::get<std::string>(it->value) == "to_be_applied_later");
    }
    ...
    SECTION("State Tracking: set_modules_at_rest settles transitional statuses but keeps FailedToStart") {
        std::vector<ActiveSlotUpdate> events;
        config_service.register_active_slot_update_handler(
            [&events](const ActiveSlotUpdate& update) { events.push_back(update); });

        config_service.set_modules_starting();
        config_service.set_modules_at_rest();
        REQUIRE(events.size() == 2);
        CHECK(events.back().status == ActiveSlotStatus::Stopped);

        // Idempotent: already resting, so no duplicate event. The usual ShutdownFinalizing -> Idle
        // sequence must emit exactly one Stopped update, not two.
        config_service.set_modules_at_rest();
        CHECK(events.size() == 2);

        // A recorded start failure survives arriving at rest, so it can still be reported while the
        // manager sits in Idle.
        config_service.notice_cfg_validation_failed();
        REQUIRE(events.size() == 3);
        config_service.set_modules_at_rest();
        REQUIRE(events.size() == 3);
        CHECK(events.back().status == ActiveSlotStatus::FailedToStart);

        // ...and the next start attempt clears it.
        config_service.set_modules_starting();
        CHECK(events.back().status == ActiveSlotStatus::Starting);
    }

    SECTION("State Tracking: FailedToStart stays recoverable") {
        std::string valid_yaml = R"(
active_modules:
  dummy_module:
    module: TESTValidManifest
    ...
    config_implementation:
      main:
        ...
        // The manager could not start the modules from this configuration.
        config_service.notice_cfg_validation_failed();

        // Loading a corrected configuration into the ACTIVE slot is the recovery action: it must be
        // accepted, and it must actually replace the active configuration.
        std::string corrected_yaml = R"(
active_modules:
  corrected_module:
    module: TESTValidManifest
    config_module:
      valid_config_entry: "corrected"
    config_implementation:
      main:
        valid_config_entry: "corrected"
)";
        auto load_result = config_service.load_from_yaml(corrected_yaml, "Corrected", 0);
        INFO(load_result.error_message);
        REQUIRE(load_result.success);

        auto active = config_service.get_active_module_configurations();
        REQUIRE(active != nullptr);
        CHECK(active->count("corrected_module") == 1);
        CHECK(active->count("dummy_module") == 0);
    }
...
active_modules:
  dummy_module:
    ...
    SECTION("Slot Management: delete_slot switches away from the active slot while FailedToStart") {
        std::string valid_yaml = R"(
active_modules:
  dummy_module:
    module: TESTValidManifest
    ...
    config_implementation:
      main:
        ...
        REQUIRE(config_service.load_from_yaml(valid_yaml, "Slot 0", 0).success);
        REQUIRE(config_service.load_from_yaml(valid_yaml, "Slot 1", 1).success);

        config_service.mark_active_slot(0);
        config_service.reinitialize_from_db(true);
        REQUIRE(config_service.get_active_slot_id() == 0);

        // Stage slot 1 for the next boot, then record a failed start. The convenience path that
        // switches the active slot to the staged one is gated on the modules being at rest, which
        // FailedToStart is - so deleting the failed active slot must succeed.
        REQUIRE(config_service.mark_active_slot(1) == SetActiveSlotStatus::Success);
        config_service.notice_cfg_validation_failed();

        REQUIRE(config_service.delete_slot(0) == DeleteSlotStatus::Success);
  ...
  dummy_module:
    ...
    config_implementation:
      main:
        ...
        CHECK(res_active.error_message ==
              "Cannot load YAML into the active slot while the modules are running or mid-transition");
    }

    SECTION("Edge Cases: load_from_yaml into the active slot honours resting vs transient statuses") {
        std::string valid_yaml = R"(
active_modules:
  dummy_module:
    module: TESTValidManifest
    ...
    config_implementation:
      main:
        ...
        SECTION("transient statuses reject the load") {
            const auto status_setter = GENERATE_REF(
                std::pair<std::function<void()>, const char*>{[&] { config_service.set_modules_starting(); },
                                                              "Starting"},
                std::pair<std::function<void()>, const char*>{[&] { config_service.set_modules_stopping(); },
                                                              "Stopping"},
                std::pair<std::function<void()>, const char*>{[&] { config_service.notice_module_restart_triggered(); },
                                                              "RestartTriggered"});
            INFO("status: " << status_setter.second);
            status_setter.first();
            CHECK_FALSE(config_service.load_from_yaml(valid_yaml, "Active", 0).success);
        }

        SECTION("resting statuses accept the load") {
            // FailedToStart is resting: loading a corrected configuration into the active slot is
            // exactly how a failed start is recovered from, so it must not be refused.
            const auto status_setter = GENERATE_REF(
                std::pair<std::function<void()>, const char*>{[&] { config_service.set_modules_stopped(); }, "Stopped"},
                std::pair<std::function<void()>, const char*>{[&] { config_service.notice_cfg_validation_failed(); },
                                                              "FailedToStart"});
            INFO("status: " << status_setter.second);
            status_setter.first();
            auto result = config_service.load_from_yaml(valid_yaml, "Active", 0);
            INFO(result.error_message);
    ...
    SECTION("Edge Cases: set_config_parameters honours resting vs transient statuses") {
        std::string valid_yaml = R"(
active_modules:
  dummy_module:
    module: TESTValidManifest
    ...
    config_implementation:
      main:
        ...
        everest::config::ConfigurationParameterIdentifier param_id{"dummy_module", "valid_config_entry", "!module"};
        ConfigParameterUpdate update{param_id, "written_while_down"};
        Origin origin{false, "manager"};

        SECTION("transient statuses defer the write") {
            // RestartTriggered is genuinely transient - a restart really is in flight - so deferring
            // with RetryLater is actionable there.
            const auto status_setter = GENERATE_REF(
                std::pair<std::function<void()>, const char*>{[&] { config_service.set_modules_starting(); },
                                                              "Starting"},
                std::pair<std::function<void()>, const char*>{[&] { config_service.set_modules_stopping(); },
        ...
        SECTION("FailedToStart is resting: the write is accepted and persisted") {
            config_service.notice_cfg_validation_failed();

            auto result = config_service.set_config_parameters(ConfigServiceInterface::ACTIVE_SLOT, {update}, origin);
            REQUIRE(result.status == SetConfigParameterStatus::Ok);
            REQUIRE(result.parameter_results.has_value());
            // No module is alive after a failed start, so there is nothing to apply live.
            CHECK(result.parameter_results->front().status == SetConfigParameterResultEnum::WillApplyOnRestart);

            // The load-bearing part: RetryLater was replaced by a write that actually reached the
            // database, which is what makes a failed start recoverable.
            auto get_result =
                config_service.get_config_parameters(ConfigServiceInterface::ACTIVE_SLOT, {param_id}, true);
            REQUIRE(get_result.parameters.size() == 1);
            REQUIRE(get_result.parameters[0].has_value());
            CHECK(std::get<std::string>(get_result.parameters[0]->value) == "written_while_down");
        }
    }
  ...
  dummy_module:
    ...
    config_implementation:
      main:
        ...
        // FailedToStart is resting too, so the guard must be OPEN there: otherwise a start that
        // failed could never pick up a corrected configuration from the database.
        config_service.notice_cfg_validation_failed();
        REQUIRE(config_service.mark_active_slot(0) == SetActiveSlotStatus::Success);
        config_service.reinitialize_from_db(true);
        CHECK(config_service.get_active_slot_id() == 0);
    }
...
TEST_CASE("ConfigServiceCore persistence mirror", "[config_service_core]") {
...
active_modules:
  dummy_module:
    ...
    const everest::config::ConfigurationParameterIdentifier param_id{"dummy_module", "valid_config_entry", "!module"};
    const ConfigParameterUpdate update{param_id, "persisted value"};
    const Origin origin{true, std::nullopt};

    SECTION("Active-slot writes are mirrored to the user-config YAML") {
        const auto user_config_dir = Everest::tests::get_bin_dir() / "mirror_test_user_config";
        fs::remove_all(user_config_dir);
        fs::create_directories(user_config_dir);
        const auto user_config_path = user_config_dir / "config.yaml";

        ConfigServiceCore config_service(parse_settings, db,
                                         std::make_unique<everest::config::UserConfigStorage>(user_config_path));

        auto lfy_result = config_service.load_from_yaml(valid_yaml, "Mirror test", std::nullopt);
        INFO(lfy_result.error_message);
        REQUIRE(lfy_result.success == true);
        config_service.mark_active_slot(0);
        config_service.reinitialize_from_db(true);
        ...
        REQUIRE(result.parameter_results.has_value());
        INFO(result.parameter_results->front().status_info);
        // The API answer is unchanged by the mirror: the user-config is merged back into the
        // config on the next start, so the value applies on restart.
        CHECK(result.parameter_results->front().status == SetConfigParameterResultEnum::WillApplyOnRestart);

        REQUIRE(fs::exists(user_config_path));
        const auto user_config = Everest::load_yaml(user_config_path);
        CHECK(user_config.at("active_modules").at("dummy_module").at("config_module").at("valid_config_entry") ==
              "persisted value");
    }

    SECTION("A failed mirror write rejects the update") {
        // A user-config path whose parent hierarchy cannot be created makes the mirror write fail.
        const auto unwritable_path = fs::path("/proc/everest-mirror-test/user-config/config.yaml");
        ConfigServiceCore config_service(parse_settings, db,
                                         std::make_unique<everest::config::UserConfigStorage>(unwritable_path));

        auto lfy_result = config_service.load_from_yaml(valid_yaml, "Mirror failure test", std::nullopt);
        INFO(lfy_result.error_message);
        REQUIRE(lfy_result.success == true);
        config_service.mark_active_slot(0);
        config_service.reinitialize_from_db(true);
        ...
        CHECK(result.parameter_results->front().status_info == "Failed to persist change to user-config");
    }
}
