...
// TARGET_MIGRATION_FILE_VERSION is a compile definition derived by CMake (collect_migration_files)
// from the migration files in schemas/migrations, so these tests automatically cover new
// migrations as they are added.
...
TEST_CASE("Framework migration files - step by step", "[framework_migration]") {
    ...
    for (uint32_t i = 1; i <= TARGET_MIGRATION_FILE_VERSION; i++) {
        REQUIRE(updater.apply_migration_files(mdir, i));
        REQUIRE(db.get_user_version() == i);
    }

    for (uint32_t i = TARGET_MIGRATION_FILE_VERSION; i > 0; i--) {
        REQUIRE(updater.apply_migration_files(mdir, i));
        REQUIRE(db.get_user_version() == i);
    }
}
...
TEST_CASE("Framework migration files - at once", "[framework_migration]") {
    ...
    REQUIRE(updater.apply_migration_files(mdir, TARGET_MIGRATION_FILE_VERSION));
    REQUIRE(db.get_user_version() == TARGET_MIGRATION_FILE_VERSION);
