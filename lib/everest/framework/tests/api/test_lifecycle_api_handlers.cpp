...
using ActiveSlotUpdateCause = Everest::config::ActiveSlotUpdateCause;
...
// A repeated status is a real event, not noise: the manager reports a second failed start as
// FailedToStart again and a second restart request as RestartTriggered again. Suppressing those
// would leave a client that was just told "Starting"/"Restarting" with no update at all, unable to
// tell that its request failed - so the handler must forward them.
TEST_CASE("LifecycleAPI status callback republishes repeated module statuses", "[lifecycle_api]") {
    MockMQTTAbstraction mock("everest/");
    FakeConfigService svc;
    LifecycleAPI api(mock, svc, ConfigurationApiStatus::AvailableRW, /*readonly=*/false);
    REQUIRE(svc.active_slot_handler);

    // The two statuses the manager can legitimately emit twice in a row: a repeated failed start
    // (notice_cfg_validation_failed) and a repeated restart request (notice_module_restart_triggered).
    const auto status = GENERATE(std::make_pair(ActiveSlotStatus::FailedToStart, std::string("FailedToStart")),
                                 std::make_pair(ActiveSlotStatus::RestartTriggered, std::string("RestartTriggered")));
    mock.clear_published();

    svc.active_slot_handler(make_slot_update(status.first));
    REQUIRE(mock.published().size() == 1);

    // Same status again -> published again, so the second attempt's outcome reaches the client.
    svc.active_slot_handler(make_slot_update(status.first));
    REQUIRE(mock.published().size() == 2);

    const auto& record = mock.publish_records().back();
    CHECK(record.topic == STATUS_TOPIC);
    CHECK(json::parse(record.payload.get<std::string>()).at("module_status") == status.second);
}

// The counterpart: slot-info events (a new active/next-boot slot) repeat the current module status
// unchanged and carry nothing this topic reports, so they must not reach the lifecycle status topic.
// ConfigurationAPI is the consumer that wants them.
TEST_CASE("LifecycleAPI status callback ignores slot-info updates", "[lifecycle_api]") {
    MockMQTTAbstraction mock("everest/");
    FakeConfigService svc;
    LifecycleAPI api(mock, svc, ConfigurationApiStatus::AvailableRW, /*readonly=*/false);
    REQUIRE(svc.active_slot_handler);
    ...
    // Neither a slot-info event repeating the current status...
    svc.active_slot_handler(make_slot_update(ActiveSlotStatus::Running, ActiveSlotUpdateCause::SlotInfo));
    CHECK(mock.published().empty());

    // ...nor one carrying a different status (mark_active_slot does not guard on the module state)
    // says anything about the module lifecycle.
    svc.active_slot_handler(make_slot_update(ActiveSlotStatus::Stopped, ActiveSlotUpdateCause::SlotInfo));
    CHECK(mock.published().empty());
}
...
TEST_CASE("LifecycleAPI not-running status: last will and shutdown publish", "[lifecycle_api]") {
    ...
    // an unclean death - which is what lets the manager fall back to the will when it dies without
    // reaching publish_final_status_and_disconnect() (a crash, SIGKILL, keepalive timeout)
