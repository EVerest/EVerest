...
#ifndef UTILS_MQTT_FRAMEWORK_TRANSPORT_HPP
#define UTILS_MQTT_FRAMEWORK_TRANSPORT_HPP
...
/// \brief Framework-local transport backed by MQTT-C.
class MqttFrameworkTransport : public FrameworkTransport {
public:
    MqttFrameworkTransport(const MQTTSettings& mqtt_settings);

    ~MqttFrameworkTransport() override;

    MqttFrameworkTransport(MqttFrameworkTransport const&) = delete;
    void operator=(MqttFrameworkTransport const&) = delete;
...
public:
    ...
    std::size_t drain_transport_queue_for_test() {
        return this->message_queue.drain([this](const Message& message) { this->on_mqtt_message(message); });
    }
...
#endif // UTILS_MQTT_FRAMEWORK_TRANSPORT_HPP
