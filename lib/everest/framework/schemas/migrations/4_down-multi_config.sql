...
-- REQUIREMENT_INDEX has no equivalent in the old schema and is dropped here.
INSERT OR REPLACE INTO MODULE_FULFILLMENT_NEW
    SELECT MODULE_ID, REQUIREMENT_NAME, IMPLEMENTATION_ID, IMPLEMENTATION_MODULE_ID
    FROM MODULE_FULFILLMENT
    WHERE CONFIG_ID = (SELECT MIN(ID) FROM CONFIG)
    ORDER BY MODULE_ID, REQUIREMENT_NAME, REQUIREMENT_INDEX;
...
-- 2.5. Drop BOOT_CONFIG table added in migration 4.
