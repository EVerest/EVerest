...
INSERT INTO MODULE_CONFIG_ACCESS_NEW
    ...
    WHERE CONFIG_ID = (SELECT MIN(ID) FROM SETTING);
...
INSERT INTO CONFIG_META_NEW
    ...
    FROM CONFIG_META
    WHERE ID = (SELECT MIN(ID) FROM SETTING);
DROP TABLE CONFIG_META;
ALTER TABLE CONFIG_META_NEW RENAME TO CONFIG_META;

-- 1. SETTING: restore id=0 constraint, keep only the lowest-ID row mapped to ID=0.
