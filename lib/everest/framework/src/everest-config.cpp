...
static int cmd_delete(const std::string& prefix, const std::string& db_path, int slot_id) {
    ...
    // The database rejects this anyway, but a plain "failed" would not tell the user why.
    if (slot_id == slot_mgr.get_next_boot_slot_id()) {
        fmt::print(stderr, "Error: Slot {} is selected for the next boot and cannot be deleted.\n", slot_id);
        return 1;
    }
