...
static int cmd_add(const std::string& prefix, const std::string& config_file, const std::string& db_path) {
    ...
    // Mirror the bootstrap_from_database_init "else" branch:
    //   1. Set up ManagerSettings from YAML (resolves all paths from prefix + YAML settings section)
    //   2. Auto-assign a slot ID and write CONFIG + FRAMEWORK_SETTINGS via the slot manager
    //   3. Construct ManagerConfig scoped to that slot — validates YAML, writes module data, marks valid
...
static int cmd_validate(const std::string& prefix, const std::string& config_file) {
    ...
    // Same pattern as manager.cpp's check_config path.
    // ManagerSettings(prefix, config) sets boot_mode=YamlFile, resolves all paths,
    // and parses the YAML settings section. ManagerConfig validates on construction.
    ManagerConfig cfg(ManagerSettings(prefix, config_file));
...
int main(int argc, char* argv[]) {
    // Suppress all framework EVLOG_* output — this tool reports its own status via fmt::print.
