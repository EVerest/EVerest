...
void LifecycleAPI::generate_api_var_status() {
    ...
    m_config_service.register_active_slot_update_handler([this](const Everest::config::ActiveSlotUpdate& update) {
        // Slot-info events repeat the current module status unchanged and carry nothing this topic
        // reports, so they are dropped. Everything that gets past here is published unconditionally -
        // including a repeat of the status already published, which is exactly how a second failed
        // start (FailedToStart again) or a second restart request (RestartTriggered again) reaches a
        // client that was just told "Starting"/"Restarting". Deduplicating by value would swallow
        // those, leaving the client unable to tell that its request failed.
        if (update.cause != ::Everest::config::ActiveSlotUpdateCause::ModuleStatus) {
            return;
        }
