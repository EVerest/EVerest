    ...
    subscribe_api_topic("delete_slot", [this](std::string const& data) {
        ...
        if (deserialize(data, msg)) {
            ...
            if (deserialize(msg.payload, payload)) {
                ...
                API_types_ext::DeleteSlotResult response{ext_res};  //TODO(CB): We might want to add the slot_id to the response as well, for better logging on the client side?
                mqtt_abstraction.publish(msg.replyTo, serialize(response));
                return true;
            }
        }
    ...
    subscribe_api_topic("load_from_yaml", [this](std::string const& data) {
        ...
        if (deserialize(data, msg)) {
            ...
            if (deserialize(msg.payload, payload)) {
                ...
                auto ext_res = API_wrapper::to_external_api(res);  //TODO(CB): This ignores the description for now - maybe we want to add that to the config_service interface as well?
    ...
    subscribe_api_topic("get_configuration", [this](std::string const& data) {
        ...
        if (deserialize(data, msg)) {
            ...
            if (deserialize(msg.payload, payload)) {
                ...
                auto ext_res = API_wrapper::to_external_api(res); // TODO(CB): This doesn't include capabilities (may not be required) and telemetry and access (config), because the external type is lacking these
...
void ConfigServiceAPI::generate_api_var_active_slot() {
    ...
    initial_update.status =
        API_types_ext::ActiveSlotStatusEnum::Running;
...
void ConfigServiceAPI::subscribe_api_topic(std::string const& var, ParseAndPublishFtor const& parse_and_publish) {
    ...
    auto handler =
        std::make_shared<TypedHandler>(HandlerType::ExternalMQTT, std::make_shared<Handler>([=](std::string const& topic, nlohmann::json data) {
            try {
