...
void Everest::subscribe_global_all_errors(const error::ErrorCallback& raise_callback,
    ...
    for (const auto& [module_id, module_name] : this->config.get_module_names()) {
        ...
        for (const auto& impl : provides.items()) {
            ...
            const auto& interface_definition = this->config.get_interface_definitions(
                this->config.get_interfaces().at(module_name).at(impl_id)
            );
            for (const auto& error_namespace_it : interface_definition.at("errors").items()) {
                ...
                for (const auto& error_name_it : error_namespace_it.value().items()) {
                    ...
                    const std::string error_topic = fmt::format(
                        "{}/error/{}",
                        this->config.mqtt_prefix(module_id, impl_id),
                        error_type
                    );
                    const std::shared_ptr<TypedHandler> error_token =
                        std::make_shared<TypedHandler>(
                            HandlerType::SubscribeError,
                            std::make_shared<Handler>(error_handler)
                        );
                    this->mqtt_abstraction->register_handler(
                        error_topic,
                        error_token,
                        QOS::QOS2
                    );
