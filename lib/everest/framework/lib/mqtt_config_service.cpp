...
void ConfigServiceClient::register_config_change_handler(const std::string_view name, ConfigChangeHandler handler) {
    if (change_callbacks.empty()) {
        ...
        const auto handler = [this](const std::string& /*topic*/, const nlohmann::json& data) {
            mqtt_set_request(data);
        };

        change_token =
            std::make_shared<TypedHandler>(HandlerType::RuntimeConfigSet, std::make_shared<Handler>(handler));
...
void ConfigServiceClient::mqtt_set_request(const nlohmann::json& data) {
    ...
    MqttMessagePayload payload{MqttMessageType::SetConfigResponse, response};
