...
ConfigServiceCore::ConfigServiceCore(const ConfigParseSettings& parse_settings,
    ...
    m_persistence_mirror(std::move(persistence_mirror)) {
...
void ConfigServiceCore::internal_reinitialize_from_db(bool force_reload) {
    if (not modules_are_down(m_module_status)) {
        // Re-pointing the active slot underneath live or half-started modules would desynchronize
        // the running processes from the in-memory configuration.
        EVLOG_warning << "Skipping configuration reload from the database: the modules are not at rest (status: "
                      << to_string(m_module_status) << "). The in-memory configuration is unchanged.";
        return;
    }
    m_next_boot_slot_id = m_slot_manager.get_next_boot_slot_id();
...
DeleteSlotStatus ConfigServiceCore::internal_delete_slot(int slot_id) {
    if (slot_id == m_active_slot_id and m_active_slot_id != m_next_boot_slot_id and modules_are_down(m_module_status)) {
...
LoadFromYamlResult ConfigServiceCore::internal_load_from_yaml(const std::string& raw_yaml,
    ...
    if (target_slot_id == m_active_slot_id and not modules_are_down(m_module_status)) {
        const char* err_msg = "Cannot load YAML into the active slot while the modules are running or mid-transition";
        EVLOG_warning << "Failed to load from YAML into slot " << target_slot_id << ": " << err_msg
                      << " (status: " << to_string(m_module_status) << ")";
...
ConfigServiceCore::internal_set_config_parameters(int slot_id, const std::vector<ConfigParameterUpdate>& updates,
    ...
    // resolved here and now (persisted, WillApplyOnRestart) instead of being deferred to a settling
    // that would never come.
    const bool modules_in_transient = modules_in_transient_state(m_module_status);
...
void ConfigServiceCore::apply_active_slot_updates(const std::vector<ConfigParameterUpdate>& updates,
    ...
    for (size_t i = 0; i < updates.size(); ++i) {
        ...
        // When a persistence mirror is configured (in-memory database deployments), the mirror is
        // the only persistence that survives a restart, so it is written first: a failed mirror write
        // rejects the update before it leaves any trace in the slot storage.
        if (m_persistence_mirror != nullptr) {
            auto mirror_status = ec::GetSetResponseStatus::Failed;
            try {
                mirror_status = m_persistence_mirror->write_configuration_parameter(
                    update.identifier, in_memory_parameter->characteristics, update.value);
            } catch (const std::exception& e) {
                EVLOG_error << "ConfigServiceCore: Persisting the configuration parameter change to the "
                               "user-config failed: "
                            << e.what();
            }
            if (mirror_status != ec::GetSetResponseStatus::OK) {
                result_enum = SetConfigParameterResultEnum::Rejected;
                per_result.status_info = "Failed to persist change to user-config";
                continue;
            }
        }
...
void ConfigServiceCore::set_modules_at_rest() {
    post_to_actor([this]() { internal_set_modules_at_rest(); });
}
void ConfigServiceCore::internal_set_modules_at_rest() {
    if (modules_are_down(m_module_status)) {
        // Already resting: keep a recorded FailedToStart and do not emit a duplicate event.
        return;
    }
    internal_set_modules_stopped();
}
