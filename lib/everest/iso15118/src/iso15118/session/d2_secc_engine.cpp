...
d2::SessionConfig make_d2_config(const session::SessionConfig& config, bool tls_active) {
    ...
    // limits) takes precedence over the hardware-capability-derived value; mid-session changes arrive
    // as UpdateAcMaxCurrent control events (see on_control_event).
    if (config.iso2_ac_max_current.has_value()) {
        out.ac_max_current = config.iso2_ac_max_current.value();
    }
...
void D2SeccEngine::on_control_event(const d20::ControlEvent& event) {
    ...
    // AC ChargingStatusRes (and a not-yet-sent ChargeParameterDiscoveryRes) announces the new
    // EVSEMaxCurrent and the EV throttles accordingly. Context-level, not a per-state event.
