...
SdpReadResult read_single_sdp_packet(io::IConnection& connection, io::SdpPacket& sdp_packet) {
    ...
    if (first_try.connection_closed) {
        return SdpReadResult::connection_closed;
    }
    ...
    if (first_try.would_block) {
        ...
        return SdpReadResult::would_block;
    }

    if (sdp_packet.get_state() == PacketState::COMPLETE) {
        // done
        return SdpReadResult::complete;
    }
...
TimePoint const& Session::poll() {
    ...
    if (state.new_data) {
        switch (read_single_sdp_packet(*connection, packet)) {
        case SdpReadResult::connection_closed:
            logf_info("Peer closed the connection");
            close();
            return next_session_event;
        case SdpReadResult::would_block:
            state.new_data = false;
            break;
        case SdpReadResult::complete:
            break;
        }
    }
