...
SessionConfig::SessionConfig(EvseSetupConfig config) :
    ...
    iso2_ac_max_current(config.iso2_ac_max_current),
...
uint32_t auth_timeout_to_ms(uint32_t timeout_s) {
    static constexpr uint32_t MAX_TIMEOUT_S = std::numeric_limits<uint32_t>::max() / 1000;
    if (timeout_s > MAX_TIMEOUT_S) {
        logf_warning("Authorization timeout of %u s exceeds the representable maximum; clamping to %u s", timeout_s,
                     MAX_TIMEOUT_S);
        return MAX_TIMEOUT_S * 1000;
    }
