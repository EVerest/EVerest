...
create_request(const std::vector<ProtocolId>& supported_protocols,
    ...
    const bool any_dc = std::any_of(supported_energy_services.begin(), supported_energy_services.end(), is_dc_service);
    const bool any_ac = std::any_of(supported_energy_services.begin(), supported_energy_services.end(), is_ac_service);
