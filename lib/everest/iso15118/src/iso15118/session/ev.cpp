...
// mirror of the SECC-side tri-state result (iso.cpp)
...
// mirror of the SECC-side read_single_v2gtp_packet (iso.cpp)
V2GTPReadResult read_packet(io::IConnection& connection, io::SdpPacket& sdp_packet) {
    using PacketState = io::SdpPacket::State;
...
TimePoint const& EvSession::poll() {
    ...
    // Publish v2g_session_finished exactly once per session, on any termination path -- including
    // the ones where the connection is already gone and the early return below would skip the
    // end-of-poll handling.
    signal_session_finished_if_done();

    // Check timeouts BEFORE the not-connected early return: the communication-setup guard armed in the
    // constructor must also fire while the TCP/TLS handshake is still in flight (state.connected is
    // false throughout it) -- that is exactly the stalled-handshake case it exists for.
    {
        ...
        if (timeouts_reached.has_value()) {
            for (const auto& timeout : timeouts_reached.value()) {
                if (engine) {
                    ...
                    logf_error("Communication setup / SupportedAppProtocol timeout reached, terminating session");
                    driver_stopped = true;
                }
    ...
    if (state.new_data) {
        switch (read_packet(*connection, packet)) {
        case V2GTPReadResult::connection_closed:
            // TCP EOF / TLS close_notify: the SECC closed the connection. After SessionStopRes this
            // is the regular end of a session (DIN [V2G-DC-937], ISO 15118-2 [V2G2-025]); otherwise
            // it is a mid-session abort (e.g. a SECC-side sequence timeout). Close our side and
            // terminate the session instead of running into a message timeout.
            logf_info("TCP connection closed by the peer");
            connection->close();
            if (not is_finished()) {
                logf_warning("The peer closed the connection before the V2G session ended, terminating session");
                driver_stopped = true;
            }
            signal_session_finished_if_done();
            break;
    ...
    // deliver queued control events to the engine. Events queued before the engine exists (during
    // the TCP/TLS handshake and SupportedAppProtocol) are KEPT queued and delivered to the engine
    // right after its creation -- dropping them would silently lose a StopCharging/PauseCharging
    // requested while the session was still being set up.
    if (engine) {
    ...
    // send a pending request, paced to at least MIN_REQUEST_INTERVAL_MS apart (never on a
    // connection that is already gone)
    const bool has_outgoing =
        state.connected and (pending_sap_outgoing.has_value() or (engine and engine->has_outgoing()));
    if (has_outgoing) {
...
void EvSession::terminate() {
    if (driver_stopped and state.teardown_signaled) {
        return;
    }
    logf_warning("Control pilot error / plug-out: terminating the V2G session immediately");
    driver_stopped = true;
    signal_session_finished_if_done();
}

void EvSession::signal_session_finished_if_done() {
    if (not is_finished() or state.teardown_signaled) {
        return;
    }
    state.teardown_signaled = true;
    // Which of the two termination paths ended the session. Without this the
    // teardown is indistinguishable in the log from a normal SessionStop, and a
    // driver-side stop that fires early looks exactly like a completed session.
    logf_info("V2G session finished (driver_stopped=%d, engine_finished=%d)", static_cast<int>(driver_stopped),
        static_cast<int>(engine and engine->is_finished()));
    if (state.connected) {
        // The SECC keeps the TCP connection open ~5 s after SessionStopRes [V2G20-1643]; closing
        // from our side after receiving the response is correct.
        connection->close();
    }
...
void EvSession::start_supported_app_protocol() {
    // The transport is up (OPEN): the communication-setup guard from the constructor has done its job;
    // the per-message SEQUENCE timeout takes over from here.
    timeouts.stop_timeout(d20::TimeoutType::PERFORMANCE);
    ...
    if (req.app_protocol.empty()) {
        logf_error("No application protocols to offer (empty SupportedAppProtocolReq); check that the configured "
...
void EvSession::handle_supported_app_protocol_response(io::v2gtp::PayloadType payload_type,
    ...
    if (res == nullptr) {
        logf_warning("Expected SupportedAppProtocolRes! But code type id: %d", variant.get_type());
        driver_stopped = true;
        return;
    }
    ...
    if (not result.negotiated or not result.schema_id.has_value()) {
        logf_error("SupportedAppProtocol negotiation failed, terminating session");
        driver_stopped = true;
        return;
    }

    const auto offered =
        std::find_if(offered_protocols.begin(), offered_protocols.end(),
                     [&](const OfferedProtocol& o) { return o.schema_id == result.schema_id.value(); });
    if (offered == offered_protocols.end()) {
        logf_error("SupportedAppProtocol negotiated an unknown schema, terminating session");
        driver_stopped = true;
        return;
    }
...
std::unique_ptr<EvEngine> EvSession::create_engine(ProtocolId protocol_id) {
    ...
    case ProtocolId::ISO15118_20: {
        ...
        const bool is_dc = std::any_of(services.begin(), services.end(), [](dt::ServiceCategory s) {
            return s == dt::ServiceCategory::DC or s == dt::ServiceCategory::DC_BPT or s == dt::ServiceCategory::MCS or
                   s == dt::ServiceCategory::MCS_BPT;
        });
        feedback.selected_protocol(is_dc ? "ISO15118-20:DC" : "ISO15118-20:AC");

        const io::StreamOutputView view{request_buffer + io::SdpPacket::V2GTP_HEADER_SIZE,
                                        sizeof(request_buffer) - io::SdpPacket::V2GTP_HEADER_SIZE};
        return std::make_unique<D20EvEngine>(view, config, callbacks, timeouts);
    }
    case ProtocolId::ISO15118_2: {
        ...
        const io::StreamOutputView view{request_buffer + io::SdpPacket::V2GTP_HEADER_SIZE,
                                        sizeof(request_buffer) - io::SdpPacket::V2GTP_HEADER_SIZE};
        return std::make_unique<D2EvEngine>(view, std::move(d2_config), callbacks, timeouts);
    }
    case ProtocolId::DIN70121: {
        ...
        // message_2 and message_din share one EnergyTransferMode type (iso15118::datatypes).
        // [V2G-DC-625]: in DIN the EVCC shall transmit only "DC_core" or "DC_extended" in
        // EVRequestedEnergyTransferType. Map DC_core through; treat every other configured value
        // (AC modes, DC_combo_core, DC_unique) as the more capable DC_extended.
        din_config.requested_energy_transfer_type =
            (config.iso2_energy_transfer_mode == message_2::datatypes::EnergyTransferMode::DC_core)
                ? message_2::datatypes::EnergyTransferMode::DC_core
        ...
        const io::StreamOutputView view{request_buffer + io::SdpPacket::V2GTP_HEADER_SIZE,
                                        sizeof(request_buffer) - io::SdpPacket::V2GTP_HEADER_SIZE};
        return std::make_unique<DinEvEngine>(view, std::move(din_config), callbacks, timeouts);
    }
    }
    return nullptr;
}
...
void EvSession::handle_connection_event(io::ConnectionEvent event) {
    ...
    case Event::ACCEPTED:
        ...
        logf_info("Connection accepted to SECC");
        return;
    ...
    case Event::CLOSED:
        ...
        if (not is_finished()) {
            // The transport went away before the session ended -- e.g. a failed TLS handshake before
            // the session ever started. Terminate so the controller reaps this session instead of
            // idling forever with no transport (charging_active would otherwise stay latched).
            driver_stopped = true;
        }
        return;
    }
...
void EvSession::close() {
    driver_stopped = true;
    // Closes the connection (if still up) and publishes v2g_session_finished exactly once.
    signal_session_finished_if_done();
}
