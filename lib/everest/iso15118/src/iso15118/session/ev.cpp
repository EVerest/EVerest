...
TimePoint const& EvSession::poll() {
        ...
        if (timeouts_reached.has_value()) {
            for (const auto& timeout : timeouts_reached.value()) {
                if (engine) {
                    ...
                    log("Communication setup / SupportedAppProtocol timeout reached, terminating session");
                    driver_stopped = true;
                }
    ...
    if (state.new_data) {
        ...
        case V2GTPReadResult::connection_closed:
            ...
            connection->close();
            if (not is_finished()) {
                log("The peer closed the connection before the V2G session ended, terminating session");
                driver_stopped = true;
            }
            signal_session_finished_if_done();
            break;
