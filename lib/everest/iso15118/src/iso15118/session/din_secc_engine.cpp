...
// [V2G-DC-620/621]: two DIN SPEC 91286 nibbles pack into one byte ('0'-'9' -> 0x0-0x9, '*' -> 0xA,
// odd trailing nibble padded with 0xF). An empty id, or one outside that alphabet, is sent as the
// single zero byte 0x00 [V2G-DC-876], which the EVCC then ignores [V2G-DC-877].
...
// message_20 physical quantities are RationalNumbers; DIN uses plain doubles.
din::SessionConfig make_din_config(const session::SessionConfig& config) {
    ...
    // neither configured the DC_extended default stands, so an EV asking for DC_core against e.g. a
    // DC_combo_core-only config is still rejected per [V2G-DC-397].
...
void DinSeccEngine::on_control_event(const d20::ControlEvent& event) {
    ...
    if (const auto* stop = std::get_if<d20::StopCharging>(&event)) {
        ...
        if (not requested) {
            ctx.set_charger_stop_ignored(false);
        }
        return;
    }

    // Energy management pushes these for the whole session: the next CurrentDemandRes announces the new
    // EVSEMaximum*, while the ChargeParameterDiscoveryRes offer stays at the hardware capability.
    if (const auto* limits = std::get_if<d20::DcTransferLimits>(&event)) {
        apply_dc_limits(ctx.session_config, *limits);
        return;
    }

    // These feed the ChargeParameterDiscoveryRes offer, so apply them in case it is still to be sent.
    ...
    // DIN SPEC 70121 has no SECC-initiated pause -- the SECC can only ask the EV to stop. Say so rather
    ...
    if (const auto* err = std::get_if<d20::EvseError>(&event)) {
        ...
        if (err->code == d20::EvseErrorCode::EmergencyShutdown and not ctx.evse().emergency_shutdown) {
            // [V2G-DC-866]: answer FAILED and terminate with it rather than dropping the TCP connection, which
            // would leave the EV with a transport error and no reason. The physical shutdown does not wait on
            // any of this -- it runs over the control pilot.
    ...
    if (const auto* cp = std::get_if<d20::CpStateChanged>(&event)) {
        ...
        // [V2G-DC-962] CP State A (unplug) ends the session: close without the EV-first linger, even during
        // a normal end's linger -- a lingering DLINK_TERMINATE would otherwise fire seconds later, into the
        ...
        if (cp->state == d20::CpState::A) {
            ...
            // [V2G-DC-668] CP State B during the DC charging phase, where the EV must be in C/D, is a fault:
            // emergency shutdown and terminate without delay.
...
void DinSeccEngine::on_timeout(d20::TimeoutType timeout) {
    ...
    if (timeout == d20::TimeoutType::STOP_CHARGING) {
        ...
        if (not ctx.session_stopped) {
            ...
            ctx.set_charger_stop_ignored(true);
        }
        return;
    }
...
bool DinSeccEngine::is_finished() const {
    return ctx.session_stopped and not message_exchange.has_response();
}

bool DinSeccEngine::is_paused() const {
    // DIN signals a pause only through a later re-join, never through a SessionStopRes, so a DIN SECC
    // session is never paused -- the SeccEngine interface still requires the query.
    return false;
}
