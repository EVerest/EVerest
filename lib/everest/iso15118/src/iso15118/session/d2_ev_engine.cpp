...
D2EvEngine::D2EvEngine(io::StreamOutputView output_view, d2::ev::EvSessionConfig config,
    ...
    ctx(std::move(callbacks), std::move(config), active_control_event, message_exchange, timeouts),
...
void D2EvEngine::on_packet(io::v2gtp::PayloadType payload_type, const io::StreamInputView& view) {
    // All ISO 15118-2 messages share the single SAP payload type (0x8001); a frame carrying any
    // other V2GTP payload type is ignored, exactly as on the SECC side. [V2G2-086] has the receiver
    // ignore a message whose payload type it does not support, and ISO 15118-4 checks it
    // (TC_EVCC_CMN_VTB_V2GTPSessionSetup_003: a SessionSetupRes tagged 0x7001 must not be acted on,
    // and the EVCC's own message timeout then ends the session).
