...
D20SeccEngine::D20SeccEngine(io::StreamOutputView output_view, session::SessionConfig config,
    ...
    // false: the SupportedAppProtocol negotiation did run (in the SAP driver, before this engine
    // existed), so SessionSetup must emit the ev_information feedback carrying its result.
    fsm(ctx.create_state<d20::state::SessionSetup>(false)) {
