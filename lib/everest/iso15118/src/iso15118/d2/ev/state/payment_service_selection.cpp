...
d2::ev::Result PaymentServiceSelection::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("PaymentServiceSelection message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_2::PaymentServiceSelectionResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("PaymentServiceSelection failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected PaymentServiceSelectionRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
