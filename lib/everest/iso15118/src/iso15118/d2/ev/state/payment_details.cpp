...
d2::ev::Result PaymentDetails::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("PaymentDetails message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_2::PaymentDetailsResponse>()) {
        if (res->response_code >= dt::ResponseCode::FAILED) {
            logf_warning("PaymentDetails failed (response code %d), terminating session", res->response_code);
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected PaymentDetailsRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
