...
message_2::ChargeParameterDiscoveryRequest create_dc_request(const EvSessionConfig& config,
    ...
    // [V2G2-310/610] Advertise how many PMaxScheduleEntry/SalesTariff entries the EVCC can process per
    // tuple (the SAScheduleList pmax_schedule fixed_vector capacity).
    req.max_entries_sa_schedule_tuple = 12;
...
message_2::ChargeParameterDiscoveryRequest create_ac_request(const EvSessionConfig& config) {
    message_2::ChargeParameterDiscoveryRequest req;
    req.requested_energy_transfer_mode = config.requested_energy_transfer_mode;
    req.max_entries_sa_schedule_tuple = 12; // [V2G2-310/610]
...
Result handle_response(const message_2::ChargeParameterDiscoveryResponse& res) {
    ...
    if (res.sa_schedule_list.has_value() and not res.sa_schedule_list->empty()) {
        ...
        // [V2G2-286]/[V2G2-773]: the EV must reference a SAScheduleTupleID (1..255) from the CPD
        // response in the following PowerDeliveryReq. A Finished response without a usable
        // SAScheduleList leaves no legal way to continue -- fail instead of sending tuple ID 0.
        result.valid = false;
    }
...
d2::ev::Result ChargeParameterDiscovery::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            logf_warning("ChargeParameterDiscovery ongoing timeout reached, terminating session");
            ongoing_timeout_reached = true;
        } else {
            logf_warning("ChargeParameterDiscovery message timeout reached, terminating session");
        }
    ...
    if (const auto res = variant->get_if<message_2::ChargeParameterDiscoveryResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("ChargeParameterDiscovery failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected ChargeParameterDiscoveryRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
