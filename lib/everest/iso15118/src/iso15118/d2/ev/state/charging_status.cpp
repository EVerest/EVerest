...
Result handle_response(const message_2::ChargingStatusResponse& res) {
    ...
    result.renegotiation_or_receipt = (res.ac_evse_status.notification == dt::EVSENotification::ReNegotiation) or
...
d2::ev::Result ChargingStatus::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("ChargingStatus message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_2::ChargingStatusResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("ChargingStatus failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
        ...
        if (result.notification.has_value() and result.notification.value() == dt::EVSENotification::ReNegotiation) {
            logf_warning("ChargingStatus received ReNegotiation; renegotiating via PowerDelivery(Renegotiate)");
        ...
        if (result.renegotiation_or_receipt) {
            logf_warning("ChargingStatus received ReceiptRequired (unsupported under EIM), "
    ...
    logf_warning("Expected ChargingStatusRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
