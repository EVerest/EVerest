...
void Authorization::send(Event) {
    ...
    if (m_ctx.pnc.contract_selected) {
        ...
        if (signed_exi.empty()) {
            logf_warning("Authorization: failed to sign the PnC AuthorizationReq, terminating session");
...
d2::ev::Result Authorization::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            logf_warning("Authorization ongoing timeout reached, terminating session");
            ongoing_timeout_reached = true;
        } else {
            logf_warning("Authorization message timeout reached, terminating session");
        }
        m_ctx.session_stopped = true;
        return {};
    }
    ...
    if (const auto res = variant->get_if<message_2::AuthorizationResponse>()) {
        ...
        case Action::Failed:
            logf_warning("Authorization failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        case Action::Done:
    ...
    logf_warning("Expected AuthorizationRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
