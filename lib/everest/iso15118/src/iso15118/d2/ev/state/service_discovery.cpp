...
d2::ev::Result ServiceDiscovery::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("ServiceDiscovery message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_2::ServiceDiscoveryResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("ServiceDiscovery failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
        ...
        if (not result.eim_offered and not want_contract) {
            logf_warning("SECC offers no usable payment option (EIM absent, Contract unavailable/unwanted), "
                      "terminating session");
        ...
        if (not result.mode_supported) {
            ...
            logf_warning("SECC does not offer the requested energy transfer mode; continuing, expecting the SECC "
    ...
    logf_warning("Expected ServiceDiscoveryRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
