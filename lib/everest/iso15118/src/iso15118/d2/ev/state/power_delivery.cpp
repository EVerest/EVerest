...
d2::ev::Result PowerDelivery::feed(Event ev) {
    if (ev == Event::SEND_REQUEST) {
        ...
        message_2::PowerDeliveryRequest req;
        if (is_dc) {
            // A pause keeps the session resumable, so charging is not marked complete.
            const bool charging_complete =
                (m_phase == Phase::Stop) and (m_ctx.pending_stop != dt::ChargingSession::Pause);
