...
d2::ev::Result PowerDelivery::feed(Event ev) {
    if (ev == Event::SEND_REQUEST) {
        ...
        // ChargingProfile accompanies PowerDelivery(Start) in both energy transfer modes: the EV
        // states the power it intends to draw over time, within the limits of the SAScheduleTuple it
        // selected ([V2G2-673]). It is left out of Stop/Renegotiate, where there is nothing to
        // announce. The fallback power (W) comes from the EV's own voltage/current limits, not from
        // the energy amount (Wh).
        std::optional<dt::ChargingProfile> profile{std::nullopt};
        if (m_phase == Phase::Start) {
            const float fallback_power_w =
                is_dc ? m_ctx.session_config.dc_ev_max_power.value_or(m_ctx.session_config.dc_ev_max_voltage *
                                                                      m_ctx.session_config.dc_ev_max_current)
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("PowerDelivery message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_2::PowerDeliveryResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("PowerDelivery failed (response code >= FAILED), terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected PowerDeliveryRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
