...
void CableCheck::send(Event) {
    if (first_request) {
        ...
        // [V2G2-847]: the EV shall apply CP state C or D before requesting the cable check. When the
        ...
        if (m_ctx.session_config.has_cp_state_feedback and not m_ctx.cp_state_c_or_d) {
            logf_warning("CableCheck: waiting for CP state C/D before sending the first CableCheckReq");
...
d2::ev::Result CableCheck::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            ...
            logf_warning("CableCheck message timeout reached, terminating session");
        }
    ...
    if (const auto res = variant->get_if<message_2::CableCheckResponse>()) {
        ...
        case Action::Failed:
            logf_warning("CableCheck failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        case Action::Done:
            m_ctx.stop_timeout(d20::TimeoutType::ONGOING);
            if (not result.evse_ready or not result.isolation_ok) {
                logf_warning("CableCheck finished but EVSE not ready or isolation not valid, terminating session");
                m_ctx.session_stopped = true;
                return {};
            }
    ...
    logf_warning("Expected CableCheckRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
