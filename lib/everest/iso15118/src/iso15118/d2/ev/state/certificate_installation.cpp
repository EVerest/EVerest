...
d2::ev::Result CertificateInstallation::feed(Event ev) {
    ...
    if (ev == Event::SEND_REQUEST) {
        ...
        if (signed_exi.empty()) {
            logf_warning("CertificateInstallation: failed to build/sign CertificateInstallationReq, terminating");
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("CertificateInstallation message timeout reached, terminating session");
        m_ctx.session_stopped = true;
        return {};
    }
    ...
    if (res == nullptr) {
        logf_warning("Expected CertificateInstallationRes! But code type id: %d", variant->get_type());
        m_ctx.session_stopped = true;
        return {};
    }
    if (res->response_code >= dt::ResponseCode::FAILED) {
        logf_warning("CertificateInstallation failed (response code %d), terminating session", res->response_code);
        m_ctx.session_stopped = true;
        return {};
    }
    ...
    if (not crypto::verify_certificate_installation_res(variant->get_exi_payload(), pnc.v2g_root_path)) {
        logf_warning("CertificateInstallation: response signature verification failed, terminating session");
        m_ctx.session_stopped = true;
        return {};
    }
    ...
    if (contract_key_pem.empty()) {
        logf_warning("CertificateInstallation: failed to decrypt the contract private key, terminating session");
        m_ctx.session_stopped = true;
        return {};
    }
