...
Context::Context(session::ev::feedback::Callbacks feedback_callbacks,
                 EvSessionConfig session_config_, const std::optional<ControlEvent>& current_control_event_,
                 d2::MessageExchange& message_exchange_, d20::Timeouts& timeouts_) :
...
std::unique_ptr<message_2::Variant> Context::pull_response() {
    return message_exchange.pull_request();
}

message_2::Type Context::peek_response_type() const {
