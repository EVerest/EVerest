...
message_2::CableCheckResponse handle_request([[maybe_unused]] const message_2::CableCheckRequest& req,
    ...
    // signal EVSENotification StopCharging here too (EvseV2G stamps its context notification into every
    ...
    // derived from the cable check's own progress: only the module can tell Warning, Fault and above all
    // No_IMD apart, and EvseManager reports it before signalling cable_check_finished in every path
    // (No_IMD + finished(true) when no IMD is fitted at all). Without it the progress derivation stands,
    // so an EVSE that never reports keeps the previous behaviour. EvseV2G likewise copies its module
    // value into CableCheckRes (iso_server.cpp handle_iso_cable_check).
...
Result CableCheck::process_request(const message_2::CableCheckRequest& req) {
    ...
    const auto res =
        handle_request(req, m_ctx.get_session_id(), m_ctx.cable_check_done, m_ctx.cable_check_fault,
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            ...
            auto res = handle_request(message_2::CableCheckRequest{}, m_ctx.get_session_id(),
                                      ...
                                      m_ctx.error_status_code(), m_ctx.charger_stop_requested);
            res.response_code = dt::ResponseCode::FAILED;
