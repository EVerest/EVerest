...
message_2::ChargingStatusResponse handle_request([[maybe_unused]] const message_2::ChargingStatusRequest& req,
    ...
    // [V2G2-902]: the EV signs the MeterInfo the SECC sent, so it must be present when a receipt is
    // requested. Include the latest meter reading forwarded by the module.
...
Result ChargingStatus::feed(Event ev) {
    if (ev == Event::CONTROL_MESSAGE) {
        if (const auto* meter = m_ctx.get_control_event<d20::MeterInfo>()) {
            dt::MeterInfo info{};
