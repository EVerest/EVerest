...
Result PaymentDetails::feed(Event ev) {
    ...
    if (req == nullptr) {
        logf_warning("Expected PaymentDetailsReq! But code type id: %d", variant->get_type());
    ...
    if (validation.response_code != dt::ResponseCode::OK) {
        ...
        // the PnC Authorize instead. Deliberate deviation from [V2G2-899] (the SECC shall validate the
        // chain and verify that it traces back to a trusted Mobility Operator root), covering the
        // OCPP 2.0.1 C07.FR.06 deployment in which the CPO installs no MO root and the CSMS validates
        // the contract instead (EvseV2G parity).
