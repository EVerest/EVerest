...
void SessionStop::process_request(const message_2::SessionStopRequest& req) {
    ...
    if (req.charging_session == dt::ChargingSession::Pause) {
        // Retain the session id so the returning EV can re-join with OK_OldSessionJoined, and the
        // selected payment option so the resumed session's ServiceDiscoveryRes can offer only that
        // option [V2G2-741].
        m_ctx.session_paused = true;
        const auto selected_option =
            m_ctx.contract_selected ? dt::PaymentOption::Contract : dt::PaymentOption::ExternalPayment;
...
Result SessionStop::feed(Event ev) {
    ...
    if (req == nullptr) {
        logf_warning("Expected SessionStopReq! But code type id: %d", variant->get_type());
