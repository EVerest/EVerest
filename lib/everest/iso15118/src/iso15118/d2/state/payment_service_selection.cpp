...
handle_request(const message_2::PaymentServiceSelectionRequest& req, const dt::SessionId& session_id,
    ...
    if (resumed_payment_option.has_value()) {
        // On a resumed session only the previously selected payment option was offered [V2G2-741];
        // selecting anything else is invalid [V2G2-465]. A matching selection is accepted regardless of
        // the current flags -- ServiceDiscovery offered exactly this option.
        ...
        // Accept only the payment options offered in ServiceDiscoveryRes; anything else is rejected
        // [V2G2-465]. EIM is not implicitly allowed: a Contract-only (PnC-only) SECC rejects
        // ExternalPayment (EvseV2G parity; the -4 ATS models single-option SECCs,
        // TC_SECC_CMN_VTB_PaymentServiceSelection_009 is the EIM-only mirror image).
...
void PaymentServiceSelection::enter() {
    logf_debug("Enter state: PaymentServiceSelection");
}
...
Result PaymentServiceSelection::feed(Event ev) {
    ...
    if (req == nullptr) {
        logf_warning("Expected PaymentServiceSelectionReq! But code type id: %d", variant->get_type());
    ...
    // supported, so its selection is validated under the matching flag. ExternalPayment is accepted
    // under the same gating ServiceDiscovery used to offer it (incl. the fallback when the gating left
    // no option).
    ...
    // Mirror the ServiceDiscovery resume restriction [V2G2-741]: only the option offered there is
    // accepted here (same fallback when a PnC pause was resumed without TLS).
    ...
    // Report the accepted payment option, the way EvseV2G does from its own PaymentServiceSelectionReq
