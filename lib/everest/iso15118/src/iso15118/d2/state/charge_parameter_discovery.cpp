...
Result ChargeParameterDiscovery::feed(Event ev) {
    ...
    if (const auto req = variant->get_if<msg::ChargeParameterDiscoveryRequest>()) {
        ...
        const int16_t max_current_a = (nominal_v > 0) ? static_cast<int16_t>(max_power / (nominal_v * phases)) : 0;
        ...
        std::visit(
            [&departure_duration](const auto& ev_param) {
        ...
        entry.p_max = (max_power <= 32767) ? make_phys(static_cast<int16_t>(max_power), 0, dt::UnitSymbol::W)
