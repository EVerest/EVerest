...
void MessageExchange::set_input(std::unique_ptr<message_20::Variant> new_input) {
    if (input.has_value()) {
...
template <> std::unique_ptr<message_20::Variant> MessageExchange::pull_input() {
    if (not input.has_value()) {
        throw std::runtime_error("Tried to access V2G message, but there is none");
    }

    if (not std::holds_alternative<std::unique_ptr<message_20::Variant>>(input.value())) {
        throw std::runtime_error("Tried to access V2G message, but the false Variant is there");
    }

    return std::move(std::get<std::unique_ptr<message_20::Variant>>(input.value()));
}
...
template <> message_20::Type MessageExchange::peek_input_type() const {
    if (not input.has_value()) {
    ...
    return std::get<std::unique_ptr<message_20::Variant>>(input.value())->get_type();
}
