...
void EvController::loop() {
    ...
    while (session or sdp_client or not shutdown_active.load()) {
        ...
        // Control pilot error / plug-out: end everything in flight immediately ([V2G2-025],
        // [V2G2-728]). An established session tears down without SessionStop; an attempt still in
        // SDP discovery is cancelled the same way a failed attempt would be.
        if (terminate_requested.exchange(false)) {
            if (session) {
                session->terminate();
            } else if (sdp_client) {
                logf_info("Control pilot error / plug-out during session setup; cancelling the "
                          "pending SDP discovery");
                poll_manager.unregister_fd(sdp_client->get_fd());
                sdp_client->close();
                sdp_client.reset();
                sdp_response.reset();
...
void EvController::set_cp_state(bool c_or_d) {
    cp_state_c_or_d.store(c_or_d);
    {
        std::lock_guard<std::mutex> lock(mutex);
        // Someone is reporting the pilot, so the CP-dependent checks are usable from the next
        // session on (arm_session snapshots this config).
        setup_config.has_cp_state_feedback = true;
    }
    // Also deliver it to a session that is already running.
    queue_event(d20::ev::CpState{c_or_d});
}

void EvController::terminate_session() {
...
void EvController::create_session(const io::Ipv6EndPoint& endpoint, bool secure) {
    ...
    // Seed the fresh session with the pilot state as it stands. The transitions that matter happen
    // before the session exists (the vehicle is at B from plug-in, and moves to C around the time
    // the charge loop starts), and control events queued while there was no session are dropped --
    // so without this the DC cable check would wait on a state it never hears about.
    if (armed_session_config.has_cp_state_feedback) {
        session->push_control_event(d20::ev::CpState{cp_state_c_or_d.load()});
    }
}
