...
template <> void convert(const datatypes::DerCurve& in, struct iso20_ac_der_iec_DERCurveType& out) {
    ...
    const size_t in_curve_data_points_len =
        (in.curve_data_points.size() <= iso20_ac_der_iec_DataTupleType_10_ARRAY_SIZE)
            ? in.curve_data_points.size()
            : iso20_ac_der_iec_DataTupleType_10_ARRAY_SIZE;

    for (size_t i = 0; i < in_curve_data_points_len; i++) {
        convert(in.curve_data_points[i], out.CurveDataPoints.CurveDataPoint.array[i]);
    }
    out.CurveDataPoints.CurveDataPoint.arrayLen = in_curve_data_points_len;
