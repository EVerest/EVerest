...
static void convert(const struct iso2_SAScheduleListType& in, datatypes::SAScheduleList& out) {
    ...
    for (uint16_t i = 0; i < in.SAScheduleTuple.arrayLen; i++) {
        ...
        // [V2G2-659] SalesTariff support is mandatory for the EVCC; decode it (structure only,
        // the nested ConsumptionCost detail is not modelled) instead of silently dropping it.
