...
void apply_ac_der_sae_offer_rules(std::vector<dt::ServiceCategory>& services,
    ...
    if (not setup_config.has_value()) {
        ...
        setup_config =
            make_inert_default_sae_setup_config(dt::from_RationalNumber(sae_limits->grid_limits.nominal_voltage));
    }

    // setup_config is filled in above when it was missing, so both inputs are present here.
    const auto violation = validate_sae_der_setup(setup_config.value(), sae_limits.value(), ac_limits);
