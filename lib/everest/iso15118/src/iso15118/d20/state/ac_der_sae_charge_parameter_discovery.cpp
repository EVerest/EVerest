...
handle_request(const message_20::DER_SAE_AC_ChargeParameterDiscoveryRequest& req, d20::Session& session,
    ...
    // the success path overwrites with the configured one.
    convert(res.transfer_mode.der_control_cpd_res, get_default_sae_der_control());
    ...
    // Validated before the session is touched and before the DER control set is built.
