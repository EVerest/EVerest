...
message_20::DER_SAE_AC_ChargeLoopResponse handle_request(const message_20::DER_SAE_AC_ChargeLoopRequest& req,
                                                         const d20::Session& session) {
    // bool stop, bool pause,
    ...
    //     // If the ev sends a false control mode other than the previous selected ones, then the charger should
    //     terminate
    //     // the session
    ...
    //     // TODO(SL): [V2G20-3318] Decrease notificationmaxdelay based on the reaming seconds if the ev did not
    //     perform a
    //     // pause
    // }
...
void AC_DER_SAE_ChargeLoop::enter() {
    m_ctx.log.enter_state("AC_DER_SAE_ChargeLoop");
...
Result AC_DER_SAE_ChargeLoop::feed(Event ev) {
    ...
    if ([[maybe_unused]] const auto* const req = variant->get_if<message_20::DER_SAE_AC_ChargeLoopRequest>()) {
        ...
        //     const auto& func_dso_cos_phi_setpoint =
        //     der_functions.at(iec::DERControlName::DSOCosPhiSetpointProvision); if
        //     (std::holds_alternative<iec::DSOCosPhiSetpoint>(func_dso_cos_phi_setpoint)) {
    ...
    m_ctx.log("Expected PowerDeliveryReq or DER_SAE_AC_ChargeLoopRequest! But code type id: %d", variant->get_type());
