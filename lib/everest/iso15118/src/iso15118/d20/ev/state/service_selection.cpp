...
d20::ev::Result ServiceSelection::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("ServiceSelection message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_20::ServiceSelectionResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("ServiceSelection failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected ServiceSelectionRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
