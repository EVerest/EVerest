...
void DC_CableCheck::send(Event) {
    if (first_request) {
        ...
        if (m_ctx.session_config.has_cp_state_feedback and not m_ctx.cp_state_c_or_d) {
            logf_warning("DC_CableCheck: waiting for CP state C/D before sending the first DC_CableCheckReq");
...
d20::ev::Result DC_CableCheck::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            logf_warning("DC_CableCheck ongoing timeout reached, terminating session");
            ongoing_timeout_reached = true;
        } else {
            logf_warning("DC_CableCheck message timeout reached, terminating session");
        }
    ...
    if (const auto res = variant->get_if<message_20::DC_CableCheckResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("DC_CableCheck failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected DC_CableCheckRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
