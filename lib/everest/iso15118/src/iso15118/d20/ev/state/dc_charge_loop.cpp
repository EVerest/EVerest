...
d20::ev::Result DC_ChargeLoop::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("DC_ChargeLoop message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_20::DC_ChargeLoopResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("DC_ChargeLoop failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected DC_ChargeLoopRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
