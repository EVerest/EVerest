...
RequestParams build_params(const Context& ctx) {
    ...
    p.target_current = cache.target_current.has_value() ? dt::from_float(cache.target_current.value()) : dc.target_current;
    p.target_voltage = cache.target_voltage.has_value() ? dt::from_float(cache.target_voltage.value()) : dc.target_voltage;
