...
d20::ev::Result DC_WeldingDetection::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            logf_warning("DC_WeldingDetection ongoing timeout reached, terminating session");
            ongoing_timeout_reached = true;
        } else {
            logf_warning("DC_WeldingDetection message timeout reached, terminating session");
        }
    ...
    if (const auto res = variant->get_if<message_20::DC_WeldingDetectionResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("DC_WeldingDetection failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected DC_WeldingDetectionRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
