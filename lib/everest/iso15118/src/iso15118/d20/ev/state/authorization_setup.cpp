...
d20::ev::Result AuthorizationSetup::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("AuthorizationSetup message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_20::AuthorizationSetupResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("AuthorizationSetup failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected AuthorizationSetupRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
