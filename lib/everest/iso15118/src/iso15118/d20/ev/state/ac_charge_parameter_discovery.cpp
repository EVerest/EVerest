...
d20::ev::Result AC_ChargeParameterDiscovery::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("AC_ChargeParameterDiscovery message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_20::AC_ChargeParameterDiscoveryResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("AC_ChargeParameterDiscovery failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected AC_ChargeParameterDiscoveryRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
