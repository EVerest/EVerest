...
d20::ev::Result ServiceDetail::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("ServiceDetail message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_20::ServiceDetailResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("ServiceDetail failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
        ...
        if (not result.control_mode_found) {
            logf_warning("ServiceDetail response has no usable control mode, stopping session");
    ...
    logf_warning("Expected ServiceDetailRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
