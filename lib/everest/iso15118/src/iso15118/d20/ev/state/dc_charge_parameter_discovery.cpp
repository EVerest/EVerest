...
d20::ev::Result DC_ChargeParameterDiscovery::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("DC_ChargeParameterDiscovery message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_20::DC_ChargeParameterDiscoveryResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("DC_ChargeParameterDiscovery failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected DC_ChargeParameterDiscoveryRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
