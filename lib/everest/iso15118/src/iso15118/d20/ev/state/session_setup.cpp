...
d20::ev::Result SessionSetup::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("SessionSetup message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_20::SessionSetupResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("SessionSetup failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected SessionSetupRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
