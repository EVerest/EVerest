...
void DC_PreCharge::send(Event) {
    ...
    auto req =
        create_request(processing, dt::from_float(m_ctx.dc_cache.present_voltage), dt::from_float(target_voltage()));
...
d20::ev::Result DC_PreCharge::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            logf_warning("DC_PreCharge ongoing timeout reached, terminating session");
            ongoing_timeout_reached = true;
        } else {
            logf_warning("DC_PreCharge message timeout reached, terminating session");
        }
    ...
    if (const auto res = variant->get_if<message_20::DC_PreChargeResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("DC_PreCharge failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected DC_PreChargeRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
