...
d20::ev::Result ScheduleExchange::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("ScheduleExchange message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_20::ScheduleExchangeResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("ScheduleExchange failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected ScheduleExchangeRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
