...
din::ev::Result ContractAuthentication::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            logf_warning("ContractAuthentication ongoing timeout reached, terminating session");
            ongoing_timeout_reached = true;
        } else {
            logf_warning("ContractAuthentication message timeout reached, terminating session");
        }
    ...
    if (const auto res = variant->get_if<message_din::ContractAuthenticationResponse>()) {
        ...
        case Action::Failed:
            logf_warning("ContractAuthentication failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        case Action::Done:
    ...
    logf_warning("Expected ContractAuthenticationRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
