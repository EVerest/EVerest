...
din::ev::Result ServicePaymentSelection::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("ServicePaymentSelection message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_din::ServicePaymentSelectionResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("ServicePaymentSelection failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected ServicePaymentSelectionRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
