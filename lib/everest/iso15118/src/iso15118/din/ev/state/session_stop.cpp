...
din::ev::Result SessionStop::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("SessionStop message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_din::SessionStopResponse>()) {
        ...
        if (m_ctx.pending_stop.value_or(ChargingSession::Terminate) == ChargingSession::Pause) {
            logf_warning("DIN does not support pause/resume; terminating the session instead [V2G-DC-241]");
        }
        m_ctx.session_stopped = true;
        return {};
    }

    logf_warning("Expected SessionStopRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
