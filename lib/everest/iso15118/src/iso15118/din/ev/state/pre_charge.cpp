...
din::ev::Result PreCharge::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            logf_warning("PreCharge ongoing timeout reached, terminating session");
            ongoing_timeout_reached = true;
        } else {
            logf_warning("PreCharge message timeout reached, terminating session");
        }
    ...
    if (const auto res = variant->get_if<message_din::PreChargeResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("PreCharge failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected PreChargeRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
