...
Result handle_response(const message_din::CurrentDemandResponse& res) {
    ...
    // [V2G-DC-637]: EVSEStatusCodes without explicit requirements are informational only and shall not
    // influence the EV charging process -- in particular EVSE_IsolationMonitoringActive, which real
...
RequestParams build_params(const Context& ctx) {
    ...
    p.max_current_limit =
        cache.max_charge_current.has_value() ? cache.max_charge_current.value() : dc.max_current_limit;
    p.max_power_limit =
        cache.max_charge_power.has_value() ? std::optional<double>(cache.max_charge_power.value()) : dc.max_power_limit;
...
din::ev::Result CurrentDemand::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        logf_warning("CurrentDemand message timeout reached, terminating session");
    ...
    if (const auto res = variant->get_if<message_din::CurrentDemandResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("CurrentDemand failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
        ...
        // SECC-driven stop (EVSE_Shutdown/EVSE_EmergencyShutdown / EVSENotification StopCharging).
    ...
    logf_warning("Expected CurrentDemandRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
