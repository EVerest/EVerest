...
din::ev::Result WeldingDetection::feed(Event ev) {
    ...
    if (ev == Event::TIMEOUT) {
        ...
        if (timeout and *timeout == d20::TimeoutType::ONGOING) {
            logf_warning("WeldingDetection ongoing timeout reached, terminating session");
            ongoing_timeout_reached = true;
        } else {
            logf_warning("WeldingDetection message timeout reached, terminating session");
        }
    ...
    if (const auto res = variant->get_if<message_din::WeldingDetectionResponse>()) {
        ...
        if (not result.valid) {
            logf_warning("WeldingDetection failed, terminating session");
            m_ctx.session_stopped = true;
            return {};
        }
    ...
    logf_warning("Expected WeldingDetectionRes! But code type id: %d", variant->get_type());
    m_ctx.session_stopped = true;
    return {};
}
