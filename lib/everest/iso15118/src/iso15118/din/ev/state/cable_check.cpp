...
Result handle_response(const message_din::CableCheckResponse& res) {
    ...
    result.evse_ready = (res.dc_evse_status.evse_status_code == dt::DcEvseStatusCode::EVSE_Ready);
    // EvseV2G proceeds to PreCharge on Valid OR Warning (din_server.cpp:800); only Invalid/Fault (or a
    // missing status) terminate.
    const auto& isolation = res.dc_evse_status.evse_isolation_status;
    result.isolation_valid = isolation.has_value() and (isolation.value() == dt::IsolationLevel::Valid or
                                                        isolation.value() == dt::IsolationLevel::Warning);
    return result;
}
...
din::ev::Result CableCheck::feed(Event ev) {
    ...
    if (const auto res = variant->get_if<message_din::CableCheckResponse>()) {
        ...
        case Action::Done:
            ...
            if (not result.evse_ready or not result.isolation_valid) {
                m_ctx.log("CableCheck finished but EVSE not ready or isolation invalid, terminating session");
                m_ctx.session_stopped = true;
                return {};
            }
