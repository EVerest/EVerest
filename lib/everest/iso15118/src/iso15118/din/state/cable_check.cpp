...
message_din::CableCheckResponse handle_request(const message_din::CableCheckRequest& req, bool cable_check_done,
    ...
    // The module's isolation-monitoring result (update_isolation_status) takes precedence over the level
    // derived from the cable check's own progress: only the module can tell Warning and Fault apart, and
    // EvseManager reports it before signalling cable_check_finished in every path. Without it the
    // progress derivation stands, so an EVSE that never reports keeps the previous behaviour. EvseV2G
    // likewise copies its module value into CableCheckRes (din_server.cpp). No_IMD does not reach here:
    // DIN's isolationLevelType has no such enumerator, so the context maps it to Valid.
    ...
    // signal EVSEStatusCode = EVSE_Shutdown here too (EvseV2G stamps its context status code into every
    // response after handle_stop_charging; the EVSENotification stays None for DC [V2G-DC-500]).
    if (charger_stop) {
...
Result CableCheck::process_request(const message_din::CableCheckRequest& req) {
    ...
    const auto res =
        handle_request(req, m_ctx.cable_check_done, m_ctx.cable_check_fault, m_ctx.get_session_id(),
                       m_ctx.error_status_code(), m_ctx.charger_stop_requested, m_ctx.reported_isolation_level());
    m_ctx.respond(res);
...
Result CableCheck::feed(Event ev) {
    ...
    if (const auto req = variant->get_if<message_din::CableCheckRequest>()) {
        // [V2G-DC-391]: validate the SessionID before the CP gate and any side effect -- a request from
        // an unknown session must neither start the isolation test nor be parked (FAILED_UnknownSession
        // is due immediately).
        ...
        // the request until the CP state is reported or the timeout fails it.
    ...
    logf_warning("Expected CableCheckReq! But code type id: %d", variant->get_type());
