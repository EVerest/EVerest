...
message_din::ServiceDiscoveryResponse handle_request(const message_din::ServiceDiscoveryRequest& req,
    ...
    // ChargeService and PaymentOptionList are mandatory in ServiceDiscoveryRes and must be present even on
    // a FAILED_UnknownSession response (TC_SECC_VTB_ServiceDiscovery_003), so populate them before the
    // SessionID check below.
