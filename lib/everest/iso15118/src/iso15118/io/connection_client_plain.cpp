...
ConnectionClientPlain::~ConnectionClientPlain() {
    // Release the socket even if close() was never called (e.g. the session is dropped before a clean
    // teardown), so the fd does not leak. No CLOSED callback is fired during destruction.
    if (connection_open) {
...
ReadResult ConnectionClientPlain::read(uint8_t* buf, size_t len) {
    ...
    if (read_result == 0 && len > 0) {
        // A zero-length read on a stream socket means the peer closed the connection (EOF); surface it so
        // the session tears down instead of spinning on an apparently-blocked read (mirrors the server).
        return {false, 0, true};
    }
    ...
    // EAGAIN/EWOULDBLOCK/EINTR mean "retry"; anything else (e.g. ECONNRESET from a RST) is terminal,
    // so report it as a closed connection -- otherwise the level-triggered poll would spin on the
    // dead socket instead of tearing the session down (mirrors the server-side ConnectionPlain).
    if (errno == EAGAIN or errno == EWOULDBLOCK or errno == EINTR) {
        return {true, 0, false};
    }

    logf_warning("ConnectionClientPlain::read failed with error code: %d", errno);
    return {false, 0, true};
}
