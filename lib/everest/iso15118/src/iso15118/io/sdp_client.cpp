...
std::optional<SdpResponse> SdpClient::handle_response() {
    ...
    // The DECLARED payload length must match too, not just the datagram size: a header claiming a
    // different length than the SDP response payload it carries is malformed and must be ignored
    // ([V2G2-152], [V2G2-153]), so that discovery keeps retransmitting instead of connecting on the
    // strength of a frame we could not trust.
    if (not cmp_equal(sdp_payload_len, SDP_RESPONSE_PAYLOAD_LEN)) {
        logf_warning("Sdp client received a response whose declared payload length is %u, expected %u", sdp_payload_len,
                     static_cast<unsigned>(SDP_RESPONSE_PAYLOAD_LEN));
        return std::nullopt;
    }
