...
Controller::Controller(EvConfig config_, feedback::Callbacks callbacks_, DcChargeParams initial_dc_params,
    ...
    session = std::make_unique<Session>(
        ...
        &dc_params, &ac_params, config.energy_service, make_session_options(config, std::move(offer)), config.params);
...
void Controller::establish_data_path(const iso15118::io::Ipv6EndPoint& endpoint,
    ...
    guarded("establishing the data path", [&]() {
        ...
        // The Session exists before SDP has run, so the engines learn the transport here rather
        // than from a constructor argument. This is the first point at which it is known and it is
        // before any engine reads it.
        session->set_transport_security(security);
