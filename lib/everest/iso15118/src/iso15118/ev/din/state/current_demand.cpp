...
Result CurrentDemand::feed(Event ev) {
    ...
    m_ctx.feedback.dc_evse_present_values(static_cast<float>(res->evse_present_voltage),
                                          static_cast<float>(res->evse_present_current));
    ...
    if (res->dc_evse_status.evse_notification == dt::EvseNotification::ReNegotiation and not renegotiation_warned) {
        logf_warning("SECC requested renegotiation, which DIN 70121 does not support; continuing the charge loop");
        renegotiation_warned = true;
    }
