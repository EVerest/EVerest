...
Result ChargeParameterDiscovery::feed(Event ev) {
    ...
    if (result.finished and not res->dc_evse_charge_parameter.has_value()) {
        logf_error("ChargeParameterDiscovery finished without a DC_EVSEChargeParameter, stopping the session");
        m_ctx.stop_session();
        return Result::stopping();
    }
