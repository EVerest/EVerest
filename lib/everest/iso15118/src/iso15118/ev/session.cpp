...
Session::Session(feedback::Callbacks callbacks, OutboundSend outbound_send_,
    ...
    context(std::move(callbacks), message_exchange, std::move(evcc_id), std::move(advertised_app_protocols),
...
template <typename F> void Session::guarded(const char* op, F&& f) {
    // Single reactor exception boundary: poll_impl has no try/catch, so an escape would
    // kill the reactor thread and the session would neither finish nor signal.
    try {
        f();
