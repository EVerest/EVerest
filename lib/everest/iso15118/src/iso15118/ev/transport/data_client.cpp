...
constexpr everest::lib::io::tcp::SourcePortRange EVCC_SOURCE_PORTS{49152, 65535};
...
void DataClient::connect(const iso15118::io::Ipv6EndPoint& endpoint, const std::string& device,
    ...
    const auto wire = [this](auto& transport) {
        ...
        // A failed connect is reported only through this handler, never through a return value.
...
bool DataClient::register_events() {
    if (registered) {
        return true;
    }
    ...
    const auto ok = handler.register_event_handler(obj);
    if (not ok) {
