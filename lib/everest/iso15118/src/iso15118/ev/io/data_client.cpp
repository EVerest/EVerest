...
void DataClient::connect(const iso15118::io::Ipv6EndPoint& endpoint, std::function<void()> on_connected_,
    ...
    // The scope for link-local addresses is supplied via SO_BINDTODEVICE; the
    // loopback path needs none, so the device argument is left empty here.
