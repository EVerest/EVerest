...
void Feedback::dc_evse_present_values(float voltage, float current) const {
    call_if_available(callbacks.dc_evse_present_values, voltage, current);
}
...
void Feedback::der_control_scheduled(const message_20::datatypes::DER_Scheduled_AC_CLResControlMode& mode) const {
    call_if_available(callbacks.der_control_scheduled, mode);
}
