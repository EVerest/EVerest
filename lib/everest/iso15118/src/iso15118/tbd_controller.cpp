...
StartSessionResult TbdController::start_session(int connected_fd, const StartSessionOptions& start_options) {
    ...
    if (start_options.skip_app_protocol_negotiation) {
        // The multi-protocol session driver always starts on the SupportedAppProtocol engine; there is
        // no entry point behind it yet. Run the negotiation rather than refuse the session: a peer that
        // already negotiated once answers the repeated SupportedAppProtocolReq identically.
        logf_warning("start_session: skip_app_protocol_negotiation is not supported by the multi-protocol "
                     "session driver; running the negotiation anyway");
    }
    auto connection =
        std::make_unique<io::ConnectionPlain>(poll_manager, connected_fd, start_options.vehicle_cert_hash);
    session = std::make_unique<Session>(std::move(connection), session::SessionConfig(*evse_setup.handle()), callbacks,
                                        pause_ctx, d2_pause_ctx);
    shutdown_active.store(false);
    shutdown_signaled = false;
