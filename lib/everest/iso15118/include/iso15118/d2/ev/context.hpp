...
struct EvseInfo {
    ...
    // Payment options / Certificate service advertised by the SECC (from ServiceDiscoveryRes), used to
    // pick EIM vs Contract (Plug-and-Charge) in PaymentServiceSelection.
...
class Context {
public:
    Context(session::ev::feedback::Callbacks, EvSessionConfig,
    ...
    // Send an already-encoded request EXI verbatim (Plug-and-Charge: the EV signs the
    // AuthorizationReq/MeteringReceiptReq/CertificateInstallationReq itself, so it bypasses the generic
    // codec and stages the signed bytes). `type` is reported for logging only.
    void send_raw(const std::vector<uint8_t>& exi, message_2::Type type) {
        message_exchange.set_raw_response(exi.data(), exi.size(), type);
    }
    ...
    // Plug-and-Charge runtime state, populated during the handshake.
    struct PnCState {
        // Set once PaymentServiceSelection selected Contract; drives the signed AuthorizationReq.
        bool contract_selected{false};
        // GenChallenge received in PaymentDetailsRes, echoed into the signed AuthorizationReq [V2G2-684].
        dt::GenChallenge gen_challenge{};
        // The contract certificate + key actually used for PaymentDetails and AuthorizationReq signing:
        // either preloaded from config or obtained via CertificateInstallation.
