...
// CertificateInstallation/Update: the SECC relays the request to a CPS/CSMS backend, so the response can
// take noticeably longer than a locally-answered message. Kept generous to tolerate that round-trip.
constexpr uint32_t MESSAGE_TIMEOUT_CERTIFICATE_INSTALL_MS = 5000;
...
// V2G_EVCC_PreCharge_Timeout. ISO 15118-2 Table 109 specifies 7 s; deliberately relaxed to 10 s here to
// tolerate slower power supplies / EVs (intentional deviation from the standard).
