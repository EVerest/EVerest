...
public:
    ...
    // Relay-only splice: stage an already-encoded response EXI verbatim (Plug-and-Charge
    // CertificateInstallationRes delivered by the backend). Bypasses the message_2 codec; the payload
    // type is the shared SAP type and the message type stays None (protocol-neutral, like every d2 response).
    void set_raw_response(const uint8_t* data, size_t len);
