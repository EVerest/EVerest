...
struct Result {
    ...
    // Contract (Plug-and-Charge) is offered in the payment_option_list.
    bool contract_offered{false};
    // The Certificate service (ServiceID 2) is advertised in the service_list (used for
    // CertificateInstallation/Update).
