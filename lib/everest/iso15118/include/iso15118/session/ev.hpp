...
public:
    ...
    // Terminate the session immediately, without the SessionStop exchange: the control pilot left
    // the Bx/Cx/Dx range (CP state E/F) or the cable was unplugged, so ISO 15118-2 [V2G2-025] /
    // [V2G2-728] require the EVCC to end the V2G communication session right away. Closes the
    // connection and publishes the end-of-session feedback exactly once.
    void terminate();
...
private:
    ...
    // output buffer; sized like the SECC response buffer — a PaymentDetailsReq carrying a contract
    // certificate chain (leaf + up to 4 sub certificates a 800 bytes) plus signature header exceeds 1 KiB
    ...
    // End-of-session handling, exactly once per session: closes the connection (if still up) and
    // publishes v2g_session_finished + the DLINK signal. No-op unless is_finished().
    void signal_session_finished_if_done();
