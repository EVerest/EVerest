...
// \p tls_active gates plaintext-only protocols: DIN SPEC 70121 is not offered over TLS [V2G-DC-869].
// The converse is NOT enforced here: ISO 15118-20 mandates TLS [V2G20-2677], but it is still negotiated
// on a plain connection so that bring-up and ENFORCE_NO_TLS deployments keep working; SapEngine logs the
// conformance deviation instead.
