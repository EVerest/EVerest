...
public:
    ...
    // Thread-safe. Report the control pilot state the vehicle currently applies. The stack has no
    // board support connection of its own, so whoever owns it pushes the state here; the first call
    // also tells the stack that the pilot IS being reported, which is what lets the DC cable check
    // wait for state C/D ([V2G2-847]) instead of sending straight away. The value is remembered and
    // replayed into each new session, since the interesting transitions happen before a session
    // exists to deliver control events to.
    void set_cp_state(bool c_or_d);

    // Thread-safe. End the V2G communication session immediately, WITHOUT the SessionStop
    // exchange stop_charging() performs: the control pilot left the Bx/Cx/Dx range (CP state E/F)
    // or the cable was unplugged. ISO 15118-2 [V2G2-025] / [V2G2-728] require the EVCC to
    // terminate the session as soon as it detects that -- there is no pilot left to run high
    // level communication on. A pending SDP/connect attempt is cancelled the same way.
...
private:
    ...
    // terminate_session() request, picked up by the loop. Not a ControlEvent: it must take effect
    // whatever state the engine is in, including while no session exists yet.
    std::atomic_bool terminate_requested{false};
    // Last control pilot state reported via set_cp_state(), replayed into every new session.
    std::atomic_bool cp_state_c_or_d{false};
