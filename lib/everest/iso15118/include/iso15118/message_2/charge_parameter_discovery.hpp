...
// ISO 15118-2 SalesTariffEntryType (subset). The nested ConsumptionCost/Cost detail is not
// modelled, because the EVCC does not act on cost, but the tariff structure is decoded rather
// than dropped [V2G2-659].
...
struct SalesTariff {
    uint8_t sales_tariff_id{0};
    std::optional<std::string> sales_tariff_description;
    std::optional<uint8_t> num_e_price_levels;
    everest::lib::util::fixed_vector<SalesTariffEntry, 12> sales_tariff_entry;
};
