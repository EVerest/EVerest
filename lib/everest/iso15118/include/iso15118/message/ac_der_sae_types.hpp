...
// Trip curve, AMD1 M.2.2.1.10 and M.2.2.1.11: x is the duration in s. y depends on the family (%V or V for
// voltage trips, Hz for frequency trips), so its default only covers partial initialization; producers assign both.
struct DERCurve {
    bool enable{false};
    std::optional<uint16_t> priority;
