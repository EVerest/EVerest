...
struct Codec {
    ...
    // Every ISO 15118-2 message travels as V2GTP payload type 0x8001.
    template <typename Msg> static constexpr io::v2gtp::PayloadType payload_type_of() {
        return io::v2gtp::PayloadType::SAP;
    }
};
...
public:
    ...
    void send_raw(std::vector<uint8_t> exi, message_2::Type type) {
        message_exchange.set_raw_request(std::move(exi), type, io::v2gtp::PayloadType::SAP);
    }
