...
struct Result {
    // The offered ChargeService carries the EV's requested energy transfer mode exactly.
    bool charge_service_supported{false};
    // The offered mode carries DC at all, which the combo AC+DC values also do.
    bool charge_service_offers_dc{false};
