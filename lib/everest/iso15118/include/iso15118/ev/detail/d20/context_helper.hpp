...
using ev::disposition_violation;
...
// session, and returns nullptr; the caller returns {} straight away. Collapses the
// per-state validation skeleton and makes the session-id check impossible to omit.
template <typename Res> const Res* expect_response(Context& ctx, const message_20::Variant& variant) {
    ...
    if (not check_response_code(code)) {
