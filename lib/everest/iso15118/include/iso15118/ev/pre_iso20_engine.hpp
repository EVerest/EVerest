 ...
 *
 * `Traits` supplies: `Context`, `StateBase`, `StateID`, `Variant`, `MessageExchange`,
 * `InitialState`, `static constexpr ProtocolId PROTOCOL`, `static const char* name()`, and a
 * `Timeouts` type with `response_timeout(StateID)`, `ongoing_timeout(StateID)`, `MESSAGE` and
