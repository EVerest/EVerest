...
// Payload types the ISO 15118-20 Variant decodes. ISO 15118-20 spreads its messages over several
// of them, so unlike the pre-20 generations this is a set rather than a single value.
...
public:
    ...
    // Decodes @p view by payload type and stages it as the pending response. False when the payload
    // type is not one this generation decodes; the frame is then ignored [V2G20-800].
