...
struct Callbacks {
    ...
    // Live voltage and current the SECC reports in the DC charge loop, in V and A.
    std::function<void(float, float)> dc_evse_present_values;
...
public:
    ...
    void dc_evse_present_values(float voltage, float current) const;
