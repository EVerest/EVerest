...
public:
    EvFsmHelper(din::ev::EvSessionConfig config, const session::ev::feedback::Callbacks& callbacks) :
        ctx(callbacks, std::move(config), active_control_event, msg_exch, timeouts) {
...
SCENARIO("EVCC DIN FSM terminates (does not pause) on an EV-initiated PauseCharging control event") {
    // DIN SPEC 70121 has no charging pause/resume [V2G-DC-241]: a PauseCharging request is honoured as a
    // normal terminate, and the session must end (never enter the paused state).
    session::ev::feedback::Callbacks callbacks{};
...
SCENARIO("EVCC DIN SessionSetup ignores a configured resume id and starts a fresh session") {
    // DIN SPEC 70121 has no charging pause/resume [V2G-DC-241]: even if a resumed_session_id is configured
    // (leaked in from the shared config), the DIN EVCC must start a new session with SessionID=0.
    session::ev::feedback::Callbacks callbacks{};
    ...
    fsm.feed(Event::SEND_REQUEST);
    {
        ...
        REQUIRE(req.has_value());
        // The configured resume id is ignored; a fresh session uses an all-zero SessionID.
