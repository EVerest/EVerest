...
SCENARIO("EVCC SupportedAppProtocol multi-protocol offer builder") {
    ...
    GIVEN("An EV offering ISO 15118-2 and DIN for a DC service") {
        const auto req =
            sap::create_request({ProtocolId::ISO15118_2, ProtocolId::DIN70121}, {dt::ServiceCategory::DC}, std::nullopt);
    ...
    GIVEN("An EV offering ISO 15118-2 and DIN for an AC service") {
        const auto req =
            sap::create_request({ProtocolId::ISO15118_2, ProtocolId::DIN70121}, {dt::ServiceCategory::AC}, std::nullopt);
