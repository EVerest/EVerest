...
SCENARIO("EVCC DIN CableCheck request/response handling") {
    ...
    // [V2G-DC-893/894]: on Finished the EVCC disregards EVSEIsolationStatus entirely and every
    // EVSEStatusCode except EVSE_Shutdown / EVSE_EmergencyShutdown before sending PreChargeReq.
    GIVEN("A Finished response with EVSE ready and valid isolation") {
        ...
        THEN("Done and no shutdown -> proceed") {
            REQUIRE(result.action == Action::Done);
            REQUIRE(not result.evse_shutdown);
        }
    }

    GIVEN("A Finished response with a non-shutdown status code and invalid isolation") {
        message_din::CableCheckResponse res;
        res.response_code = dt::ResponseCode::OK;
        res.evse_processing = dt::EvseProcessing::Finished;
        res.dc_evse_status.evse_status_code = dt::DcEvseStatusCode::EVSE_Ready;
        ...
        const auto result = handle_response(res);
        THEN("Isolation is disregarded -> still proceed") {
            REQUIRE(result.action == Action::Done);
            REQUIRE(not result.evse_shutdown);
        }
    }

    GIVEN("A Finished response with EVSE_Shutdown") {
        message_din::CableCheckResponse res;
        res.response_code = dt::ResponseCode::OK;
        res.evse_processing = dt::EvseProcessing::Finished;
        ...
        res.dc_evse_status.evse_isolation_status = dt::IsolationLevel::Valid;
        const auto result = handle_response(res);
        THEN("Done but shutdown -> terminate") {
            REQUIRE(result.action == Action::Done);
            REQUIRE(result.evse_shutdown);
        }
    }

    GIVEN("A Finished response with EVSE_EmergencyShutdown") {
        message_din::CableCheckResponse res;
        res.response_code = dt::ResponseCode::OK;
        res.evse_processing = dt::EvseProcessing::Finished;
