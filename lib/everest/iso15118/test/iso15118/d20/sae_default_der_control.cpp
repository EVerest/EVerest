    ...
    GIVEN("The default SAE DER control") {
        ...
        THEN("Every curve carries the expected units") {
            ...
            REQUIRE(der_control.reactive_power_support.watt_var.y_unit ==
                    sae::DERUnit::PercentageEVMaximumConfiguredReactivePower);
            REQUIRE(der_control.reactive_power_support.constant_var.unit ==
                    sae::DERUnit::PercentageEVMaximumConfiguredReactivePower);
        ...
        THEN("The enter service thresholds and setpoints match the expected defaults") {
            REQUIRE_THAT(der_control.enter_service.enter_service_voltage_high,
                         WithinAbs(1.05f * NOMINAL_VOLTAGE_V, TOLERANCE));
            REQUIRE_THAT(der_control.enter_service.enter_service_voltage_low,
                         WithinAbs(0.917f * NOMINAL_VOLTAGE_V, TOLERANCE));
            ...
            REQUIRE_THAT(der_control.reactive_power_support.volt_var.reference_voltage,
        ...
        THEN("The frequency droop carries an inert over frequency branch and no under frequency branch") {
            // Table M.33: when OverFrequencyDroop is absent, UnderFrequencyDroop shall be present. Carrying
            // the over frequency branch with zeroed settings satisfies that unconditionally and stays inert.
            const auto& frequency_droop = der_control.active_power_support.frequency_droop;
            REQUIRE(frequency_droop.enable == false);
            REQUIRE(frequency_droop.over_frequency_droop.has_value() == true);
            REQUIRE_THAT(frequency_droop.over_frequency_droop.value().db, WithinAbs(0.0f, TOLERANCE));
            REQUIRE_THAT(frequency_droop.over_frequency_droop.value().droop_factor, WithinAbs(0.0f, TOLERANCE));
            REQUIRE(frequency_droop.over_frequency_droop.value().power_reference ==
                    sae::PowerReference::MaximumActivePower);
            REQUIRE_THAT(frequency_droop.over_frequency_droop.value().open_loop_response_time,
                         WithinAbs(0.0f, TOLERANCE));
            REQUIRE(frequency_droop.under_frequency_droop.has_value() == false);
        }
    }

    GIVEN("The inert default at 120 V") {
        const auto der_control = iso15118::d20::get_default_sae_der_control(120.0f);

        THEN("The EnterService bands and the VoltVar reference voltage follow the nominal") {
            REQUIRE_THAT(der_control.enter_service.enter_service_voltage_high, WithinAbs(126.0f, 1e-2f));
            REQUIRE_THAT(der_control.enter_service.enter_service_voltage_low, WithinAbs(110.04f, 1e-2f));
            REQUIRE_THAT(der_control.reactive_power_support.volt_var.reference_voltage, WithinAbs(120.0f, 1e-2f));
        }
    }
}
...
create_ac_der_sae_evse_setup(const std::optional<iso15118::d20::SaeDerTransferLimits>& sae_limits,
    ...
    iso15118::session::EvseSetupConfig setup{};
...
SCENARIO("SessionConfig fills in a default SAE DER setup config") {
    ...
    const auto offers_ac_der_sae = [](const iso15118::session::SessionConfig& config) {
    ...
    GIVEN("AC_DER_SAE with limits but no setup config") {
        const auto config = iso15118::session::SessionConfig(
    ...
    GIVEN("AC_DER_SAE without limits") {
        const auto config = iso15118::session::SessionConfig(create_ac_der_sae_evse_setup(std::nullopt, std::nullopt));
    ...
    GIVEN("AC_DER_SAE with a caller supplied setup config") {
        ...
        const auto config = iso15118::session::SessionConfig(
            create_ac_der_sae_evse_setup(iso15118::d20::SaeDerTransferLimits{}, supplied));
...
iso15118::session::SessionConfig make_session_config(const iso15118::d20::SaeDerTransferLimits& limits,
    ...
    return iso15118::session::SessionConfig(create_ac_der_sae_evse_setup(limits, setup));
}

bool offers_ac_der_sae(const iso15118::session::SessionConfig& config) {
    namespace dt = iso15118::message_20::datatypes;
...
SCENARIO("SessionConfig validates a caller supplied SAE DER setup config") {
    ...
    GIVEN("The unmodified shipped default") {
        THEN("The service is kept") {
            REQUIRE(session_config_offers_ac_der_sae(
                        valid_limits, iso15118::d20::get_default_sae_der_control(NOMINAL_VOLTAGE_V)) == true);
        }
    }
    ...
    GIVEN("A positive var injection limit") {
        ...
        THEN("The service is stripped") {
            REQUIRE(session_config_offers_ac_der_sae(
                        limits, iso15118::d20::get_default_sae_der_control(NOMINAL_VOLTAGE_V)) == false);
        }
    }
    ...
    GIVEN("A positive maximum discharge power") {
        ...
        const auto config = make_session_config(limits, iso15118::d20::get_default_sae_der_control(NOMINAL_VOLTAGE_V));
    ...
    GIVEN("Sign conformant limits") {
        ...
        THEN("The service is kept") {
            REQUIRE(session_config_offers_ac_der_sae(
                        limits, iso15118::d20::get_default_sae_der_control(NOMINAL_VOLTAGE_V)) == true);
        }
    }
}

SCENARIO("SessionConfig checks the SAE nominals against the maxima at offer time") {

    const auto make_config_with_ac_limits = [](const iso15118::d20::SaeDerTransferLimits& limits,
                                               const iso15118::d20::AcTransferLimits& ac_limits) {
        auto setup = create_ac_der_sae_evse_setup(
            limits, iso15118::d20::DerSaeSetupConfig(iso15118::d20::get_default_sae_der_control(NOMINAL_VOLTAGE_V),
                                                     sae::RequiredDEROperatingMode::GridFollowing,
                                                     sae::GridConnectionMode::GridConnected));
        setup.ac_limits = ac_limits;
        return iso15118::session::SessionConfig(setup);
    };

    iso15118::d20::AcTransferLimits ac_limits{};
    ac_limits.charge_power.max = {11, 3};
    ac_limits.charge_power.min = {0, 0};

    iso15118::d20::SaeDerTransferLimits limits{};
    limits.max_discharge_power = {-5, 3};

    GIVEN("A nominal discharge power whose magnitude exceeds the maximum discharge power") {
        limits.nominal_discharge_power = {-7, 3};

        THEN("The service is stripped") {
            REQUIRE(offers_ac_der_sae(make_config_with_ac_limits(limits, ac_limits)) == false);
        }

        THEN("The violation names the quantity") {
            const auto violation = iso15118::d20::validate_sae_der_setup(
                iso15118::d20::make_inert_default_sae_setup_config(NOMINAL_VOLTAGE_V), limits, ac_limits);
            REQUIRE(violation.has_value() == true);
            REQUIRE(violation.value().find("nominal_discharge_power") != std::string::npos);
        }
    }

    GIVEN("A nominal charge power exceeding the maximum charge power") {
        limits.nominal_charge_power = {12, 3};
        ...
        THEN("The violation names the quantity") {
            const auto violation = iso15118::d20::validate_sae_der_setup(
                iso15118::d20::make_inert_default_sae_setup_config(NOMINAL_VOLTAGE_V), limits, ac_limits);
            REQUIRE(violation.has_value() == true);
            REQUIRE(violation.value().find("nominal_charge_power") != std::string::npos);
        }
    }

    GIVEN("A per phase nominal charge power without the corresponding maximum") {
        limits.nominal_charge_power_L2 = {1, 3};

        THEN("The service is stripped") {
            REQUIRE(offers_ac_der_sae(make_config_with_ac_limits(limits, ac_limits)) == false);
        }

        THEN("The violation names the missing maximum") {
            const auto violation = iso15118::d20::validate_sae_der_setup(
                iso15118::d20::make_inert_default_sae_setup_config(NOMINAL_VOLTAGE_V), limits, ac_limits);
            REQUIRE(violation.has_value() == true);
            REQUIRE(violation.value().find("nominal_charge_power_L2") != std::string::npos);
            REQUIRE(violation.value().find("charge_power_L2") != std::string::npos);
        }
    }

    GIVEN("A per phase nominal discharge power without the corresponding maximum") {
        limits.nominal_discharge_power_L3 = {-1, 3};
    ...
    GIVEN("Nominals within their maxima") {
        limits.nominal_charge_power = {11, 3};
        limits.nominal_discharge_power = {-5, 3};
        ac_limits.charge_power_L2 =
            iso15118::d20::Limit<iso15118::message_20::datatypes::RationalNumber>{{4, 3}, {0, 0}};
        limits.nominal_charge_power_L2 = {3, 3};
        limits.max_discharge_power_L2 = {-4, 3};
        limits.nominal_discharge_power_L2 = {-3, 3};

        THEN("The service is kept") {
            REQUIRE(offers_ac_der_sae(make_config_with_ac_limits(limits, ac_limits)) == true);
        }
    }
}
...
SCENARIO("Replacing the offered energy services revalidates the SAE DER setup") {
    ...
    GIVEN("A session config without SAE DER limits") {
        auto config = iso15118::session::SessionConfig(create_ac_der_sae_evse_setup(std::nullopt, std::nullopt));
        REQUIRE(offers_ac_der_sae(config) == false);
    ...
    GIVEN("A session config with a conformant AC_DER_SAE offer") {
        auto config = make_session_config(iso15118::d20::SaeDerTransferLimits{},
                                          iso15118::d20::get_default_sae_der_control(NOMINAL_VOLTAGE_V));
        REQUIRE(offers_ac_der_sae(config) == true);
...
SCENARIO("The SAE DER validator names the violated rule") {
    ...
    const iso15118::d20::SaeDerTransferLimits valid_limits{};
    const auto valid_setup = iso15118::d20::make_inert_default_sae_setup_config(NOMINAL_VOLTAGE_V);
    const iso15118::d20::AcTransferLimits valid_ac_limits{};

    GIVEN("A conformant setup") {
        THEN("No violation is reported") {
            REQUIRE(iso15118::d20::validate_sae_der_setup(valid_setup, valid_limits, valid_ac_limits).has_value() ==
                    false);
        }
    }
    ...
    GIVEN("A sign violating limit") {
        ...
        THEN("The violation names the limit and the expected sign") {
            const auto violation = iso15118::d20::validate_sae_der_setup(valid_setup, limits, valid_ac_limits);
            REQUIRE(violation.has_value() == true);
