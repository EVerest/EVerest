...
SCENARIO("DIN SECC ContractAuthentication state handling") {
    ...
    GIVEN("The auth timeout elapsed without any authorization result") {
        const auto res = din::state::handle_request(false, session, false, true);
        THEN("FAILED and Finished (not Ongoing forever)") {
            REQUIRE(res.response_code == dt::ResponseCode::FAILED);
            REQUIRE(res.evse_processing == dt::EvseProcessing::Finished);
        }
    ...
    GIVEN("Authorized just as the timeout elapsed") {
        const auto res = din::state::handle_request(true, session, false, true);
        THEN("The timeout wins: a session already declared failed must not proceed to charging") {
            REQUIRE(res.response_code == dt::ResponseCode::FAILED);
            REQUIRE(res.evse_processing == dt::EvseProcessing::Finished);
        }
    }
