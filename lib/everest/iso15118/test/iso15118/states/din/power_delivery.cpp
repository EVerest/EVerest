...
SCENARIO("DIN SECC PowerDelivery state handling") {
    ...
    GIVEN("A module-reported malfunction while the EV asks to start charging") {
        ...
        const auto res =
            din::state::handle_request(req, session, /*charger_stop=*/false, dt::DcEvseStatusCode::EVSE_Malfunction);
        THEN("FAILED_PowerDeliveryNotApplied with EVSE_Malfunction [V2G-DC-401]") {
    ...
    GIVEN("A module-reported malfunction while the EV ends power delivery") {
        message_din::PowerDeliveryRequest req;
        req.header.session_id = session;
        req.ready_to_charge_state = false;
