...
SCENARIO("ISO 15118-2 SECC ChargingStatus handling") {
    ...
    GIVEN("An EIM session with a meter reading") {
        ...
        THEN("MeterInfo is reported although no receipt is requested") {
            REQUIRE(res.receipt_required.has_value());
            REQUIRE(res.receipt_required.value() == false);
            REQUIRE(res.meter_info.has_value());
            REQUIRE(res.meter_info->meter_id == "PNX-METER-1");
    ...
    GIVEN("No meter reading has been forwarded yet") {
        const auto res = d2::state::handle_request(req, id, config, 1, false, false, std::nullopt);
