...
SCENARIO("ISO 15118-2 SECC CurrentDemand handling") {
    ...
    // will sign; without a receipt the MeterInfo is omitted.
    dt::MeterInfo meter{};
    meter.meter_id = "PNX-METER-1";
    meter.meter_reading = 4211U;

    GIVEN("A receipt is requested and meter info is available") {
        const auto res = d2::state::handle_request(req, id, config, 400.0f, 20.0f, 1, false, true, meter);
    ...
    GIVEN("No receipt is requested but meter info is available") {
        const auto res = d2::state::handle_request(req, id, config, 400.0f, 20.0f, 1, false, false, meter);
