...
SCENARIO("ISO15118-20 EV ServiceDetail transitions to ServiceSelection with chosen parameter set") {
    ...
    primed.handle_response(make_response(SESSION_HEADER, ResponseCode::OK, 7));
...
SCENARIO("ISO15118-20 EV ServiceDetail stops session on empty parameter list") {
    ...
    auto res = make_response(SESSION_HEADER, ResponseCode::OK, 0);
...
SCENARIO("ISO15118-20 EV ServiceDetail rejects malformed responses") {
    ...
    const auto make_ok = [](const message_20::Header& header) { return make_response(header, ResponseCode::OK, 1); };
