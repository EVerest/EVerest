...
SCENARIO("ISO15118-20 EV DC_PreCharge sends initial Starting request on enter") {
    ...
    ctx.dc_pre_charge_target_voltage = message_20::datatypes::RationalNumber{4, 2}; // 400.0 V
    ...
    REQUIRE(request_message->target_voltage.value == 4);
    REQUIRE(request_message->target_voltage.exponent == 2);
}

SCENARIO("ISO15118-20 EV DC_PreCharge transitions to PowerDelivery on OK response") {
    const ev::d20::session::feedback::Callbacks callbacks{};
    auto state_helper = FsmStateHelper(callbacks);
    auto& ctx = state_helper.get_context();
    ctx.get_session().set_id(SESSION_HEADER.session_id);
    ...
    const auto res = message_20::DC_PreChargeResponse{SESSION_HEADER, message_20::datatypes::ResponseCode::OK,
                                                      message_20::datatypes::RationalNumber{1, 2}};
    ...
    // feed() queues the Finished precharge request and hands off to PowerDelivery, whose
    // enter() overwrites the single-slot exchange, so the pending request is that one.
    REQUIRE_FALSE(ctx.get_request<message_20::DC_PreChargeRequest>().has_value());
