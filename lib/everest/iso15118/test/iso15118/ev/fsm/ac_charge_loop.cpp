...
SCENARIO("ISO15118-20 EV AC_ChargeLoop states limits and the present measurement for a ThreePhase connector") {
    const ev::feedback::Callbacks callbacks{};
    ...
    REQUIRE(message_20::datatypes::from_RationalNumber(mode->max_charge_power) == 11100.0f);
    REQUIRE_FALSE(mode->max_charge_power_L2.has_value());
    REQUIRE_FALSE(mode->max_charge_power_L3.has_value());

    REQUIRE(message_20::datatypes::from_RationalNumber(mode->min_charge_power) == 900.0f);
    REQUIRE_FALSE(mode->min_charge_power_L2.has_value());

    // The present reading is one aggregate measurement, stated as the sum rather than invented
...
SCENARIO("ISO15118-20 EV AC_ChargeLoop fires stop_from_charger and drives PowerDelivery(Stop) on Terminate for BPT") {
    StopObserver obs;
    PrimedState<ev::d20::state::AC_ChargeLoop> primed{obs.callbacks, message_20::datatypes::ServiceCategory::AC_BPT,
                                                      seed_bpt_present_5000};
