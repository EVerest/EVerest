...
SCENARIO("ISO15118-2 EV Session runs a Plug & Charge contract session") {
    GIVEN("a DC Session preferring a pre-installed contract, over TLS") {
        auto params = walk_params();
        // A contract is only presented over a secure transport, so this walk is a TLS session; the
        // Controller sets this from the SDP response, and the fixture has no Controller.
...
SCENARIO("ISO15118-2 EV Session survives a frame larger than the old accumulator ceiling") {
    ...
    // measures 4143 bytes of EXI on the wire, well past the 2048-byte inbound V2GTP
    // accumulator that rejected such a frame as malformed. Framed under a payload type
    // the -2 engine ignores, so this asserts the transport ceiling alone and not
    // message semantics.
