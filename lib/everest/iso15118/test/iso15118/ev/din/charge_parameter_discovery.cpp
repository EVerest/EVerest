...
SCENARIO("DIN 70121 EV ChargeParameterDiscovery stops the session when a finished response omits the charge "
         "parameter") {
    bool power_ready = false;
    std::optional<ev::feedback::DcMaximumLimits> limits;
    ev::feedback::Callbacks callbacks{};
    ...
    // Without DC_EVSEChargeParameter there are no limits and no way to see an EVSE_Shutdown or a
    // StopCharging notification, so the charge cannot start.
    expect_stops_session(primed,
                         make_response(header(), dt::ResponseCode::OK, dt::EvseProcessing::Finished, std::nullopt),
                         StateID::ChargeParameterDiscovery);
    REQUIRE(power_ready == false);
