...
SCENARIO("DIN 70121 EV PowerDelivery sends ReadyToChargeState per phase") {
    ...
    SECTION("Stop") {
        ...
        // EVReady means "the EV is ready to charge" (ISO 15118-2 Table 99, which DIN reuses), so
        // the stop request must not carry it set alongside ReadyToChargeState=FALSE.
        REQUIRE(request->dc_ev_power_delivery_parameter.has_value());
