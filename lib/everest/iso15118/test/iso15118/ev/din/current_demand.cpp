...
SCENARIO("DIN 70121 EV CurrentDemand keeps charging through a renegotiation notification") {
    const ev::feedback::Callbacks callbacks{};
    PrimedState<State> primed{callbacks, seed_params};
    primed.take_requests();

    // DIN 70121 has no renegotiation state. The EV logs the SECC's request and carries on rather
    // than ending the session the way the ISO 15118-2 sibling does.
    expect_continues(primed, make_response(header(), dt::ResponseCode::OK, dt::DcEvseStatusCode::EVSE_Ready,
                                           dt::EvseNotification::ReNegotiation));
    expect_continues(primed, make_response(header(), dt::ResponseCode::OK, dt::DcEvseStatusCode::EVSE_Ready,
                                           dt::EvseNotification::ReNegotiation));
    REQUIRE(primed.ctx.is_session_stopped() == false);
}

SCENARIO("DIN 70121 EV CurrentDemand reports the SECC's present voltage and current every loop") {
    std::vector<std::pair<float, float>> reported;
    ev::feedback::Callbacks callbacks{};
    callbacks.dc_evse_present_values = [&reported](float v, float c) { reported.emplace_back(v, c); };
    PrimedState<State> primed{callbacks, seed_params};

    auto first = ok_response(header());
    first.evse_present_voltage = 398.0;
    first.evse_present_current = 120.0;
    expect_continues(primed, first);

    auto second = ok_response(header());
    second.evse_present_voltage = 401.0;
    second.evse_present_current = 124.0;
    expect_continues(primed, second);

    // Live values, not a one-shot: the module's SoC integrator closes its loop on them.
    REQUIRE(reported.size() == 2);
    REQUIRE(reported.at(0) == std::pair<float, float>{398.0f, 120.0f});
    REQUIRE(reported.at(1) == std::pair<float, float>{401.0f, 124.0f});
}
