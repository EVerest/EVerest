...
SCENARIO("Se/Deserialize power delivery messages") {
    ...
    GIVEN("Deserialize power_delivery_req") {
        ...
        THEN("It should be deserialized successfully") {
            REQUIRE(variant.get_type() == msg::d20::Type::PowerDeliveryReq);

            const auto& msg = variant.get<msg::d20::PowerDeliveryRequest>();
            ...
            REQUIRE(msg.charge_progress == msg::d20::datatypes::Progress::Start);
            ...
            REQUIRE(msg::d20::datatypes::from_RationalNumber(power_profile.entries[0].power) == 1000);

            REQUIRE(
                std::holds_alternative<msg::d20::datatypes::Scheduled_EVPPTControlMode>(power_profile.control_mode));
            const auto& mode = std::get<msg::d20::datatypes::Scheduled_EVPPTControlMode>(power_profile.control_mode);
            REQUIRE(mode.power_tolerance_acceptance == msg::d20::datatypes::PowerToleranceAcceptance::Confirmed);
    ...
    GIVEN("Serialize power_delivery_req") {
        ...
        req.processing = msg::d20::datatypes::Processing::Finished;
        req.charge_progress = msg::d20::datatypes::Progress::Start;
        req.power_profile = msg::d20::datatypes::PowerProfile{};
        req.power_profile->time_anchor = 0;
        req.power_profile->entries.emplace_back();
        req.power_profile->entries[0].duration = 23;
        req.power_profile->entries[0].power = msg::d20::datatypes::RationalNumber{10, 2};
        req.power_profile->control_mode.emplace<msg::d20::datatypes::Scheduled_EVPPTControlMode>();
        auto& mode = std::get<msg::d20::datatypes::Scheduled_EVPPTControlMode>(req.power_profile->control_mode);
        mode.power_tolerance_acceptance = msg::d20::datatypes::PowerToleranceAcceptance::Confirmed;
    ...
    GIVEN("Serialize power_delivery_res") {
        ...
        msg::d20::PowerDeliveryResponse res;

        res.header = msg::d20::Header{{0x3D, 0x4C, 0xBF, 0x93, 0x37, 0x4E, 0xD8, 0x9B}, 1725456333};
    ...
    GIVEN("Deserialize power_delivery_res") {
        ...
        THEN("It should be deserialized successfully") {
            REQUIRE(variant.get_type() == msg::d20::Type::PowerDeliveryRes);

            const auto& msg = variant.get<msg::d20::PowerDeliveryResponse>();
            const auto& header = msg.header;
