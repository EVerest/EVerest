...
SCENARIO("Se/Deserialize ISO-2 charging status messages") {
    ...
    GIVEN("Round-trip charging_status_res") {
        ...
        THEN("The wire carries the right unit, which from_physical_value does not read") {
            const auto& raw = decode_raw(serialize_helper(res)).V2G_Message.Body.ChargingStatusRes;

            REQUIRE(raw.EVSEMaxCurrent_isUsed);
            REQUIRE(raw.EVSEMaxCurrent.Unit == iso2_unitSymbolType_A);
        }
    }
}
