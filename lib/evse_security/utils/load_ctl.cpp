...
namespace evse_security {  
    void load_ctl(const fs::path& cert_dir,
                const std::map<CaCertificateType, fs::path>& ca_bundle_path_map) {


        
        // DC to CA type mapping
        static const std::map<std::string, CaCertificateType> dc_to_ca_type = {
            {"V2G",  CaCertificateType::V2G},
        ...
        for (const auto& entry : fs::directory_iterator(cert_dir)) {
            ...
            if (pkcs7) {
                ...
                if (certs && sk_X509_num(certs) > 0) {
                    EVLOG_info << "Extracted " << sk_X509_num(certs)
                            << " certificates from PKCS7: " << file_path;
            ...
            for (X509* cert : extracted_certs) {
                ...
                if (it != dc_to_ca_type.end()) {
                    ...
                    EVLOG_warning << "Could not determine CA type for cert with DC='"
                                << dc << "' in file: " << file_path;
                }
